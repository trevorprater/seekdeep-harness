...
#
# Environment: SEEKDEEP_WINE_NODE_MAJOR (default $PRIMARY_NODE_VERSION, then 24)
# picks the Windows Node line; SEEKDEEP_WINE_GATE_CACHE_DIR relocates the cache;
# SEEKDEEP_WINE_GATE_KEEP=1 preserves the scratch tree for inspection.

set -euo pipefail

repo_root="$(git rev-parse --show-toplevel)"
node_major="${SEEKDEEP_WINE_NODE_MAJOR:-${PRIMARY_NODE_VERSION:-24}}"
cache_dir="${SEEKDEEP_WINE_GATE_CACHE_DIR:-$repo_root/.cache/wine-windows}"
...
scratch="$(mktemp -d "${TMPDIR:-/tmp}/seekdeep-wine-gates.XXXXXX")"
cleanup() {
  wineserver -k > /dev/null 2>&1 || true
  if [ "${SEEKDEEP_WINE_GATE_KEEP:-0}" = '1' ]; then
