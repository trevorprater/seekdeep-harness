...
fn respond(status: u16, content_type: &str, body: &str) -> MockResponse {
    MockResponse::Respond(ResponseSpec::plain(
        status,
        content_type,
...
fn policy_validates_scheme_credentials_and_length() {
    ...
    assert_eq!(
        code(&validate_fetch_url("ftp://example.com", 2048.0).expect_err("ftp")),
        "WEB_INVALID_URL"
    );
    assert_eq!(
        code(&validate_fetch_url("not a url", 2048.0).expect_err("bad")),
        "WEB_INVALID_URL"
    );
    assert_eq!(
        code(&validate_fetch_url("https://user:pass@example.com", 2048.0).expect_err("creds")),
        "WEB_BLOCKED_URL"
    );
    let long = format!("https://example.com/{}", "a".repeat(3000));
    assert_eq!(
        code(&validate_fetch_url(&long, 2048.0).expect_err("long")),
        "WEB_INVALID_URL"
    );
}
...
fn policy_classifies_content_types() {
    assert_eq!(
        ...
        Some(seekdeep_web_fetch_http::FetchableKind::Html)
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(seekdeep_web_fetch_http::FetchableKind::Text)
    );
    assert_eq!(
...
fn policy_parses_charset() {
    ...
    assert_eq!(
        parse_charset(Some("text/plain; charset=\"iso-8859-1\"")).as_deref(),
        Some("iso-8859-1")
    );
...
fn policy_builds_decoder_and_defaults_to_utf8() {
    ...
    assert_eq!(
        decoder_for_charset(Some("iso-8859-1")).expect("latin"),
...
async fn enforces_redirect_hop_cap_exactly() {
    ...
    assert!(
        err.to_string()
            .contains("exceeded the maximum of 2 redirects")
    );
...
async fn reports_over_limit_before_cross_origin() {
    ...
    let err = provider(limits)
        .fetch(&req(&format!("{}/?n=0", server.url)), None)
        .await
        .expect_err("exceeded");
    assert_eq!(code(&err), "WEB_REDIRECT_BLOCKED");
    assert!(
        err.to_string()
            .contains("exceeded the maximum of 1 redirects")
    );
}
...
async fn plugin_rejects_invalid_limits_at_construction() {
    ...
    assert!(
        err.to_string()
            .contains("maxResponseBytes must be a positive finite number")
    );
    ...
    assert!(
        err.to_string()
            .contains("timeoutMs must be no greater than 2147483647")
    );
