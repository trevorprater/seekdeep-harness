...
Portable injected tests verify success, rollback, cleanup aggregation, idempotent waiting, and option-order invariants. Platform-gated native tests cross-check the `windows-sys` ABI layouts and exercise real DACL edits, restricted-token access checks, and the compiled runner's process effects. The pull-request `windows-native` job and master `serial-windows` standby run both ACL crates on a Windows kernel; non-Windows development can cross-clippy the same test targets for `x86_64-pc-windows-msvc`.
