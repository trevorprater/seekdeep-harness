//! Efficient local Boa job queue that sleeps while native bindings await.
...
use crate::engine::ensure_worker_not_terminal;

/// FIFO ECMAScript job executor whose async queue waits on wakeups instead of
/// repeatedly polling pending host futures. That distinction is required for
/// a fair worker busy-time meter.
#[derive(Debug, Default)]
pub(crate) struct WorkerJobExecutor {
...
impl JobExecutor for WorkerJobExecutor {
    ...
    async fn run_jobs_async(self: Rc<Self>, context: &RefCell<&mut Context>) -> JsResult<()> {
        ...
        loop {
            // Boa can represent a pending async function as a chain of
            // immediately re-enqueued native jobs. Yield once per pump turn
            // so wall/abort timers and completed native sleeps are not
            // starved by that self-waking chain.
            ...
            if let Err(error) = ensure_worker_not_terminal() {
                self.clear();
                return Err(error);
            }
        }
