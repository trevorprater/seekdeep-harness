//! Workflow-run dictionary namespace and exact source copy.

/// Dictionary namespace owned by the workflow-run plugin.
pub const WORKFLOW_RUN_NS: &str = "workflowRun";

/// Simplified Chinese dictionary in source key order.
pub const WORKFLOW_RUN_ZH: [(&str, &str); 18] = [
    ("run.title", "{name}"),
...
pub const WORKFLOW_RUN_EN: [(&str, &str); 18] = [
    ("run.title", "{name}"),
