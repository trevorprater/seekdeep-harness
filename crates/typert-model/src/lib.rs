//! The Typert `FaceModel`: the serializable type model the source generator analyzes and the
//! Rust generator, catalog, and Host artifact runtime share. Moved out of the generator so
//! runtime crates embed compiled Host artifacts without the analyzer's V8 dependency.

/// A model that cannot be walked (an unsupported node variant).
...
pub fn child_type_node_ids(node: &TypeNodeModel) -> Result<Vec<TypeNodeId>, ModelError> {
    let node = match node {
        TypeNodeModel::Defined(node) => node,
        TypeNodeModel::Unsupported(raw) => {
            return Err(ModelError(format!("unsupported model variant {raw}")));
        }
    };
    Ok(match &node.kind {
...
impl DocumentationModel {
    /// Documentation with no summary, description, tags, or `JSDoc`.
    #[must_use]
    pub fn empty() -> Self {
        Self::default()
    }
