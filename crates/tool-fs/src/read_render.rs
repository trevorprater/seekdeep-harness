...
use seekdeep_lossless_json::{JsonNumber, JsonRef, JsonString, JsonValue};
...
pub struct ReadWindow {
    /// 1-based first line to return, kept as the JavaScript number the caller sent.
    pub offset: JsonNumber,
    /// Maximum number of lines to return, kept as the JavaScript number the caller sent.
...
fn consume_line(acc: &mut WindowAccumulator, raw_line: &[u16], request: &ReadWindow) {
    ...
    if acc.truncated_by_bytes
        || JsonNumber::from(acc.total_lines) < request.offset
        || JsonNumber::from(acc.lines.len()) >= request.limit
    {
        return;
    }
    ...
    acc.lines.push(FileTextLine {
        number: JsonNumber::from(acc.total_lines),
        text,
    });
}
...
fn finish(
    ...
    let total_lines = JsonNumber::from(acc.total_lines);
    if !acc.truncated_by_bytes
        && request.offset > total_lines
        && !(acc.total_lines == 0 && request.offset == 1.0)
    {
...
pub fn format_read_output(display_path: &str, outcome: &FileReadOutcome) -> JsonString {
    let end_line = outcome.lines.last().map_or_else(
        || JsonNumber::new((outcome.offset - 1.0).get().max(0.0)),
        |line| line.number,
    );
...
///
/// Beyond shape, the source's semantic contract applies to replayed JSON:
/// `offset` is a 1-based integer, `totalLines` a non-negative integer, every
/// line number a 1-based integer, the numbers strictly increase from `offset`,
/// and none exceeds `totalLines`, all compared as JavaScript numbers.
#[must_use]
pub fn read_meta_from_meta(meta: &JsonValue) -> Option<FsReadMeta> {
    let path = meta.get("path")?.deserialize().ok()?;
    let offset = json_number(meta.get("offset")?)?;
    if !offset.is_integer() || offset < 1.0 {
        return None;
    }
    let total_lines = json_number(meta.get("totalLines")?)?;
    if !total_lines.is_integer() || total_lines < 0.0 {
        return None;
    }
    ...
    let lang = meta
        ...
        .transpose()
        .ok()?;
    let mut previous = offset - 1.0;
    let mut decoded = Vec::with_capacity(lines.len());
    for line in lines {
        let number = json_number(line.get("number")?)?;
...
fn json_number(value: JsonRef<'_>) -> Option<JsonNumber> {
    value.as_f64().map(JsonNumber::new)
}
...
mod tests {
    ...
    fn line_caps_count_utf16_units_and_preserve_a_split_surrogate() {
        let window = super::ReadWindow {
            offset: JsonNumber::new(1.0),
            limit: JsonNumber::new(10.0),
    ...
    fn window(offset: f64, limit: f64) -> ReadWindow {
        ReadWindow {
    ...
    fn window_numbers_and_bounds_lines() {
        ...
        .expect("window");
        assert_eq!(result.total_lines, 4.0);
        assert_eq!(
            result
                .lines
                .iter()
                .map(|line| (line.number.get(), line.text.as_str()))
                .collect::<Vec<_>>(),
            [(2.0, Some("two")), (3.0, Some("three"))]
        );
    }

    #[test]
    fn window_rejects_out_of_range_offset() {
        let err = build_window(["one\n".to_owned()], &window(5.0, 10.0), "/f").expect_err("range");
    ...
    fn format_read_output_renders_envelope_and_footer() {
        let outcome = FileReadOutcome {
            ...
            total_lines: JsonNumber::new(1.0),
    ...
    fn incremental_window_retains_only_the_capped_line_prefix() {
        ...
        assert_eq!(result.total_lines, 3.0);
        assert_eq!(result.lines.len(), 1);
        assert_eq!(
            result.lines[0].text,
