...
export function makeWorkspacePickerBench(){
  ...
  const React={Fragment,createElement(kind,supplied,...children){const flat=flatten(children);const props={...(supplied??{})};if(flat.length===1)props.children=flat[0];else if(flat.length>1)props.children=flat;return{kind,props,children:flat}},useState(initial){const at=si++;if(!(at in states))states[at]=typeof initial==='function'?initial():initial;return[states[at],value=>{states[at]=typeof value==='function'?value(states[at]):value}]},useEffect(run,deps){const at=ei++;const old=effects[at];const same=old&&old.deps.length===deps.length&&deps.every((v,i)=>Object.is(v,old.deps[i]));if(!same){old?.cleanup?.();const cleanup=run();effects[at]={deps:[...deps],cleanup:typeof cleanup==='function'?cleanup:undefined}}},useCallback(callback,deps){const at=mi++;const old=memos[at];const same=old&&old.deps.length===deps.length&&deps.every((v,i)=>Object.is(v,old.deps[i]));if(!same)memos[at]={deps:[...deps],value:callback};return memos[at].value},useMemo(factory){return factory()},useRef(initial){return{current:initial}}}
  ...
  cached={React,primitives:{Button:primitive('Button'),HoverCard:primitive('HoverCard'),IconArchiveOutline20:primitive('IconArchiveOutline20'),IconBranchOutline16:primitive('IconBranchOutline16'),IconCloseFill14:primitive('IconCloseFill14'),IconEditOutline16:primitive('IconEditOutline16'),IconEllipsisOutline16:primitive('IconEllipsisOutline16'),IconFolderClose16:primitive('IconFolderClose16'),IconFolderOpen16:primitive('IconFolderOpen16'),IconPersonalizationOutline16:primitive('IconPersonalizationOutline16'),IconPlusOutline16:primitive('IconPlusOutline16'),IconProjectAddOutline16:primitive('IconProjectAddOutline16'),IconSearchOutline16:primitive('IconSearchOutline16'),IconTrashOutline16:primitive('IconTrashOutline16'),IconTriangleRightFill14:primitive('IconTriangleRightFill14'),Menu:primitive('Menu'),Modal:primitive('Modal'),StateDot:primitive('StateDot'),Tooltip:primitive('Tooltip')},t:key=>copy[key]??key,render(component,props){si=0;ei=0;mi=0;return resolve(React.createElement(component,props))},reset(){for(const effect of effects.reverse())effect?.cleanup?.();states.length=0;effects.length=0;memos.length=0}}
  return cached
}
