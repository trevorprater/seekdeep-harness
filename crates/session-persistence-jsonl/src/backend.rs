...
mod tests {
    ...
    async fn final_drain_tolerates_retirement_while_initialization_is_pending() {
        ...
        let backend =
            JsonlSessionPersistence::build(sessions, JsonlConfig::new(temporary.path())).unwrap();
        let session = Session::create_with_clock(
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn materialization_keeps_new_logs_private_and_preserves_existing_directory_modes() {
        ...
        for compression in [JsonlCompression::None, JsonlCompression::Zstd] {
            for existing_root in [false, true] {
                ...
                let (backend, _, context) = backend_with_compression(&root, compression);
                let session = balanced_session("private-modes");
                ...
                let location = backend.locate(session.header()).unwrap();
                assert_eq!(mode(&location.path) & 0o077, 0);
                let directory = location.path.parent().unwrap();
                assert_eq!(mode(directory) & 0o077, 0);
                assert_eq!(mode(directory.parent().unwrap()) & 0o077, 0);
                if existing_root {
                ...
                assert_eq!(
                    backend.inspect(session.id(), None).await.unwrap().events,
                    session.events()
                );
                context.fiber().dispose().await.unwrap();
            }
