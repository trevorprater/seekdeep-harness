...
#[test]
fn release_build_generates_the_sdk_binding_entry_before_building_a_wheel() {
    let root = fixture();
    fs::remove_file(
        root.path()
    ...
    let path = root
        .path()
        .join("wheels/seekdeep_harness_sdk-1.2.3rc1-py3-none-any.whl");
    let mut archive = zip::ZipArchive::new(fs::File::open(path).unwrap()).unwrap();
    let mut entry = String::new();
    std::io::Read::read_to_string(
        &mut archive.by_name("deepseek_harness/__init__.py").unwrap(),
        &mut entry,
    )
    .unwrap();
    assert!(entry.contains("Generated by seekdeep-python-sdk"));
}
