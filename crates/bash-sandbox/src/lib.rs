//! Sandbox-consuming `Bash` executor over the local process mechanics.

use std::{path::Path, sync::Arc};

use async_trait::async_trait;
use parking_lot::Mutex;
use seekdeep_bash_local::{
    BashProcessObserver, BashProcessSettlement, BashSpawnFailure, LocalBashExecutor,
...
/// Cordis plugin name.
pub const NAME: &str = "bash-sandbox";
...
pub type Config = seekdeep_bash_local::Config;
...
pub fn is_runner_spawn_failure(
    ...
    runner_program.is_some()
        ...
        && error.downcast_ref::<BashSpawnFailure>().is_some()
}

/// Sandboxing `Bash` provider that occupies `ctx.shell`.
...
impl std::fmt::Debug for SandboxBashExecutor {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("SandboxBashExecutor")
...
#[derive(Debug)]
struct SandboxBashProcess {
    inner: ShellProcessHandle,
    sandbox: Arc<Mutex<Option<ShellSandboxInfo>>>,
}

#[async_trait]
impl ShellProcess for SandboxBashProcess {
...
#[async_trait]
impl ShellExecutor for SandboxBashExecutor {
    ...
    async fn run(&self, spec: ShellExecSpec) -> anyhow::Result<ShellRunResult> {
        let policy = spec.sandbox_policy.clone().ok_or_else(|| {
            anyhow::anyhow!("bash-sandbox: resolved spec is missing sandboxPolicy")
        })?;
    ...
    fn start(&self, spec: ShellExecSpec) -> anyhow::Result<ShellProcessHandle> {
        ...
        let observer: BashProcessObserver = Arc::new(move |settlement: &BashProcessSettlement| {
        ...
        Ok(Arc::new(SandboxBashProcess { inner, sandbox }))
    }
}
...
pub async fn apply(context: &Context, config: Config) -> anyhow::Result<Arc<SandboxBashExecutor>> {
    let sandbox = context
        .get(SANDBOX)
        .ok_or_else(|| anyhow::anyhow!("bash-sandbox requires sandbox"))?;
    let policy = context
        .get(SANDBOX_POLICY)
        .ok_or_else(|| anyhow::anyhow!("bash-sandbox requires sandboxPolicy"))?;
    let local = build_local(context, config).await?;
    let executor = Arc::new(SandboxBashExecutor {
        local,
        sandbox,
        policy,
    });
