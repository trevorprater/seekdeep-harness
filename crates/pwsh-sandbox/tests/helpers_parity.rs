...
use seekdeep_pwsh_local::PwshSpawnFailure;
use seekdeep_pwsh_sandbox::{classify_runner_failure, is_runner_spawn_failure, matches_signature};
...
fn spawn_attribution_requires_runner_provenance_usable_cwd_and_known_error_kind() {
    ...
    for detail in ["runner not found", "runner permission denied"] {
        let error = anyhow::Error::new(PwshSpawnFailure::new(detail));
    ...
    let missing = anyhow::Error::new(PwshSpawnFailure::new("missing"));
    assert!(!is_runner_spawn_failure(&missing, None, directory.path()));
    assert!(!is_runner_spawn_failure(
        &missing,
        runner,
        Path::new("/nonexistent-seekdeep-pwsh-sandbox-cwd")
    ));
}
