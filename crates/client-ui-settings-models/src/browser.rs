//! Shared bindings for the model-settings browser package.

use js_sys::{Array, Function, Object, Promise, Reflect};
use wasm_bindgen::{JsCast as _, JsValue};
use wasm_bindgen_futures::{JsFuture, future_to_promise};

pub(crate) const PACKAGE_ID: &str = "@seekdeep-ai/seekdeep-client-ui-settings-models";
...
pub(crate) fn class(class_name: &str) -> Result<Object, JsValue> {
...
pub(crate) fn inject_prefixed_style(name: &str, source: &str) -> Result<(), JsValue> {
    let prefixed = prefix_css_classes(source, "seekdeep-settings-models-");
    inject_style(name, &prefixed, &[])
}

pub(crate) fn css(name: &str) -> String {
    format!("seekdeep-settings-models-{name}")
}
