...
- `has_owner_activity(owner)` spans unpublished setup through final close, so lifecycle policy can fence the exact owner without a publication race.
...
- `TerminalSendResult.wait_reason` and `session_status` are independent. `session_exit` describes the top-level PTY process, not an arbitrary foreground command.
...
The seam contains no native PTY implementation, sandbox, tool schema, prompt, task, or terminal-rendering policy. Implementations own terminal mechanics; consumers own model presentation and optional background-job registration.
...
Nothing directly. This crate registers no prompt or tool; `seekdeep-tool-terminal` owns visible schemas and result text.
