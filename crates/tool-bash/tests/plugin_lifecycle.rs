//! Cordis dependency activation and reversible Bash-tool registration parity.
...
async fn missing_shell_keeps_plugin_pending_then_hmr_disposal_removes_tool_and_prompt() {
    ...
    let plugin = context
        .plugin(seekdeep_tool_bash::plugin(), Value::Null)
        .expect("mount tool bash");
    ...
    .expect("subprocess");
    seekdeep_bash_local::apply(&context, BashConfig::default())
        .await
    ...
    plugin.dispose().await.expect("dispose tool bash");
    assert!(tools.schemas(None).is_empty());
    assert!(
        prompt
            .assemble(AssembleContext::default())
            ...
            .sections
            .iter()
            .all(|section| section.name != "tool:bash")
    );
}
