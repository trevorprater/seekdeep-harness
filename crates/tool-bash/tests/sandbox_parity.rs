//! Bash-tool sandbox composition, policy, and approval parity.
...
fn sandbox_harness(with_approval: bool, jobs: bool) -> Harness {
    ...
    apply(&context, Config::default()).expect("tool bash");
    Harness {
        context,
        tools,
        shell,
...
fn split_sandbox_composition_fails_before_tool_registration() {
    ...
    assert!(tools.get("bash", None).is_none());
}
...
async fn approval_outcomes_map_distinctly_and_grants_stamp_foreground_and_background_policy() {
    ...
    assert_eq!(text(&background), "started background job bash-1");
    assert_eq!(
        harness.shell.modes(),
        vec![
