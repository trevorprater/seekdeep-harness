//! Model-facing Bash tool over foreground shell and detached Jobs seams.
...
/// Schema-decoded Bash tool arguments.
...
/// Registers the Bash tool and prompt guidance.
///
/// # Errors
///
...
pub fn apply(context: &Context, config: Config) -> anyhow::Result<()> {
    ...
    let output = DefineToolOutput::new(
        ...
        Arc::new({
            ...
            move |_args: &BashArgs, value: &BashValue| {
                let text = match value {
                    BashValue::Background { job_id } => {
                        format!("started background job {job_id}")
                    }
                    BashValue::Foreground {
                        exit_code,
                        signal,
                        timed_out,
                        aborted,
    ...
    let definition = define_tool(
        DefineToolOptions::new(
            "bash",
            bash_description(background_enabled, &escalation_modes),
            parameter_schema(background_enabled, &escalation_modes),
            output,
            Arc::new(move |args: BashArgs, execution| {
        ...
        .present_call(Arc::new(|args: &BashArgs| Some(present_bash_call(args))))
        .present_result(Arc::new(|args: &BashArgs, result: &ToolResult| {
            present_bash_result(args, result)
        })),
    )?;
...
async fn execute_bash(
    ...
    if args.run_in_background == Some(true) {
        ...
        return Ok(BashValue::Background { job_id });
    }
    ...
    Ok(canonical_bash_result(&result))
}

async fn approve_bash_escalation(
    context: &Context,
    mode: &str,
    justification: &str,
    execution: &ToolRunContext,
...
fn validate_args(args: &BashArgs) -> anyhow::Result<()> {
    anyhow::ensure!(
...
fn canonical_bash_result(result: &ShellRunResult) -> BashValue {
...
fn bash_description(background_enabled: bool, escalation_modes: &[SandboxMode]) -> String {
    ...
    let mut description = format!(
        "Execute a bash command (`bash -c`) and return its stdout/stderr. Each call runs in a fresh shell: no state (cwd, variables, functions) persists between calls — pass `workdir` instead of using `cd`. Non-zero exits are reported as `[exit code: N]`. Current harness environment facts are exposed through managed `${SEEKDEEP_ENV_PREFIX}*` variables; inspect them when needed. Commands may run under a file sandbox; a blocked file operation is reported as `[sandbox: file access denied under <mode> mode]` — a policy denial, not a bug in the command; do not retry another way. Long output is truncated to its tail; the full output is saved to a file whose path is reported when available. {background}"
    );
    if !escalation_modes.is_empty() {
...
fn present_bash_result(args: &BashArgs, result: &ToolResult) -> Option<ToolResultView> {
...
mod tests {
    ...
    fn args(background: bool) -> BashArgs {
        BashArgs {
    ...
    fn presentation_distinguishes_terminal_runs_background_acks_and_errors() {
        ...
        assert_eq!(
            serde_json::to_value(present_bash_call(&foreground)).unwrap(),
            json!({
                "card":"terminal",
                "title":"printf hi",
        ...
        assert_eq!(
            serde_json::to_value(present_bash_call(&foreground)).unwrap()["cwd"],
            json!("sub")
        );

        let background = args(true);
        assert_eq!(
            serde_json::to_value(present_bash_call(&background)).unwrap(),
            json!({
                "card":"generic",
                "title":"printf hi",
                "kind":"execute",
        ...
        assert!(matches!(
            present_bash_result(&foreground, &result("tool call aborted", true)),
    ...
    fn terminal_presenter_consumes_only_real_final_exit_markers() {
        ...
        for (raw, expected) in [
            ...
            assert_eq!(
                serde_json::to_value(
                    present_bash_result(&foreground, &result(raw, false)).unwrap()
                )
                .unwrap(),
                expected
            );
        ...
        assert!(present_bash_result(&foreground, &reasoning).is_none());
        ...
        assert!(present_bash_result(&foreground, &multiple).is_none());
    }
