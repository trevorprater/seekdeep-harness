...
fn compiled_client_tree_has_no_typescript_css_module_inputs() {
    ...
    let rust = std::fs::read_to_string(root.join("crates/client-ui-primitives/src/browser_blocks.rs"))
        .unwrap();
    assert!(rust.contains("include_str!(\"../../../packages/client/ui-primitives/src/DiffBlock.module.css\")"));
}
