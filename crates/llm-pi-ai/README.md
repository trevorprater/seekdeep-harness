...
Native Rust multi-provider adapter for the SeekDeep Harness LLM seam, preserving the behavior and pinned catalog contract of [`@earendil-works/pi-ai` 0.82.1](https://www.npmjs.com/package/@earendil-works/pi-ai). One plugin instance owns a dict of provider profiles keyed by route; every request selects a profile with `GenerateOptions.provider` and resolves `GenerateOptions.model` against that route's configured catalog. A route naming an installed pi-ai provider inherits its endpoint, wire protocol, and model catalog as defaults and overrides them field by field; a route pi-ai does not ship is declared outright, so an OpenAI-compatible gateway, a self-hosted server, or a provider newer than the installed catalog is configuration rather than a code change.

The crate exposes the Cordis plugin contract and typed Rust modules for `PiAiAdapter`, catalog resolution, provider construction, replay conversion, stream conversion, and native protocol execution. `provider::supported_protocols()` is the explicit-profile protocol gate.

## Config

Configure credentials, the model catalog, and deployment-specific transport settings per provider, keyed by the provider route itself. `apiKeyEnv` is a credential *reference* resolved per request, so no secret enters this file. Omitting it delegates authentication to the installed provider: ordinary routes use pi-ai's provider-native ambient discovery, while `openai-codex` uses the official Codex CLI's file-backed ChatGPT login. A configured reference that resolves to nothing fails the request with `MISSING_CREDENTIAL` instead, because falling through would authenticate with whatever unrelated key the environment happens to hold. One credential serves every model on its route.
...
- id: llm
  ...
  config:
    providers:
      # ChatGPT subscription route: models and protocol come from pi-ai, while
      # authentication comes from `codex login`; do not set apiKeyEnv.
      openai-codex: {}
...
Add `openai-codex` from Settings → Models without entering an API key, or store the empty profile shown above. The adapter reads `auth.json` on every request, so a later `codex login` takes effect without restarting the Harness. `$CODEX_HOME` selects the directory when it was present in the Harness launch environment; otherwise the path is `~/.codex/auth.json`.

The Codex CLI owns login and logout. The adapter accepts only a current `auth_mode: "chatgpt"` document whose file is owner-only, exposes that credential only to the `openai-codex` provider, and returns `MISSING_CREDENTIAL` with the setup command when no file-backed ChatGPT session exists. A malformed, overexposed, or account-inconsistent document fails as `INVALID_CREDENTIAL` without including token material in the diagnostic. pi-ai refreshes an expired access token and the adapter atomically preserves the rotated access and refresh tokens back into the shared Codex document while retaining Codex-owned and unknown fields; a concurrent Codex rotation observed before commit wins, and a refresh cannot change the selected ChatGPT account. `codex logout`, not a Harness settings edit, removes the shared session.
...
Supported profile fields are `apiKeyEnv`, `displayName`, `api`, `baseURL`, `models`, `modelOverrides`, `compat`, `defaultContextWindow`, `defaultMaxTokens`, `defaultInput`, `headers`, `reasoning`, `thinkingBudgets`, `cacheRetention`, `transport`, `timeoutMs`, `websocketConnectTimeoutMs`, `streamIdleTimeoutMs`, and `retryPolicy`. Each profile's optional retry policy is captured with that provider route; omission uses bounded normal defaults. The stream-idle interval is a positive finite runtime timer delay, defaults to five minutes, and covers only an outstanding provider read, not consumer think time. Harness app attribution wins a conflicting configured header name.
...
Each resolution produces one **immutable** snapshot containing the ordered profiles and their fully materialized model catalogs, and every operation captures that whole snapshot before its first `await`. A configuration change builds a new snapshot rather than mutating one already in use, so a request that started under one configuration cannot finish under another or fail on a provider that disappeared mid-flight. This is what makes the seam's per-step call freeze (`llm.prepare_call()`) hold end to end — switching models mid-reply takes effect on the next step, never inside the one in flight. `NativePiExecutor` dispatches the descriptor's closed protocol enum to the Rust Chat Completions, Responses, Anthropic, Google, Vertex, Bedrock, Mistral, Azure, or Codex engine. Catalog routes retain the pinned provider and model facts; hand-declared routes are built through `provider::supported_protocols()`.

API-key credentials never enter that snapshot. The harness resolves a route's key through its own seam before native protocol dispatch and freezes it in the per-call options, preserving fail-loud reference semantics. The one file-backed OAuth store is restricted to `openai-codex`; every other provider sees no stored credential and retains its provider-native ambient path. A non-Codex route naming no credential resolves as configured-but-keyless and leaves the requirement to its provider or protocol, which is where it actually lives.
...
Every request carries the shared attribution header from seekdeep-llm's `attributionHeaders()`, merged through pi-ai's `headers` stream option. Provider-specific app-attribution headers are not synthesized. See [seekdeep-llm § App attribution](../llm/README.md#app-attribution-attributionts).
...
- **Codex OAuth requires the official file-backed credential store** — the adapter does not read Codex credentials kept only in the operating-system keyring. Set `cli_auth_credentials_store = "file"` and run `codex login`; `auto` works only when Codex actually wrote `auth.json`. The file format belongs to Codex rather than this package, so an incompatible future format fails closed with `INVALID_CREDENTIAL` until the adapter is updated. `$CODEX_HOME` is captured at Harness launch even though the selected file is reread for every request. Other OAuth-only pi-ai providers remain withheld unless this plugin gains an explicit provider-scoped credential source for them.
...
- **Retry policy is provider-owned, not an SDK retry** — each provider profile may configure nested `retryPolicy`, which `seekdeep-llm-retry` executes at the agent failed-step extension point; pi-ai SDK retries stay disabled so durable agent steps and `llm/retry` events own every visible attempt, and direct `ctx.llm.stream()` calls remain single-attempt.
