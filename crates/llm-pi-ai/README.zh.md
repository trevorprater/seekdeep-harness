...
SeekDeep Harness LLM（大语言模型）seam 的原生 Rust 通用多提供方适配器，完整保留 [`@earendil-works/pi-ai` 0.82.1](https://www.npmjs.com/package/@earendil-works/pi-ai) 的行为与固定 catalog 约定。一个插件实例拥有一份以路由为键的提供方 profile 字典；每个请求使用 `GenerateOptions.provider` 选择 profile，并针对该路由已配置的 catalog 解析 `GenerateOptions.model`。点名了已安装 pi-ai 提供方的路由会继承其端点、协议格式（wire format）与模型 catalog 作为默认值，并逐字段覆盖；pi-ai 未提供的路由则整体声明出来，因此接入 OpenAI 兼容网关、自建服务，或比已安装 catalog 更新的提供方，都属于配置而非改代码。

crate 导出 Cordis 插件约定，以及 `PiAiAdapter`、catalog 解析、提供方构造、回放转换、流转换与原生协议执行所需的强类型 Rust 模块。`provider::supported_protocols()` 是显式 profile 的协议门。

## 配置

按提供方配置凭据、模型 catalog 与部署特定传输设置，并以提供方路由本身为键。`apiKeyEnv` 是按请求解析的凭据*引用*，因此机密不进入该文件。省略它会把认证交给已安装的提供方：普通路由使用 pi-ai 的提供方原生环境发现，`openai-codex` 则使用 Codex 官方 CLI 存于文件的 ChatGPT 登录。已配置却解析不出任何值的引用则相反，会让请求以 `MISSING_CREDENTIAL` 失败，因为放行下去就会用环境里恰好持有的某个无关密钥完成认证。一条凭据服务该路由下的全部模型。
...
字典形状使重复路由无法表示，发布前的数组形状（每个 profile 携带 `provider` 字段）会加载失败并给出迁移指引。`providers` 也可以为空或整体省略：适配器将以**休眠**姿态挂载——零路由、模型选择器不多一条——一旦 `llm-pi-ai:` settings 分节提供了 profile 就即时注册路由，分节清空时随之撤销。无论是否休眠，插件都会在可配置提供方目录（`ctx.llm.listConfigurableProviders()`，settings 路径 `providers.<provider>`）中声明它能认证的每个已安装 catalog 提供方，并与当前 profile 声明的每条路由取并集，因此配置界面既能在任何路由存在之前就提供可服务的 catalog，也能寻址一条手工声明的路由。每个条目都带上 `declared`：pi-ai 在这个键下是否什么都没有。它跟随已安装 catalog 而非设置文档，因为收窄一个内置提供方的模型同样会存下 profile，而那条路由仍然是 pi-ai 认识的——只有适配器分得清两者，所以由目录直接给出答案，而不是留给界面去猜。哪些适配器存在归组合面；哪些提供方在运行可以完全交给用户的设置文档。向 `ctx.llm` 注册具有原子性：如果与另一适配器已拥有的任何提供方路由冲突，插件会加载失败，不注册剩余路由。模型 id 不是生命周期配置；路由未配置的模型会在发起任何提供方请求前以 `LlmError('UNKNOWN_MODEL')` 失败。

## 通过 Codex 使用 ChatGPT OAuth

`openai-codex` 与 Codex 官方 CLI 共用同一个 ChatGPT 会话。先配置 Codex 把凭据保存在仅限所有者访问的文件中，再由 Codex 自身完成登录：
...
在 Settings → Models 中添加 `openai-codex`，不要输入 API 密钥；也可以存入上面展示的空 profile。适配器会在每次请求时重新读取 `auth.json`，因此稍后执行的 `codex login` 无需重启 Harness 即可生效。若 Harness 启动环境中存在 `$CODEX_HOME`，它会选择该目录；否则路径为 `~/.codex/auth.json`。

登录与登出归 Codex CLI 所有。适配器只接受当前的 `auth_mode: "chatgpt"` 文档，且文件必须仅限所有者访问；它只向 `openai-codex` 提供该凭据，并在不存在存于文件的 ChatGPT 会话时，以携带设置命令的 `MISSING_CREDENTIAL` 失败。格式错误、权限过宽或账户不一致的文档会以 `INVALID_CREDENTIAL` 失败，诊断中不含任何 token 内容。pi-ai 会刷新过期的 access token；适配器以原子替换把轮换后的 access token 与 refresh token 保存在共享 Codex 文档中，同时保留 Codex 持有的字段与未知字段；提交前观察到 Codex 并发轮换时，以 Codex 的结果为准，且刷新不能更改所选 ChatGPT 账户。删除共享会话要运行 `codex logout`，不是编辑 Harness settings。
...
`provider::supported_protocols()` 刻意窄于 pi-ai 的完整流式 API 集合：它只保留 profile 能用密钥、端点与标头**完整描述**的那些协议。Bedrock 要用 AWS 凭据与 region 做 SigV4 签名，Vertex 需要 project、location 与应用默认凭据，Azure 需要提供方环境外加 api-version，Codex 走 OAuth——提供它们只会交回一个无法完成认证的路由。catalog 路由仍可经原生 Rust executor 抵达这些协议；被拒绝的只有显式覆盖。
...
API 密钥凭据在每次流调用时通过 `apiKeyEnv` 与可选的 `ctx.credentials` seam 解析；未挂载该 seam 时，适配器只读取该引用指向的环境变量。只有完全没有点名任何凭据的 profile——仅限这一种情况——才交给提供方原生环境发现；`openai-codex` 例外，它会读取上文所述、仅对该提供方可见的 Codex 文件存储。每个解析出的密钥都会在使用前去除首尾空白并校验格式，因此 HTTP 标头无法承载的值会被拒绝，而不是以语义不明的传输错误形式浮现；这种拒绝会抛出 `LlmError('INVALID_CREDENTIAL')`，点名失败的路由与凭据引用，但绝不透露密钥的任何部分。路由集合与每条路由捕获的重试策略是注册级事实：两者任一变化时，插件都会原子地替换自己的注册（同一适配器实例，候选集合先经校验），因此某条路由若已被另一适配器占有，先前的路由会继续服务，而改回可用配置时注册会重新生效。提供方键的顺序绝不算作变化。本适配器无法服务的分节会在写入处被拒——注册的 `validate` 会解析整份 profile 集合，因此 `ctx.settings.mutate` 以 resolver 自身的错误拒绝（该协议将其报为 `settings-rejected`），什么都不会存储。已存储分节若因其他途径变得不可服务——比如外部编辑了 `settings.yaml`——则由 settings seam 保留该 namespace 最后可用的值并告警。entry 配置本身仍会使插件加载失败；而 llm 注册表拒绝的路由（已被另一适配器族占有的那种）会被记录下来，先前注册的路由继续服务。
...
受支持的 profile 字段是 `apiKeyEnv`、`displayName`、`api`、`baseURL`、`models`、`modelOverrides`、`compat`、`defaultContextWindow`、`defaultMaxTokens`、`defaultInput`、`headers`、`reasoning`、`thinkingBudgets`、`cacheRetention`、`transport`、`timeoutMs`、`websocketConnectTimeoutMs`、`streamIdleTimeoutMs` 和 `retryPolicy`。每个 profile 的可选重试策略都会与该提供方路由一同捕获；省略时使用有界的常规默认值。流空闲间隔必须是正的有限运行时定时器延迟，默认为五分钟，且只覆盖未完成提供方读取，不包括消费方思考时间。若已配置标头中有同名项，则以 Harness 应用归因为准。
...
每次解析都会产出一份**不可变**快照，其中包含有序 profiles 与完全物化的模型 catalog；每个操作都在第一个 `await` 之前整体捕获它。配置变化会构造新快照，而不是改动在途请求所持的快照，因此在旧配置下开始的请求不会在新配置下结束，也不会撞上回复途中被移除的 provider。这正是 seam 的每步调用冻结（`llm.prepare_call()`）能贯通到底的原因——回复途中切换模型会在下一步生效，绝不会影响当前在途步骤。`NativePiExecutor` 按 descriptor 的封闭协议枚举分派到 Rust 的 Chat Completions、Responses、Anthropic、Google、Vertex、Bedrock、Mistral、Azure 或 Codex engine。catalog 路由保留固定的 provider 与模型事实；手工声明路由则经 `provider::supported_protocols()` 构造。

API 密钥凭据绝不进入该快照。harness 在原生协议分派前经自身 seam 解析路由密钥，并将它冻结到单次调用选项中，从而保留明确失败的引用语义。唯一的文件 OAuth 存储只对 `openai-codex` 开放；其他提供方看不到任何已存凭据，并保留各自的原生 ambient 路径。非 Codex 路由没有点名凭据时，会解析为「已配置但无密钥」，把该要求留给提供方或协议——那才是它真正所在的位置。
...
每个请求都携带 seekdeep-llm `attributionHeaders()` 的共享归因标头，并通过 pi-ai `headers` 流选项合并。不会合成提供方特定应用归因标头。详见 [seekdeep-llm § 应用归因](../llm/README.md#app-attribution-attributionts)。

## 依赖体量

该 Rust crate 将提供方协议与认证依赖限制在适配器自身范围内。请求构造、流处理、凭据刷新、回放与生命周期所有权均由原生 Rust 实现，不使用浏览器 JavaScript。
...
- **Codex OAuth 要求使用官方的文件凭据存储**：适配器不会读取仅保存在操作系统钥匙串中的 Codex 凭据。请设置 `cli_auth_credentials_store = "file"` 并运行 `codex login`；只有 Codex 确实写出了 `auth.json` 时，`auto` 才可用。文件格式归 Codex 而非本包所有，因此未来若出现不兼容格式，会以 `INVALID_CREDENTIAL` 关闭失败，直至适配器更新。虽然每次请求都会重新读取所选文件，`$CODEX_HOME` 仍在 Harness 启动时捕获。其他仅以 OAuth 认证的 pi-ai 提供方仍不予提供，除非本插件为其获得显式、仅限该提供方的凭据来源。
...
- **重试策略由提供方持有，而不是 SDK 重试**：每个提供方 profile 都可以配置嵌套的 `retryPolicy`，由 `seekdeep-llm-retry` 在 agent 的失败步骤扩展点上执行；pi-ai SDK 重试仍保持禁用，因此持久化的 agent 步骤与 `llm/retry` 事件记录每次可见尝试，直接 `ctx.llm.stream()` 调用仍只尝试一次。
