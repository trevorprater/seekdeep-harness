    ...
    context.events().on_waterfall(
        ...
        move |_, args, next| {
            ...
            Box::pin(async move {
                ...
                let mut payload =
                    JsonValue::from(bridge.base(Some(&event.agent), "UserPromptSubmit"));
...
fn pre_tool_payload(bridge: &Bridge, execution: &ToolExecution) -> JsonValue {
    let mut payload = JsonValue::from(bridge.base(execution.agent.as_ref(), "PreToolUse"));
    payload
        .insert("tool_name", Value::String(execution.name.clone()).into())
        .expect("hook payload is an object");
    payload
        .insert("tool_input", execution.arguments.clone())
        .expect("hook payload is an object");
    payload
        .insert(
            "tool_use_id",
...
fn post_tool_payload(
    ...
    let mut payload = JsonValue::from(bridge.base(execution.agent.as_ref(), "PostToolUse"));
    payload
        .insert("tool_name", Value::String(execution.name.clone()).into())
        .expect("hook payload is an object");
    payload
