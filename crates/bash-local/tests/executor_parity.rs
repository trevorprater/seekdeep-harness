...
//! Real local-process parity for the Bash execution provider.

use std::{collections::BTreeMap, path::Path, sync::Arc, time::Duration};

use seekdeep_bash_local::{Config, LocalBashExecutor, apply, assert_serviceable_bash_config};
...
async fn setup(mut config: Config) -> Harness {
    ...
    .expect("subprocess");
    let bash = apply(&root, config).await.expect("bash");
    Harness {
        _root: root,
        _spill: spill,
        runtime,
...
async fn foreground_resolution_output_limits_and_environment_match_the_source() {
    ...
    let result = harness
        .bash
        .run(harness.bash.resolve(request("echo hi")).expect("resolve"))
        .await
        .expect("run");
    ...
    let pwd = harness
        .bash
        .run(harness.bash.resolve(request("pwd")).expect("resolve"))
        .await
        .expect("pwd");
    assert_eq!(
    ...
    let root = harness
        .bash
        .run(harness.bash.resolve(root_request).expect("resolve root"))
        .await
        .expect("root pwd");
    ...
    assert_number(
        harness.bash.resolve(capped).expect("capped").timeout_ms,
        6_000.0,
    );
    assert_number(
        harness
    ...
    let output = harness
        .bash
        .run(harness.bash.resolve(wide_stdout).expect("resolve output"))
        .await
        .expect("output run");
    ...
    let spec = harness.bash.resolve(seam).expect("seam resolve");
    assert_eq!(spec.stdin.as_deref(), Some("piped\n"));
    assert_eq!(spec.env.as_ref().unwrap()["SEAM_VAR"], "env-ok");
    let seam_result = harness.bash.run(spec).await.expect("seam run");
    assert_eq!(seam_result.stdout.text, "piped\n[env-ok][seekdeep-ok]\n");

    let plain = harness.bash.resolve(request("true")).expect("plain");
...
fn configuration_validation_names_every_unserviceable_field() {
    for (config, field) in [
        ...
        let error = assert_serviceable_bash_config(&config).unwrap_err();
...
async fn request_validation_timeout_abort_self_signal_and_spawn_failure_are_distinct() {
    ...
    let timed = harness
        .bash
        .run(harness.bash.resolve(timed).expect("timed resolve"))
        .await
        .expect("timed run");
    ...
    let spec = harness.bash.resolve(aborted).expect("abort resolve");
    let bash = harness.bash.clone();
    let pending = tokio::spawn(async move { bash.run(spec).await });
    ...
    missing.workdir = Some("/nonexistent-seekdeep-bash-local".into());
    let error = harness
        .bash
        .run(harness.bash.resolve(missing).expect("missing resolve"))
        .await
        .unwrap_err();
    assert!(
...
async fn background_streams_are_consuming_and_report_both_spill_paths() {
    ...
    for (command, expected) in [
        ...
        let process = harness
            .bash
            .start(harness.bash.resolve(request(command)).expect("resolve"))
            .expect("start");
        process.done().await;
...
async fn background_kill_abort_self_signal_and_spawn_failure_settle_without_orphans() {
    ...
    let aborted = harness
        .bash
        .start(harness.bash.resolve(abort_request).expect("abort resolve"))
        .expect("abort start");
    signal.abort();
    aborted.done().await;
    ...
    let failed = harness
        .bash
        .start(harness.bash.resolve(missing).expect("missing resolve"))
...
async fn subprocess_owner_outlives_executor_and_disposal_joins_live_process_trees() {
    ...
    .unwrap();
    let executor_fiber = Fiber::active_child("bash-test");
    ...
    .unwrap();
    let process = bash
        .start(bash.resolve(request("echo $$; sleep 60")).unwrap())
        .unwrap();
