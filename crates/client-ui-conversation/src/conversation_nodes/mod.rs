...
/// Builds every built-in Chat event definition in source registration order.
#[must_use]
pub fn conversation_event_definitions() -> Vec<AssemblerNodeDefinition> {
    let mut definitions = Vec::new();
    definitions.extend(conversation_inbox_definitions());
    definitions.push(conversation_message_definition());
    definitions.push(conversation_assistant_definition());
    definitions.push(conversation_tool_definition());
    ...
    definitions.push(conversation_turn_tail_definition());
    definitions
}
