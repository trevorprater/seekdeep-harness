//! Model-facing Pwsh tool parity over the real local process and Jobs providers.
...
use seekdeep_pwsh_local::{Config as PwshConfig, ENCODING_PREAMBLE, LocalPwshExecutor};
...
async fn harness(config: Config, jobs: bool, controller: bool) -> Harness {
    ...
    let pwsh = seekdeep_pwsh_local::apply(
        &context,
        PwshConfig {
            timeout_ms: 10_000.0,
            grace_ms: 200.0,
    ...
    apply(&context, config).expect("tool pwsh");
    Harness {
        context,
        _spill: spill,
        prompt,
...
async fn call_with(
    ...
    let input = ToolExecutionInput::new(CallId::new("tool-pwsh-call"), name, arguments, signal);
...
async fn foreground_commands_render_output_exit_timeout_abort_workdir_and_failures() {
    ...
    let missing = call(
        ...
        json!({
            ...
            "workdir": "/nonexistent-seekdeep-tool-pwsh"
        }),
    )
    .await;
...
async fn schemas_value_validation_prompt_and_background_opt_out_stay_in_lockstep() {
    ...
    assert!(schemas[0].description.contains("$env:SEEKDEEP_*"));
    assert!(schemas[0].description.contains("`pwsh -Command`"));
    assert!(schemas[0].description.contains("`C:\\...`"));
    ...
    assert!(
        assembly.sections.iter().any(|section| {
            section.name == "tool:pwsh" && section.text.contains("[exit code: N]")
        })
    );
    ...
    for arguments in [
        ...
        let result = call(&default_harness, "pwsh", arguments).await;
        assert!(result.is_error(), "{}", text(&result));
    }
...
async fn background_jobs_ack_stream_complete_kill_and_preflight_without_orphans() {
    ...
    assert_eq!(
        ...
        Some(json!({"kind":"background", "jobId":"pwsh-1"}))
    );
    assert_eq!(text(&started), "started background job pwsh-1");
    ...
    .await;
    assert_eq!(text(&started), "started background job pwsh-2");
    let killed = call(&full_harness, "job_kill", json!({"job_id": "pwsh-2"})).await;
    assert_eq!(text(&killed), "requested cancellation of job pwsh-2");
    let killed_output = call_until(
        &full_harness,
        "job_output",
        json!({"job_id": "pwsh-2", "wait": true}),
...
async fn background_preabort_never_spawns_and_owned_jobs_are_session_isolated() {
    ...
    .await;
    assert_eq!(text(&owned_start), "started background job pwsh-1");
    let anonymous = call(&full_harness, "job_output", json!({"job_id":"pwsh-1"})).await;
