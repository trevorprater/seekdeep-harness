//! Dynamic `shell` settings-section parity for the local `PowerShell` provider.
...
use seekdeep_pwsh_local::{Config, LocalPwshExecutor, apply};
...
async fn boot() -> Bench {
    ...
    let settings = SettingsService::install(&settings_context, Arc::new(MemoryStorage::default()))
        .await
        .expect("settings");
    let executor_fiber = Fiber::active_child("pwsh-provider");
...
async fn user_layer_validates_updates_every_read_and_falls_back_on_provider_detach() {
    ...
    assert_number(bench.pwsh.config().timeout_ms, 60_000.0);
    ...
    assert_number(bench.pwsh.config().timeout_ms, 5_000.0);
    ...
    bench
        ...
        .update(
            ...
            json!({
                ...
                "cwd": "/tmp",
                "pwshPath": "/opt/seekdeep/pwsh"
            }),
            None,
        )
        .await
        .expect("update execution settings");
    assert_eq!(bench.pwsh.pwsh_path(), "/opt/seekdeep/pwsh");
    bench
        .settings
        .update(&namespace, json!({"timeoutMs": 4_000}), None)
        .await
    ...
    assert_number(bench.pwsh.config().timeout_ms, 60_000.0);
    assert_number(bench.pwsh.config().max_output_bytes, 64_000.0);
    assert_eq!(bench.pwsh.config().pwsh_path, None);
}
...
async fn no_provider_keeps_composition_entry_and_executor_unload_releases_namespace() {
    let root = Context::new();
    LocalSubprocessRuntime::install(&root).expect("subprocess");
    let no_settings_fiber = Fiber::active_child("pwsh-without-settings");
    ...
    .await
    .expect("pwsh");
    assert_number(pwsh.config().timeout_ms, 1_234.0);
    no_settings_fiber.dispose().await.expect("dispose pwsh");
