...
| Layer | Package | Role |
|---|---|---|
| tool / executor | `@seekdeep-ai/seekdeep-tool-fs` | model-facing `read`/`write`/`edit` schemas + read windowing + text rendering; reads/writes/edits via `ctx.fs`, dispatches the `fs/*` events |
| policy | `@seekdeep-ai/seekdeep-fs-observation-policy` | observed-state + read-before-edit + version-guarded write/edit, contributed through the `fs/*` event gate (no service) |
| provider contract | `@seekdeep-ai/seekdeep-fs` (this) | `ctx.fs`: execution-world paths, text IO, and atomic mutation primitives (optional version guard); owns the `fs/*` event vocabulary |
| provider | `@seekdeep-ai/seekdeep-fs-local` | the host-filesystem implementation |
...
Indirectly, through `seekdeep-tool-fs`, which renders provider text and errors as bounded, retained filesystem tool results.
