# @seekdeep-ai/seekdeep-fs-observation-policy

English | [中文](README.zh.md)

The **fs-observation-policy plugin**: it records observed presence or absence and adds read-before-edit plus guarded write/edit on top of the `ctx.fs` provider contract ([`@seekdeep-ai/seekdeep-fs`](../fs)) — through the `fs/*` event gate, **NOT** through a method service. This plugin registers **no** `ctx.fsPolicy` service and has no public `read`/`write`/`edit`/`resolve` methods. It is the policy third of the filesystem stack: not a swappable seam, but the policy that does not belong on the `FileSystem` provider base class.

```ts
import type { Context } from '@seekdeep-ai/cordis'
import * as FsPolicy from '@seekdeep-ai/seekdeep-fs-observation-policy'

declare const ctx: Context

// No service to inject — this plugin only registers the three fs/* listeners.
// Load it alongside a ctx.fs provider (e.g. @seekdeep-ai/seekdeep-fs-local) and the
// @seekdeep-ai/seekdeep-tool-fs tools; the tools dispatch the fs/* events this plugin
...
| Layer | Package | Role |
|---|---|---|
| tool / executor | `@seekdeep-ai/seekdeep-tool-fs` | model-facing schemas + read windowing + text rendering; reads/writes/edits via `ctx.fs`, dispatches the `fs/*` events |
| policy | `@seekdeep-ai/seekdeep-fs-observation-policy` (this) | observed-state + read-before-edit + version-guarded write/edit, contributed through the `fs/*` event gate (no service) |
| provider contract | `@seekdeep-ai/seekdeep-fs` | `ctx.fs`: text IO + atomic mutation primitives (optional version guard); owns the `fs/*` event vocabulary |
| provider | `@seekdeep-ai/seekdeep-fs-local` | local implementation of `ctx.fs` |

## How the gate participates

Three `fs/*` events (declared by `@seekdeep-ai/seekdeep-fs`, dispatched by `@seekdeep-ai/seekdeep-tool-fs`):
...
Because the plugin influences the world only through events, removing it does not break `@seekdeep-ai/seekdeep-tool-fs` at a service-injection boundary: the tool falls through to the bare `ctx.fs` provider (unconditional write/edit, no observed-state). Loading it back layers the policy on. That graceful add/remove is the whole point of the event gate over a mandatory method service.
