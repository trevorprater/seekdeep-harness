...
The model-facing `pwsh` tool registered over the `ctx.shell` executor seam. Intended for Windows compositions where a PowerShell executor (e.g. `@seekdeep-ai/seekdeep-pwsh-local`) backs `ctx.shell`; the tool contract is PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables. Behavior mirrors `seekdeep-tool-bash` call-for-call — foreground and `run_in_background` execution through the generic job runtime, the managed `SEEKDEEP_*` environment through the shared `shell-env` registry, the sandbox denial rendering with the same-turn `sandbox_permissions` escalation surface, and the bash marker/truncation rendering story (a clean exit produces no marker).
...
Every foreground and background model pwsh call receives a freshly collected trusted `SEEKDEEP_*` environment through the shared [`seekdeep-shell-env`](../shell-env/) registry: `SEEKDEEP_HOME` (the absolute Harness home), `SEEKDEEP_SHELL=1`, the agent's `SEEKDEEP_SESSION_ID`, and `SEEKDEEP_SESSION_JSONL` when the active persistence backend locates one. Plugins contributing `SEEKDEEP_*` facts to `ctx.shellEnv` apply to pwsh calls exactly as they do to bash calls. The snapshot passes through the dedicated `ShellExecRequest.seekdeepEnv` channel; `process.env` is never modified. The description teaches the generic `$env:SEEKDEEP_*` convention rather than naming persistence-specific variables.
