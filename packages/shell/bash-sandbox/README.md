...
Sandbox-consuming Service Provider for the [`@seekdeep-ai/seekdeep-shell`](../shell/) executor seam. Load it **instead of** `@seekdeep-ai/seekdeep-bash-local`, together with a [`ctx.sandbox`](../../sandbox/sandbox/) provider (e.g. [`@seekdeep-ai/seekdeep-sandbox-local`](../../sandbox/sandbox-local/)) and a [`ctx.sandboxPolicy`](../../sandbox/sandbox-policy/) (which owns the default mode + workspace root, shared with the sandboxed filesystem) — no alternate tool plugin is needed; `seekdeep-tool-bash` detects the executor's `sandboxMode` capability and adds the escalation fields.
...
- Process mechanics (spawn, process-group kills, output collection/spill, background handles, credential scrub) are inherited from [`seekdeep-bash-local`](../bash-local/); runner selection lives in [`seekdeep-sandbox-local`](../../sandbox/sandbox-local/).

Deny-only at the seam: a denial is a reported fact, and this executor never negotiates permissions itself — the approval question lives in the tool layer (`seekdeep-tool-bash`), which drives the override this package honors.

```yaml
- id: sandbox
  name: '@seekdeep-ai/seekdeep-sandbox-local'
- id: sandbox-policy
  name: '@seekdeep-ai/seekdeep-sandbox-policy'
  config:
    mode: read-only
    workspaceRoot: !!js process.cwd() # fallback for calls without a session cwd
- id: bash
  name: '@seekdeep-ai/seekdeep-bash-sandbox'
```
...
Small fixed schema increment on requests where `bash` is visible, plus the current-policy clause owned by `seekdeep-sandbox-policy`.
