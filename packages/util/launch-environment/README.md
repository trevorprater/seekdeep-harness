...
| `$SEEKDEEP_HOME/.env` | `user-env` | The user's own machine-level defaults |
...
import { launchEnvironmentOf } from '@seekdeep-ai/seekdeep-launch-environment'
...
- **The snapshot is not a subprocess boundary** — every layer is also materialized into `process.env`, so ordinary project variables reach child processes under [`seekdeep-subprocess`](../../subprocess/subprocess/README.md)'s scrub. The product launcher's [`.env` contract](../../boot/app-boot/README.md#profiles) rejects bootstrap variables before materialization.
