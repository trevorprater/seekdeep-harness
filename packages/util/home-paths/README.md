...
Shared filesystem path helpers for SeekDeep Harness user data.

## SEEKDEEP home

`resolveSeekdeepHome()` resolves the single-root SeekDeep Harness home. Precedence, highest first: an explicit configured path, `$SEEKDEEP_HOME`, then `~/.seekdeep`. The harness keeps all user data under one root.

`seekdeepHomePath(...segments)` joins child segments onto that resolved home with Node's platform path rules. With no segments it returns the home itself.

`seekdeepHomeDisplay()` names an active root symbolically for user-facing paths: `~/.seekdeep` for the default home, `$SEEKDEEP_HOME` for any configured home. It never leaks an absolute machine path.

`SEEKDEEP_HOME_DIR_NAME` owns the default user-data directory name: `.seekdeep`.

`defaultSeekdeepHome()` returns the default SeekDeep Harness home by joining the operating-system home directory with `.seekdeep`, using Node's platform path rules.
