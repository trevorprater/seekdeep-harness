...
A package brands the ids it owns — `CallId` in `seekdeep-llm`, the shared agent/session `SessionId` in `seekdeep-session`, and `JobId` in `seekdeep-jobs`. Brand cross-package ids that could plausibly be confused; not every string needs one.

This package owns only the primitive. Keeping it dependency-free lets `seekdeep-jobs`, for example, brand `JobId` without importing an unrelated capability package merely to reach `Branded`.
