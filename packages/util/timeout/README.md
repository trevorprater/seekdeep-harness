...
```ts
import { clampTimeout, deadline, idleWatchdog, MAX_TIMER_DELAY_MS, timeoutOf, TimeoutReason } from '@seekdeep-ai/seekdeep-timeout'
```
...
```ts
import { deadline, timeoutOf } from '@seekdeep-ai/seekdeep-timeout'
...
Indirectly, through consumers such as `seekdeep-tool-call-timeout-policy`, which may replace a provider result with a retained timeout error or suppress a late result.
