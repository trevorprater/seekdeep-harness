...
Durable objective state for an agent session, owned independently of the model-facing tools and continuation policy that consume it. Goal state is part of the owning session log; consumers depend on `seekdeep-goal`, never on the concrete agent loop.
