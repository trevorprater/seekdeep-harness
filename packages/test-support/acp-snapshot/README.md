...
Committed session fixtures use canonical packed rows; the [temporary repository migrator](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/scripts/migrate-packed-session-fixtures.ts) (`pnpm run migrate:packed-session-fixtures`) rewrites older fixture layouts, and its [removal proposal](../../../.agents/notes/proposed/process/2026-07-26-remove-packed-session-fixture-migrator.md) owns its deletion.
...
- **Built mode requires current artifacts** — run `pnpm run build` before selecting `SEEKDEEP_EXAMPLE_MODE=lib`; source mode remains the zero-build path.
