...
JSONL 持久会话存储后端：`SessionPersistence` 的一个具体实现（`seekdeep-session-persistence` seam）。每个会话有一个仅追加的逻辑 JSONL 日志，默认存储为 `.jsonl.zstd`；禁用压缩时使用原始 `.jsonl`。
