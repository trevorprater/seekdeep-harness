# @seekdeep-ai/seekdeep-command-compact
...
- id: command-compact
  name: '@seekdeep-ai/seekdeep-command-compact'
```

The shipped `seekdeep` base mounts it beside `compaction-basic`, and the Web client provides the command adapter. Automation surfaces that compose no command adapter keep automatic compaction only.
