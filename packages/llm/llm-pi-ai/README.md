...
```yaml
- id: llm
  name: '@seekdeep-ai/seekdeep-llm-pi-ai'
  config:
    providers:
...
API-key credentials never enter that collection. The harness resolves a route's key through its own seam before the request reaches pi-ai and passes it as the request's `apiKey` option, which pi-ai treats as the highest-priority auth override, preserving the harness's fail-loud reference semantics. The collection receives one credential store whose reads and writes are restricted to `openai-codex`; every other provider sees no stored credential and retains its existing ambient path. A non-Codex route naming no credential resolves as configured-but-keyless and leaves the requirement to its provider or protocol, which is where it actually lives.
