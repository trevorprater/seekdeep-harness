...
- name: '@seekdeep-ai/seekdeep-llm-retry'
```

The executor has no policy config. Multi-provider adapters such as `seekdeep-llm-pi-ai` place `retryPolicy` inside each provider profile, avoiding a second provider-name list.
