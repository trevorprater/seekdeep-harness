...
npm scope: `@seekdeep-ai/seekdeep-*`; Cordis `Service` subclasses and function plugins contribute through `ctx.effect()`, `ctx.on()`, or `ctx.waterfall()`. Rules: [package](AGENTS.md), [root](../AGENTS.md#conventions).

## Hierarchy

Groups hold `packages/<group>/<pkg>/`; names stay `@seekdeep-ai/seekdeep-<pkg>`. **Group READMEs own package/ctx-key maps.**
...
| [`bundle/`](bundle/README.md) | Installable `seekdeep --profile` patch layers | Product — stable API |
