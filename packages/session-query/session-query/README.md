# @seekdeep-ai/seekdeep-session-query
...
- `listEvents(sessionId)` loads the live-preferred raw log and classifies each event as `current`, `shadowed`, or `log-only` with the shared `seekdeep-session` surface fold.
