...
| Package | Role |
|---|---|
| `@seekdeep-ai/seekdeep-lsp` (this) | Service Definition: the service, provider registry keyed by branded id + extension mapping, per-query selection, request/result vocabulary, the `LspError` taxonomy |
| `@seekdeep-ai/seekdeep-lsp-stdio` | Service Provider: a generic local backend that registers configured stdio language-server providers |
| `@seekdeep-ai/seekdeep-tool-lsp` | Consumer: the model-facing `lsp` tool over `ctx.lsp` |
...
Providers register **capabilities**, not tools. `seekdeep-tool-lsp` is the only owner of the model-facing name, description, prompt guidance, schema, and presentation.
...
Indirectly, through `seekdeep-tool-lsp`, which owns the model-facing `lsp` schema, prompt, and rendered results while this registry contributes no prompt or schema itself.

#### KV Cache effect

No direct invalidation; `seekdeep-tool-lsp` owns request-prefix changes.
