...
的运行机制——模式、fixture、golden，以及与 `seekdeep web` 之间刻意保留的组合差异——记录在
...
## 不要在此 import `@seekdeep-ai/seekdeep-client-*`

import 一个 Client 包——无论值还是类型——都会把它整个 TypeScript 工程、以及它引用的每个工程
拉进 **Host 构建图**。这已经坑过本 lane 一次：四个 Client 消费方包引用了 `api/remotes` 的
Client face，而该 face 必须等 Host tsdown 生成 `@seekdeep-ai/seekdeep-goal/remote` 之后才能编译，
...
`@seekdeep-ai/seekdeep-client-web` import `AppWebEntry`、从
`@seekdeep-ai/seekdeep-client-modules/client` import boot manifest 类型：启动真实 shell 正是该
harness 的用途，且这两个包本来就在 Host 图中。另外，chat 场景从
`@seekdeep-ai/seekdeep-client-runtime/client` import `conversationContextKey`，因为
