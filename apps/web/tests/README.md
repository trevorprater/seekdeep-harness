...
the deliberate composition divergences from `seekdeep web` — are documented in
...
## Do not import `@seekdeep-ai/seekdeep-client-*` here
...
`@seekdeep-ai/seekdeep-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@seekdeep-ai/seekdeep-client-web` and the boot-manifest
type from `@seekdeep-ai/seekdeep-client-modules/client`: booting the real shell is what
...
`@seekdeep-ai/seekdeep-client-runtime/client` because `client/runtime` is reachable
