...
The executable locates the two assets that ship as npm data the way the source resolved them through Node's module lookup: the built web frontend (`@seekdeep-ai/seekdeep-web-frontend/dist`) and the shipped agent presets (`@seekdeep-ai/seekdeep/config/agent-presets`). Each is searched in order: the explicit `SEEKDEEP_WEB_DIST` or `SEEKDEEP_AGENT_PRESET_DIR` directory; the package found through `node_modules` from the installation anchor below the seekdeep home, then from the executable's own location (an npm install hoists the packages next to the platform package); a `web` or `agent-presets` directory beside the executable or under its `lib/seekdeep`; and finally the source checkout the executable was built from.
