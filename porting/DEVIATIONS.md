...
## DEV-005: instrumented coverage measures native lines, functions, and regions

- **Source behavior.** `pnpm run test:coverage` runs the unit suites under V8 instrumentation and holds every file under `packages/*/*/src` to a per-file 100% bar on statements, branches, functions, and lines, with an exclusion roster in `vitest.config.ts` (Windows-only, Windows-unsupported, and pwsh-conditional entries among them) and a reporter that names every uncovered location.
- **Ported behavior.** The same command runs the Rust suites under `cargo llvm-cov` (`crates/repository-tools/src/coverage_uncovered_locations/lane.rs`), measures the crates that realize a `packages/*/*/src` file (derived from the parity manifest, minus `src/bin` and `src/main.rs` entry points), feeds the source reporter the translated export, and holds each measured file to 100% lines, functions, and regions. `scripts/coverage-roster.json` is the exclusion roster: entries carry a reason and may be conditioned on `platforms` or lifted `unless: pwsh`, probed through the pwsh executor's own resolution as the source did. The gate rejects entries for missing or unmeasured files and names entries that no longer need lifting. The instrumented run tests the measured crates only: the crates hosting the compiled heavy suites (`scripts/coverage-exempt.ts`) and every other unmeasured package (tooling, native helpers, vendor ports) run uninstrumented in the static and exempt lanes, as the source kept its heavy suites out of instrumentation.
- **Observable delta.** Branch coverage is not measured: stable Rust has no branch instrumentation (`-C coverage-options=branch` is nightly-only), so the bar is three metrics rather than four. Browser code compiled to WebAssembly is not instrumented (LLVM's profiling runtime has no `wasm32` target), so client crates are measured only through the host-side paths their tests exercise. The roster is checked in as data rather than computed in a configuration file, so an entry that should hold on one host only must say so.
- **Rationale.** The alternative, keeping the Vitest lane, measured nothing once the TypeScript suites were withdrawn. Regions are the LLVM counterpart of Istanbul statements; the per-file bar, the roster semantics, and the uncovered-location output are preserved, and the two unmeasurable metrics are named here rather than reported as covered.
- **Affected surfaces.** `crates/repository-tools/src/coverage_uncovered_locations/{lane,launcher}.rs`, `crates/repository-tools/src/coverage_exempt.rs` (`INSTRUMENTED_LANE_EXCLUDED_PACKAGES`), `scripts/coverage-roster.json`, the `node-24-coverage` and `windows-native` CI lanes, and the `scripts/run-gates.ts`, `scripts/ci-workflow.spec.ts`, and `vitest.config.ts` parity rows.

## Explicit non-deviations

These port at full parity; they are listed so removal never creeps beyond DEV-001, DEV-002, DEV-003, DEV-004, and DEV-005:
