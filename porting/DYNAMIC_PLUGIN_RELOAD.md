...
- Generated Remote descriptors originate from the native Typert analyzer, which hosts the pinned TypeScript compiler library inside a Rust-owned engine behind a Rust filesystem system. `cargo xtask typert-corpus` runs the pinned generator specs and snapshots against the Rust generator, and `cargo xtask remote-contracts --capture-source` requires the Rust Host face model to equal the source analyzer's before emitting descriptors; both run inside `cargo xtask remote-milestone`. See the [native analyzer record](../.agents/notes/implemented/architecture/2026-09-06-native-typert-analyzer-hosts-the-compiler.md).
- The vendor Loader, Include, and Schemastery packages are the Rust `seekdeep-loader` and `seekdeep-schemastery` crates. The Loader keeps the source's entry identity, generated ids, `!!js` interpolation (tree carriers literal, `disabled` evaluated), diff-classified update versus replacement, previous-plugin reconstruction, aggregate rollback failures, nested-id resolution across Include subtrees, isolation realms, intercepts, and exact-generation settlement; plugin import is the Rust plugin catalog plus the hosted JavaScript module realm rather than Node's internal ESM loader hooks. File-backed Include trees are edited through their files (HMR refresh, patches, `initial` on first boot); runtime write-back of programmatic subtree mutations and self-disposed rows to the file is not implemented because no shipped consumer mutates an Include subtree at runtime, `loader/*` Cordis events have no Rust consumer, and `enableLogs` apply/reload/unload logging is not set by any shipped configuration. `crates/loader/tests/include_patch_oracle.rs` runs the pinned `applyEntryPatches` under Node and requires the Rust composer's entries and diagnostics to match. Schemastery factories the product never constructs (`lazy`, `transform`, `bitset`, `percent`, `natural`, `date`, `regExp`, `arrayBuffer`, custom `extend`, `simplify`) are not ported; Typert schemas are zod.
...
- `Context.is` preserves the source's shared symbol brand, mutable brand key, inherited markers, and getter receivers. Source comparison and Chromium cover independent runtime copies and foreign-realm objects; the brand is not a capability grant.
- Browser service access and callback binding share a Rust/WASM tracer with source tracker, shadow, receiver, and immediate-return semantics. Source comparison and Chromium verify listener-owned effect cleanup.
- Browser updates run the source Fiber-local and global waterfalls, preserve raw/config identity and veto results, validate before hooks, retain startup error identity, and use a nontransactional restart separate from native `PluginFiber::update`. Awaitable handles inherit the underlying Fiber. Symbol event registration, dispatch failures, identity, and teardown have source and Chromium evidence. The exposed event-hook table and complete per-handle lifecycle-state shadows remain open in the [GUI architecture record](../.agents/notes/implemented/architecture/2026-07-19-gui-web-client-architecture.md).
- Browser roots inject concurrent Fiber disposal with reverse start order and joined completion, so dependent disposers progress together and restart waits for the captured batch. Native roots keep serial disposal by default; linked children inherit the selected policy.
- `remote-loader` composes the browse picker through an ordinary home patch and loads the public Runtime package with real Immer. Its `pickDirectory()` call must issue one Host request and preserve the native-capability rejection exactly; path and null outcomes have focused WASM coverage. No interactive OS chooser is opened by this check.
