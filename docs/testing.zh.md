...
- **单元测试**（`pnpm run test`）：`cargo test --workspace --all-features` 运行每个 crate 自身的测试套件；测试文件与其所覆盖的 crate 放在一起。每个注册表都有一个 HMR（热模块替换）安全测试（对向该注册表贡献内容的 fiber 执行 dispose（资源释放），并断言清理完成）。优先覆盖边界情况、错误路径、事件顺序、并发竞态，以及针对约定回归的永久测试。
...
只 mock 开销高或不确定的边界（LLM（大语言模型）适配器、网络、时钟）；下游一切保持真实。手写替身只能证明桥接层在搬运字节，不能证明交付的工具行为符合断言。桥接工具调用测试将脚本化 mock 模型与真实工具和执行器配合使用：`makeBridgeHarness({ withBash: true })` 接入 `seekdeep-bash-local` 与 `seekdeep-tool-bash`，然后运行 `echo`。
...
- 「真实入口路径」指构建产物而非源码树：工作区交付的 Rust 二进制文件、其背后的 WebAssembly 产物，以及 vendored 兼容清单发布的 `lib` 输出。构建产物冒烟测试针对这些输出运行——`cargo xtask client-test-runtime-built-smoke` 会构建并检验编译后的 Client 运行时，`cargo xtask wasm-package` 会重建各清单的 `lib`——真正缺失的配置必须以非零状态退出，而不是回退到默认值。
