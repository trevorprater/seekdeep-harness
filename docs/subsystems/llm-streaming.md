...
Source: [`packages/llm/llm/src/types.ts`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/llm/llm/src/types.ts)
...
Source: [`packages/llm/llm/src/message.ts`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/llm/llm/src/message.ts)
...
- **An empty completion is a retryable error, not a silent success.** Both adapters map a terminal `stop` finish that carried no content blocks to `finish {kind:'error'}` with the canonical `EMPTY_RESPONSE` code, and `seekdeep-llm-retry` retries it by default; see [empty model responses are retryable](../../.agents/notes/implemented/bug-fix/2026-07-24-empty-model-response-is-retryable.md).
...
The static public application identity every adapter sends to providers ([`packages/llm/llm/src/attribution.ts`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/llm/llm/src/attribution.ts)). `attributionHeaders(identity?)` maps it to the standard `User-Agent` header only; OpenRouter-specific app attribution headers are intentionally not supported by this contract. The default `APP_IDENTITY` sources its version from the package manifest; every field is a public product fact - no secrets, paths, session ids, or per-user identifiers, and nothing per-request may influence the values. Rationale: [Mandatory `User-Agent` attribution](../../.agents/notes/implemented/architecture/2026-06-21-mandatory-app-attribution-headers.md).
...
`BlockAssembler` ([`packages/llm/llm/src/assembler.ts`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/llm/llm/src/assembler.ts)) is the single shared implementation that folds a `StreamChunk` stream back into `ContentBlock`s, usage, finish reason, and replay state. The loop logs the raw chunks while feeding the same chunks through an assembler, then stores the assembled assistant content with the provider and model that produced it. A consumer that needs the assembled result without re-implementing the fold uses this.
...
declare class BlockAssembler {
   ...
   */
  message(source: MessageSource = { kind: 'plugin', plugin: 'seekdeep-llm/assembler' }): Message;
}
```
...
interface LlmConfigurableProvider {
  ...
  */
  ...
  /** Authentication setup the owning adapter can actually serve. */
...
interface GenerateOptions {
   ...
   * the derived history (seekdeep-agent-loop); a hand-built one-shot passes any list.
   */
  messages: Message[]
...
`GenerateOptions.tools` carries `ToolSchema` — the JSON-schema description of a tool, as sent to the model. It is declared in seekdeep-llm (not seekdeep-tools) precisely because it is part of the request the loop assembles every step:

```ts type-equiv
/**
 * JSON-schema description of a tool, as sent to the model.
 *
 * Declared here (not in seekdeep-tools) because it is part of {@link GenerateOptions};
 * seekdeep-tools' ToolDefinition and seekdeep-system-prompt's PromptAssembly both import
...
Source: [`packages/llm/llm/src/index.ts:284`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/llm/llm/src/index.ts)
...
Source: [`packages/llm/llm/src/types.ts:23`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/llm/llm/src/types.ts)
...
Source: [`packages/llm/llm/src/index.ts:64`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/llm/llm/src/index.ts)
