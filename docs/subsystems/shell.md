...
The bash execution seam is split across a Service Definition ([seekdeep-shell](../../crates/shell), `ctx.shell`), Service Providers ([seekdeep-bash-local](../../packages/shell/bash-local) and [seekdeep-bash-sandbox](../../packages/shell/bash-sandbox)), and Consumer ([seekdeep-tool-bash](../../packages/shell/tool-bash), the `bash` schema). Generic background-job ids, ownership, and controls live in [jobs.md](jobs.md); this seam returns a task-free process handle. Raw process-group mechanics live behind the [subprocess seam](subprocess.md).

Source: [`packages/shell/shell/src/types.ts`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/shell/shell/src/types.ts)

## Managed shell environment namespace

`SEEKDEEP_*` variables are Harness-owned child-process facts. The model-facing bash tool collects them through `ctx.shellEnv` and passes them through `ShellExecRequest.seekdeepEnv`; the subprocess service removes inherited `SEEKDEEP_*` names before merging the current snapshot. The `SeekdeepEnvironmentKey`/`SeekdeepEnvironment` vocabulary is owned by the [subprocess seam](subprocess.md) and re-exported by `seekdeep-shell`.
...
interface ShellExecRequest {
   ...
   * scrub. Managed facts belong in {@link seekdeepEnv}, which merges after this
  ...
  /**
   * Harness-owned `SEEKDEEP_*` variables for this execution (typed to managed
   * keys). Executors discard ambient `SEEKDEEP_*` entries before merging this
   ...
   */
  seekdeepEnv?: SeekdeepEnvironment | undefined
...
interface ShellExecSpec {
   ...
   * {@link ShellExecRequest.env}; {@link seekdeepEnv} still merges after them.
   * OPTIONAL on the spec for the same reason as `stdin`: absent means no
   * ordinary extra environment.
   */
  env?: Record<string, string> | undefined
  /** Managed `SEEKDEEP_*` snapshot (typed to managed keys); merges after {@link env}. */
  seekdeepEnv?: SeekdeepEnvironment | undefined
...
Each stream is a `CollectedOutput` — the (possibly truncated) text plus recovery info; when truncated, `text` is the **tail** and the complete stream spills to a private file. The fields are owned by the [subprocess seam](subprocess.md) and re-exported by `seekdeep-shell`.

## File sandbox: `ShellSandboxInfo`

A sandbox-consuming executor exposes its configured mode fallback through `ShellExecutor.sandboxMode`. The tool layer asks [`@seekdeep-ai/seekdeep-sandbox-policy`](../../crates/sandbox-policy/README.md) to resolve each calling session's durable `sandbox/mode` override and immutable cwd into `ShellExecRequest.sandboxPolicy`; a user-approved strictly wider call replaces only the mode. The mode/root/enforcement vocabulary is owned by the [`@seekdeep-ai/seekdeep-sandbox` seam](sandbox.md); modes govern file effects only.
...
`start()` returns a handle with no id or owner. `seekdeep-tool-bash` adapts it into `ctx.jobs.start()` hooks; the generic runtime then owns job identity and lifecycle. `done` resolves when the process closes and never rejects, reads remain valid after settlement, and sandbox facts are stamped before `done` resolves.
...
`ShellExecutor` owns `resolve`, foreground `run`, background-process `start`, and the `sandboxMode` capability fact. `seekdeep-bash-local` owns command defaulting, timeout/abort classification, the terminal environment, and the background read merge; process groups, bounded collectors, spill files, credential scrubbing, and disposal quiescence are the [subprocess service](subprocess.md)'s. `seekdeep-tool-bash` owns model-facing rendering and adapts background handles into the [generic job runtime](jobs.md). `seekdeep-shell` owns the shell tools' shared exit-status contract: the exported `parseExitStatus`/`ParsedExitStatus` inverts the `[exit code: N]` / `[killed by signal: X]` markers `seekdeep-tool-bash`'s `renderResult` and `seekdeep-tool-pwsh`'s `renderPwshResult` append, and both tools' `presentResult` use it to split the rendered text into the terminal card's output body and its exit-status pill.
...
Source: [`packages/shell/shell/src/index.ts:65`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/shell/shell/src/index.ts)

<a id="ctxshellenv--shellenvregistry"></a>

### `ctx.shellEnv` — `ShellEnvRegistry`

Registry (`ctx.shellEnv`) for trusted, per-execution `SEEKDEEP_*` variables. The namespace is rebuilt for every model shell call: ambient `SEEKDEEP_*` values are discarded by the executor, then the registry's current snapshot is injected. Built-in shell facts remain owned by the registry itself while plugins can register additional, enumerable facts with effect-scoped disposal.
...
/**
 * Build the trusted `SEEKDEEP_*` snapshot for one shell tool execution.
 * @param execution - the current tool execution.
 * @returns an immutable environment overlay containing built-ins and current contributions.
 */
collect(execution: ToolExecution): SeekdeepEnvironment
...
Types: [SeekdeepEnvironment](subprocess.md) · [ToolExecution](tools.md)

Source: [`packages/shell/shell-env/src/index.ts:89`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/shell/shell-env/src/index.ts)
