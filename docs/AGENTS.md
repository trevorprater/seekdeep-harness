...
This file defines document structure, Markdown tiers, writing rules, and `verify-doc-budgets` ceilings. Use [seekdeep-doc-standards](../.agents/skills/seekdeep-doc-standards/SKILL.md) for placement and validation, and [seekdeep-prose-standard](../.agents/skills/seekdeep-prose-standard/SKILL.md) for required coverage and editorial judgment; the [doc-tiers Agent Note](../.agents/notes/implemented/process/2026-07-04-doc-tiers-and-budgets.md) owns rationale.
...
| Subtree `AGENTS.md` (`crates/`, `packages/`, `examples/`, `docs/`, `.agents/notes/`) | Subtree-specific orders | Repo-wide rules the root file already carries |
...
- **Pairs update together**: [Terminology-guided](i18n/terminology.md), single-pass active-agent work repositions first-use annotations, preserves untouched prose, and re-records; `seekdeep-translate-docs` remains user-invoked ([contract](i18n/README.md)).
- **Comments and JSDoc state complete contracts, not reasoning transcripts.** Preserve behavior, failure, timing, ownership, modality, exceptions, consequences, and non-obvious orientation; delete narration, test walkthroughs, review analysis, and code restatement. Keep the local contract and link its rationale. Use [seekdeep-prose-standard](../.agents/skills/seekdeep-prose-standard/SKILL.md) for details.
...
Hunt these in any doc; [seekdeep-doc-standards](../.agents/skills/seekdeep-doc-standards/SKILL.md) runs this list as an audit:
