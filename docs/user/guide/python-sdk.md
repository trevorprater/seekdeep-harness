...
# export SEEKDEEP_MODEL=deepseek-v4-flash
# export SEEKDEEP_SYSTEM_PROMPT='You are a helpful software engineer assistant.'
```
...
| Property | Value |
|---|---|
| System prompt | `SEEKDEEP_SYSTEM_PROMPT`, falling back to `You are a helpful software engineer assistant.` |
| Model in `minimal.py` | `--model`, then `SEEKDEEP_MODEL`, then `deepseek-v4-flash` |
...
| Session persistence | Uncompressed JSONL under `SEEKDEEP_SESSION_ROOT` |
