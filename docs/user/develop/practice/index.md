...
- **Service Definition** (`seekdeep-shell`) — defines the Cordis service and Bash request and result types
- **Service Provider** (`seekdeep-bash-local`) — executes commands on the local machine
- **Consumer** (`seekdeep-tool-bash`) — exposes the capability as a model-callable tool

```
┌─────────────┐     ┌──────────────────┐     ┌──────────────┐
│  seekdeep-shell   │────▶│  seekdeep-bash-local  │     │ seekdeep-tool-bash│
...
import { MyCapService, type MyCapRequest, type MyCapResult } from '@seekdeep-ai/seekdeep-my-cap'
...
```yaml
- name: '@seekdeep-ai/seekdeep-my-cap-local'
- name: '@seekdeep-ai/seekdeep-tool-my-cap'
```
