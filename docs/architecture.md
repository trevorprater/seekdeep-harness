...
[Cordis](cordis-primer.md) is the framework under seekdeep: plugins contribute services, typed events, and reversible effects to a shared context. Every part of the product is a plugin, including the model adapter, the tool registry, the session log, and the agent loop itself, so every part is replaceable from configuration.

There is no privileged core to patch: you extend seekdeep by mounting a plugin beside the others, and registrations are effects that unwind when their plugin unloads.

## Profiles and bundles

A running `seekdeep` is a plugin tree composed at boot from ordered layers.
...
Each declares itself in its own `package.json` under a `seekdeep` field: `seekdeep.profile` lists a profile's bundles, and `seekdeep.bundle` points at a bundle's patch file.

[`seekdeep-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`seekdeep-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`seekdeep-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all.
...
```sh
seekdeep --profile web --dump-config
```
...
| Add persistent terminal execution | register a `ctx.terminals` backend plus `seekdeep-tool-terminal` |
