...
import { CallId } from '@seekdeep-ai/seekdeep-llm'
...
```ts
import type { Context } from '@seekdeep-ai/cordis'
import type {} from '@seekdeep-ai/seekdeep-tools'
...
The `import type {} from '@seekdeep-ai/seekdeep-tools'` line pulls in the package's declaration merges so `'tools/result'` and its payload are typed — the same move as chapter 4's `stats.ts` import, at package scale.

## Compose and run

```yaml
- name: '@seekdeep-ai/seekdeep-system-prompt'
- name: '@seekdeep-ai/seekdeep-tools'
- name: './tool-logger.ts'
- name: './greet-tool.ts'
```

`@seekdeep-ai/seekdeep-tools` injects the `systemPrompt` service because tools contribute schemas to the system prompt, so the composition lists its provider too. Without it, the tools plugin remains PENDING as described in [chapter 6](06-composition-and-hmr.md).
