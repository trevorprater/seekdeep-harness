...
- **Unit** (`pnpm run test`): `cargo test --workspace --all-features` over every crate's own suites; tests stay with the crate they exercise. Every registry gets an HMR-safety test (dispose the contributing fiber, assert cleanup). Prefer edge cases, error paths, event ordering, concurrency races, and permanent tests for contract regressions.
...
- "Real entry path" means the built artifact rather than the source tree: the Rust binaries the workspace ships, the WebAssembly outputs behind them, and the `lib` output the vendored compatibility manifests publish. Built-artifact smokes run against those outputs — `cargo xtask client-test-runtime-built-smoke` builds and exercises the compiled client runtime, and `cargo xtask wasm-package` rebuilds each manifest's `lib` — and a genuinely missing config must exit non-zero rather than fall back to a default.
