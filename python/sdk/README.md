# SeekDeep Harness Python SDK

English | [中文](https://github.com/deepseek-ai/seekdeep-harness/blob/master/python/sdk/README.zh.md)

Python subprocess SDK for driving SeekDeep Harness over JSON-RPC stdio. The
runtime inherits normal SeekDeep Harness environment variables such as
...
Install the `seekdeep-harness-sdk` distribution from PyPI; the import module remains `deepseek_harness`:

```sh
python -m pip install seekdeep-harness-sdk
```

Installing `seekdeep-harness-sdk` installs the exact same-version `seekdeep-harness-runtime-bin` platform wheel. The normal entry point therefore needs no executable argument:
...
By default, the SDK launches the bundled single-file `seekdeep-jsonrpc-agent` executable from the `seekdeep-harness-runtime-bin` package and injects that package's default configuration (the stdio JSON-RPC server, agent core, preloaded DeepSeek adapter, JSONL session persistence with an explicitly composed semantic checkpoint policy, local bash) via `SEEKDEEP_CORDIS_CONFIG`. To run a plugin composition of your own, keep the `@seekdeep-ai/seekdeep-sdk-jsonrpc-server` entry in the config and pass the Cordis config path.
...
The [Python SDK tutorial](https://github.com/deepseek-ai/seekdeep-harness/blob/master/docs/user/guide/python-sdk.md) provides an ordered installation and first-run path without the Web UI. The [`jsonrpc-agent` example](https://github.com/deepseek-ai/seekdeep-harness/blob/master/examples/jsonrpc-agent/README.md) owns the complete standalone Cordis file used there.
...
The same behavior can be selected for the runtime subprocess with `SEEKDEEP_CORDIS_CONFIG`. The injection lives in `HarnessClient.start()`, so the low-level client's default launch gets it too: when the launch resolves to the bundled runtime and neither `cordis` nor a non-empty `SEEKDEEP_CORDIS_CONFIG` is set (the runtime treats an empty value as absent, and so does the injection check), the bundled default configuration is used; an explicit `runtime_bin`, `bridge_bin`, or `launch_args_override` disables the injection entirely. See the [sdk-runtime README](https://github.com/deepseek-ai/seekdeep-harness/blob/master/python/sdk-runtime/README.md) for the runtime carriers (production exe vs dev-only node closure) and how to obtain them.

`cwd` and `runtime_cwd` are resolved to absolute paths before subprocess launch, environment injection, and the wire hanseekdeepake. The public API exposes only applied options: deployment persona and persistence belong in `cordis.yml`, while `session_root` remains the high-level convenience that sets `SEEKDEEP_SESSION_ROOT`.
