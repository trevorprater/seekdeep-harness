...
- Runtime code reload follows [`porting/DYNAMIC_PLUGIN_RELOAD.md`](porting/DYNAMIC_PLUGIN_RELOAD.md): built-ins use native lifecycle remounting, reloadable code is Rust-produced WebAssembly, and dynamically replaceable native-only integrations use Rust subprocesses behind versioned RPC. Do not use ordinary Rust dynamic-library unloading as the general reload mechanism.
