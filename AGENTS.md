...
- Determinism perimeter: crates implementing the Cordis core, agent loop, session-persistence decision logic, and retry/timeout policy take wall-clock time and any task-interleaving policy through injectable seams rather than ambient `Instant::now()`/`SystemTime::now()` or executor defaults, so a seeded scheduler and clock can drive them in tests. Boundary crates (terminal, subprocess, provider clients, sandbox, native integrations) may use ambient time; their effects are observed at the call boundary. This applies to new and substantially rewritten code only — do not refactor existing verified crates to add seams without explicit instruction.
- No ambient randomness: the workspace intentionally contains no `rand`/`thread_rng`/`fastrand` usage. Code that needs a PRNG must take an injected, seedable source.
