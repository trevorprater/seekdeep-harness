---
name: seekdeep-doc-site-sync
description: Use when publishing, updating, moving, or removing SeekDeep Harness documentation website pages; editing website/docs.json mappings or navigation; diagnosing a page missing from the VitePress site; fixing projected documentation links; or running the docs:dev, docs:check, and doc-sync workflow after website-content changes.
---

# Synchronizing the SeekDeep Harness Documentation Site

Keep repository Markdown as the only editable content source. Treat the website as a tested projection: [website/docs.json](../../../website/docs.json) selects public pages, [the Rust projector](../../../crates/repository-tools/src/doc_site.rs) rewrites them into the disposable `website/.generated/` tree, and VitePress builds that tree.
...
- Read [docs/AGENTS.md](../../../docs/AGENTS.md) and use [seekdeep-doc-standards](../seekdeep-doc-standards/SKILL.md) when deciding where content belongs or changing product documentation prose.
...
- Read the current `DocsPage` type in [doc_site.rs](../../../crates/repository-tools/src/doc_site.rs) and entries in [website/docs.json](../../../website/docs.json) before changing the manifest; do not rely on a remembered field set.
- Read [the Rust site configuration](../../../crates/repository-tools/src/doc_site/config.rs) before adding a new section, sidebar collection, locale, or top-level navigation item. [.vitepress/config.mjs](../../../website/.vitepress/config.mjs) only connects compiled behavior to VitePress.
...
- `source`: repository-relative canonical Markdown path. For a complete bilingual pair, add root and English entries naming the `.zh.md` and `.md` siblings, their content locales, and counterpart aliases.
- `route`: public VitePress path including the `.md` suffix.
- `label`: sidebar label, not necessarily the document H1.
- `sidebar`: reuse `zh-guide`, `zh-develop`, `zh-reference`, `en-guide`, `en-develop`, or `en-reference` unless the information architecture needs another collection.
- `section`: reuse an existing section when possible. If adding one, also declare its per-locale placement and collapse policy in the manifest's `sections` map.
...
Point both locale entries at the same source only when the page intentionally falls back to one available language. Update the root entry and aliases when its Chinese counterpart is added. Keep the manifest an explicit public allowlist. Do not publish RFCs, postmortems, testing guides, `AGENTS.md`, or maintainer workflows merely because they exist under `docs/`; add internal material only when the user explicitly expands what the site publishes.
...
`docs:prepare` compiles the callbacks with the workspace's WASM toolchain and installs the website's locked third-party dependencies into its ignored build cache. The website importer in the root lockfile remains the version authority. Canonical declaration links generated from the oracle pass through [doc_source_links.rs](../../../crates/repository-tools/src/doc_source_links.rs); their pinned source paths and line numbers must not be redirected to unrelated Rust declarations.
...
Use [seekdeep-pre-push-checks](../seekdeep-pre-push-checks/SKILL.md) before pushing. Report the canonical files changed, manifest entries added or removed, public routes affected, and the exact checks run.
