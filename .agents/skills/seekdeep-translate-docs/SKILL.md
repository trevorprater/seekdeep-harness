---
name: seekdeep-translate-docs
description: Manually run the extended SeekDeep Harness bilingual-document workflow, including generated briefings, delegated prose translation, whole-document translation, and scoped pairing verification.
...
Run this extended workflow only when the user explicitly invokes `seekdeep-translate-docs` by name. Never select or load it for ordinary documentation work, from another skill, or from an inferred translation need; routine translation follows the one-shot, one-pass rule in [docs/AGENTS.md](../../../docs/AGENTS.md).
...
- **[seekdeep-prose-standard](../seekdeep-prose-standard/SKILL.md)** — required prose coverage and editorial judgment. Apply it to both sides without adding or dropping source propositions.
...
4. Before the PR: the touched pairs are green under the scoped check; `pnpm run doc-sync` (which includes the corpus-wide pairing check plus `verify-md-wrap`/`verify-md-links`) runs once at PR level per [seekdeep-pre-push-checks](../seekdeep-pre-push-checks/SKILL.md), not inside each translation task.
