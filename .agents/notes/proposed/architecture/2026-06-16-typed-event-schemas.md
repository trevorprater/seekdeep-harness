...
- **Six merge-extensible maps** (~370 LOC of core types): `ContentBlockMap`, `MessageSourceMap`, `FinishReasonMap` (in `seekdeep-llm`); `TurnTriggerMap`, `TurnEndReasonMap`, `SessionEventMap` (in `seekdeep-session`).
- **~10 `declare module` augmentation sites** across `seekdeep-agent`, `seekdeep-agent-loop`, `seekdeep-shell`, `seekdeep-llm`, `seekdeep-session`, `seekdeep-session-persistence`, `seekdeep-system-prompt`, `seekdeep-tools` — each would move from declaration merging to a runtime `register()` call.
- **The event producers** — 16 `session.append(...)` call sites in the loop — unchanged in shape but now validated at the boundary.
- **~7 switch-consumers** that branch on these unions: `deriveMessages` and the package-owned invariant companion (`seekdeep-session`), `BlockAssembler` (`seekdeep-llm`), both LLM adapters (`seekdeep-llm-deepseek`, `seekdeep-llm-pi-ai`), and the tool schema layer (`seekdeep-tools`). The `assertNever`-on-closed-unions vs fall-through-on-extensible-unions convention (a documented lint rule) would need rethinking — runtime variants are not statically exhaustive.
- **The `defineTool` `InferArgs` DSL** (`seekdeep-tools`), which derives zero-cast `execute` arg types from a compile-time schema spec — the showcase of the current approach.
