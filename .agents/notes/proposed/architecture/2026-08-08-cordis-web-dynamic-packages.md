...
The model needs to extend the current SEEKDEEP process temporarily without modifying repository source, rebuilding the application, or refreshing the browser. An extension may run in the Host Node.js process, in a Client browser page, or as one plugin whose Host half retrieves data and whose Client half presents it.
...
| --- | --- | --- |
| `tool-cordis` | `@seekdeep-ai/seekdeep-tool-cordis` | Registers the System Prompt, seven model-facing Tools, Host Inspect Providers, `@pluginId` context injection, and Tool presentation metadata |
| `cordis-host-runner` | `@seekdeep-ai/seekdeep-cordis-host-runner` | Stores the authoritative Registry, allocates IDs, executes Host code, and manages versions, approvals, Runs, private handlers, Inspect routing, and model feedback |
| `cordis-client-runner` | `@seekdeep-ai/seekdeep-cordis-client-runner` | Synchronizes Inspect manifests in the browser, orchestrates approved Host→Client activation, evaluates Client code, and manages the Guard, Loader/Fiber, timer, styles, and teardown |
| `ui-cordis` | `@seekdeep-ai/seekdeep-client-ui-cordis` | Renders Define/Run Tool cards, the global Cordis panel, approval controls, version selection, runtime status, and Package-specific business views |
...
The Rust port implements the Host Runner and lifecycle authority in native Rust. Rust-owned source-compatible infrastructure evaluates or translates model-authored Host JavaScript according to the strategy selected by [`porting/DYNAMIC_PLUGIN_RELOAD.md`](../../../porting/DYNAMIC_PLUGIN_RELOAD.md); typed messages carry only lossless JSON and lifecycle commands across that boundary. Host/Client wire types live in the target-portable `seekdeep-cordis-dynamic-types` crate, so the Rust/WASM Client never depends on a Host evaluator, Host Tools, or Agent runtime code. The browser Runner is Rust compiled to WebAssembly and controls Client evaluation without moving registration, approval, generation ownership, teardown, or protocol state into handwritten browser JavaScript. The custom Cordis core provides typed Services, exact-generation Fibers, a reversible effect ledger, deterministic cancellation, and injectable timing and scheduling policies. These correctness mechanisms may strengthen the source implementation but must not change its observable protocol.
...
The static `ui-cordis` browser package is Rust compiled to WebAssembly. Rust owns its four React component functions, Slot and Remote assembly, inventory generation guard, approval and version actions, `@pluginId` source, and exact Host/Client status projection. It receives the page's existing React and primitive-module instances through generated bindings, while pinned-source generation supplies localized dictionaries and class-scoped CSS. Handwritten JavaScript does not own UI state, lifecycle decisions, or registration behavior.
...
`DynamicCordisRunnerService` and its internal Registry are the sole authority in the current SEEKDEEP process. They store:
...
Host and Client both execute only plain JavaScript function bodies, without TypeScript, JSX, or bundler transformation. Rust-owned Host infrastructure evaluates or translates the accepted source-compatible subset; the Rust/WASM Client Runner requests evaluation through the browser engine inside a restricted closure. These contexts reduce misuse and provide instructional errors, but they are not security boundaries against malicious code.
...
The Rust Host evaluator assigns every lifecycle command an activation-local identity and publishes it through a generation-owned command pump. When apply, handler, Tool, Service, or callback code awaits, the worker flushes registrations before suspending and re-enters only for generation-owned callbacks or deactivation; stopping rejects pending timer Promises. A disposer removes only its exact published effect. The timer façade preserves callback and Promise timeouts, callback and async-iterator intervals, and throttle/debounce wrapper disposal. Lossless JSON cloning and Host conversion use explicit work stacks so deeply nested plain data does not recurse through JavaScript or Rust call stacks.
