...
Three packages hand-roll promise-wrapped timers that the `node:timers/promises` builtin already provides, while other packages (`seekdeep-llm-mock-server` `pause()`, `seekdeep-lsp-stdio`, `seekdeep-acp-snapshot`) already use the builtin — so the hand-rolled copies are also a consistency gap:
