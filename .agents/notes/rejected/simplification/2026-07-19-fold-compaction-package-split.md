...
Compaction is split between `@seekdeep-ai/seekdeep-compaction`, which owns an abstract two-method service and shared types, and `@seekdeep-ai/seekdeep-compaction-basic`, which owns the only complete provider. Shipped configurations load only the basic package, and no production package independently consumes the Service Definition package except that provider.
...
Move the basic implementation into `@seekdeep-ai/seekdeep-compaction` and remove `@seekdeep-ai/seekdeep-compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `@seekdeep-ai/seekdeep-compaction-basic` and its workspace/package metadata are removed.
- `@seekdeep-ai/seekdeep-compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.
