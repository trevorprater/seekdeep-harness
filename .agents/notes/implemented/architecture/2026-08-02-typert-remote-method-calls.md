...
`@seekdeep-ai/seekdeep-api-gateway`, located at `packages/api/gateway`, provides two symmetric faces: its default entry provides Host `ctx.typertGateway`, while its `/client` entry provides consumer-side `ctx.remote`. Each side consumes a locally generated `InvocationDescriptor` from the same model; descriptors are not sent over the wire. The Remote data protocol runs over Connection's shared `/api` RPC channel. The business calling interface does not change when Connection migrates from HTTP to WebSocket.

`@seekdeep-ai/seekdeep-api-remotes`, located at `packages/api/remotes`, is the BFF layer above the Gateway. Its Host entry owns Agent/Session identity resolution and Typert lookup configuration; its `/client` entry selects the generated Remote contributions exposed by the application. The Client entry consumes the shared `TypertClientRemote` contract through Cordis rather than importing the concrete Gateway implementation.

## Components and Cordis services

| Component | Cordis service | Responsibility |
|---|---|---|
| `@seekdeep-ai/seekdeep-typert-protocol` | Declares only the minimal `ctx.typert` protocol | `TypertRemoteService`, decorators, binding fallback, descriptors, lookup/Context, and the Remote map; no dependency on the compiler, Zod, Connection, or Browser |
...
Business packages depend only on the lightweight `@seekdeep-ai/seekdeep-typert-protocol`. It provides `TypertRemoteService` and declaration protocols for decorators, the binding fallback, lookup, Remote Scope, and descriptors, without depending on the TypeScript compiler, Zod, HTTP, or the Client runtime.
...
In SRC mode, the decorator may record the prototype, method name, and invocation mode in a `WeakMap` internal to `seekdeep-typert-protocol`. It writes no custom properties to a Service instance, prototype, constructor, or method function.
...
```text
InvocationDescriptor {
  id: '@seekdeep-ai/seekdeep-goal#goals/create'
  service: 'goals'
  namespace: 'goals'
  method: 'create'
...
Business-object and scoped-Context packages own stable declarations and default resolvers through `lookups.register()` and `contexts.registerHost()`; Host composition supplies effect-scoped asynchronous policies through `lookups.configure()` and `contexts.configureHost()`. `SessionStore` owns the `session` provider and its live-only default resolver: it composes in either Service load order, follows Typert Service replacement, and withdraws with the Session Store fiber. Configuration may precede provider registration, but does not by itself make an identity available without a live provider; unloading the configuration restores the provider's default resolver. API Remotes creates the shared `agentFor()` resolver for `agent` and `session` lookups and the `agent` Host Context: live Agents are reused, ordinary cold sessions are resumed automatically, concurrent resumes are deduplicated by Session ID, and the subagent ownership fence returns the existing `agent-busy`. The standard Web API Proxy supplies its Agent defaults and scope setup and consumes that resolver for legacy methods. The configured `session` lookup returns the resolved Agent's Session, while the `agent` Host Context returns its Context, so all three projections share one resume lifecycle.
...
```text
import type { SessionId } from '@seekdeep-ai/seekdeep-session/types'
import type { CreateGoalRequest, CreateGoalResult } from '@seekdeep-ai/seekdeep-goal/types'
```
...
```text
import goalsRemote from '@seekdeep-ai/seekdeep-goal/remote'
```
...
Code that needs only static types may use `import type {} from '@seekdeep-ai/seekdeep-goal/remote'`. This import is erased at runtime, loads no JS, and cannot trigger runtime registration. An environment that makes real calls must pass the contribution from a normal value import to the Client Remote Service.
...
`cargo xtask remote-declarations` emits the five Remote declaration/map pairs and their canonical compatibility-header closure. `cargo xtask remote-artifacts` also builds the public WASM values and Client facade; it validates each business package's `/remote` opt-in and keeps all output beneath the selected workspace output root. An ordinary single-package WASM build writes only that package. Browser packaging preserves the public headers instead of exposing backend WASM declarations. Each public `/remote` value delegates construction to compiled Rust/WASM and retains the ordinary `zod` import, so consumers share their installed schema dependency and configuration. The Client facade's classic bundle keeps its own bundled dependency, matching the source's distinct publication forms.
...
- `@seekdeep-ai/seekdeep-typert-protocol`: lightweight protocols for decorators, bindings, lookup, Remote Scope, and descriptors.
...
- `@seekdeep-ai/seekdeep-api-gateway`: its default entry associates Host definitions with Services, claims Remote endpoints, performs lookup, resolves Context receivers, invokes methods, encodes results, and registers an `/api` interceptor with Connection; its `/client` entry mounts Remote contributions, creates strict Remote namespace Services and methods, and delegates calls to `ctx.connection.rpc`. The entries share the Remote protocol but do not import each other's Cordis interface merges.
- `@seekdeep-ai/seekdeep-api-remotes`: the BFF layer; owns the Host Agent/Session resolver, selects Client `/remote` contributions, and exposes the merged Remote types to business packages through the shared `TypertClientRemote` contract.
...
- `cargo xtask remote-milestone` rebuilds the real Rust Host and production browser bundles, compares all selected descriptors and codec graphs with the pinned emitter, runs the source registry and gateway corpora against WASM, and executes the checked public consumer in Chromium through the real Connection transport. The browser check asserts root/scoped Goal persistence, rejection before transport, cancellation, undefined results, Host error preservation, and descriptor withdrawal across all five namespaces. This gate does not establish full workspace installation, release packaging, or complete Web UI parity.
- `cargo xtask remote-consumer --browser` compiles through the actual package exports with strict declaration checking, rejects invalid identities, payloads, arity, cancellation positions, mutable-result assumptions, unselected events, and Host-only access, then executes the checked consumer through the public WASM contributions against the Rust Host. `remote-codec-oracle` separately imports those values through package exports and compares both publication forms with the pinned codecs. Full workspace installation, release packaging, and complete Web UI parity remain outside this focused evidence.
...
- Importing `@seekdeep-ai/seekdeep-goal/remote` adds the strict `ctx.remote.goals.create(...)` type and declaration navigation to `remoteExportCreate`; omitting that import omits the namespace.
