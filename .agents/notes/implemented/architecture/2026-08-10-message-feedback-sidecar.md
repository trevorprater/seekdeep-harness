...
`@seekdeep-ai/seekdeep-message-feedback` owns the `ctx.messageFeedback` service and stores message feedback as one storage-domain sidecar row per Session. The sidecar is neither Session-log content nor a Session projection. It emits no `feedback/record` event and performs no telemetry handoff; the command-feedback and message-feedback contracts remain independent.
...
`put` accepts a target only when the current logical observation—directly from a live Session, otherwise from `SessionPersistence.inspect()`—contains a non-empty, append-origin `assistant/message` with that `MessageId`. Replacement-origin messages, empty usage-only assistant records, and non-assistant targets are rejected. Cold inspection neither publishes or resumes an Agent nor commits log repair merely to validate feedback. A cold `listSnapshots()` preflight classifies definite absence; inspection failure for a catalogued Session remains an infrastructure failure. A request in the narrow live-detach-to-header-materialization interval can therefore return `session-not-found`, and the caller retries after retirement materialization.
...
The source suite and independent Rust suite pin the Remote namespace, exact business envelopes, note and target validation, immutable item snapshots, per-message CAS and ABA fencing, per-Session serialization, reused-id lifecycle fencing, admission drain, cold-catalog races, physical checkpoint ordering, missing/failing flush paths, and detach during an admitted checkpoint. A real Rust Loader composition with JSONL Session persistence and JSON storage writes a target plus sidecar, tears down, cold-boots the same composition, and reads the item back. Persistence validation remains distinct from the optional live invariant so a previously validated assistant surface replacement can survive a cold restart without weakening live step enforcement.
