...
`@seekdeep-ai/seekdeep-api-gateway` 位于 `packages/api/gateway`，提供对称的两个 face：默认入口提供 Host `ctx.typertGateway`，`/client` 入口提供消费端 `ctx.remote`。两边各自在本地消费由同一模型生成的 `InvocationDescriptor`，descriptor 不通过 wire 发送。Remote 数据协议运行在 Connection 共享的 `/api` RPC channel 上；业务调用界面不随 Connection 从 HTTP 迁移到 WebSocket 而改变。

`@seekdeep-ai/seekdeep-api-remotes` 位于 `packages/api/remotes`，是 Gateway 上层的 BFF 层。其 Host 入口负责 Agent/Session 身份解析与 Typert lookup 配置；`/client` 入口选择应用对外暴露的生成 Remote contribution。Client 入口通过 Cordis 消费共享的 `TypertClientRemote` 约定，而不导入具体 Gateway 实现。

## 组件和 Cordis 服务

| 组件 | Cordis 服务 | 职责 |
|---|---|---|
| `@seekdeep-ai/seekdeep-typert-protocol` | 只声明 `ctx.typert` 的最小协议 | `TypertRemoteService`、decorator、binding 回退、descriptor、lookup/Context 和 Remote map；不依赖 compiler、Zod、Connection 或 Browser |
...
业务包只依赖轻量的 `@seekdeep-ai/seekdeep-typert-protocol`。它提供 `TypertRemoteService`，以及 decorator、binding 回退、lookup、Remote Scope 和 descriptor 的声明协议，不依赖 TypeScript compiler、Zod、HTTP 或 Client runtime。
...
SRC 运行时允许 decorator 在 `seekdeep-typert-protocol` 内部的 `WeakMap` 记录 prototype、方法名和调用模式。它不向 Service 实例、prototype、constructor 或方法函数写入自定义属性。
...
仅需要静态类型时可以使用 `import type {} from '@seekdeep-ai/seekdeep-goal/remote'`；这种 import 在运行时会被擦除，不会加载 JS，也不能触发任何运行时注册。需要真实调用的环境必须把普通 value import 得到的 contribution 交给 Client Remote Service。
...
`@seekdeep-ai/seekdeep-api-remotes/client` 集中加载需要的 Remote contribution：
...
Client 业务包只引用 `@seekdeep-ai/seekdeep-api-remotes/client`，不直接依赖 API Gateway 或各业务 `/remote` 运行时入口。API Remotes 消费共享的 `TypertClientRemote` 约定和 Cordis `ctx.remote` 服务，再重新导出声明，使所选 Remote map 进入业务编译；新增或移除整套 Client 能力只修改这一处 assembly。
...
`cargo xtask remote-declarations` 生成五组 Remote 声明/映射文件及其规范兼容声明闭包。`cargo xtask remote-artifacts` 还构建公共 WASM 值和 Client facade；它校验各业务包对 `/remote` 的显式启用，并把全部输出限制在选定的工作区输出根目录内。普通的单包 WASM 构建只写入该包。浏览器打包保留公共声明，不暴露后端 WASM 声明。每个公共 `/remote` 值都将构建委托给编译后的 Rust/WASM，并保留普通的 `zod` 导入，使消费方共享其安装的 schema 依赖与配置。Client facade 的 classic bundle 保留自己的打包依赖，与源码两种不同的发布形式一致。
...
- `@seekdeep-ai/seekdeep-typert-protocol`：轻量 decorator、binding、lookup、Remote Scope 和 descriptor 协议。
...
- `@seekdeep-ai/seekdeep-api-gateway`：默认入口关联 Host definition 与 Service，认领 Remote endpoint，执行 lookup、Context receiver 解析、调用和结果编码，并向 Connection 注册 `/api` interceptor；`/client` 入口挂载 Remote contribution，创建严格 Remote namespace Service 和方法，并把调用交给 `ctx.connection.rpc`。两个入口共享 Remote 协议，但不互相导入各自的 Cordis interface merge。
- `@seekdeep-ai/seekdeep-api-remotes`：BFF 层；负责 Host Agent/Session resolver，选择 Client `/remote` contribution，并通过共享的 `TypertClientRemote` 约定向业务包暴露合并后的 Remote 类型。
...
- `cargo xtask remote-milestone` 重新构建真实 Rust Host 和生产浏览器 bundle，将所有选定 descriptor 与 codec 图同固定版本的 emitter 比较，针对 WASM 运行源码注册表和 gateway 测试集，并在 Chromium 中经真实 Connection 传输执行已检查的公共消费方。浏览器检查针对全部五个 namespace 断言 root/scoped Goal 持久化、传输前拒绝、取消、undefined 结果、Host 错误保留以及 descriptor 撤销。该门禁不证明全工作区安装、发布打包或完整 Web UI 等价性。
- `cargo xtask remote-consumer --browser` 通过真实包导出进行严格声明检查，拒绝无效标识、payload、参数数量、取消参数位置、可变结果假设、未选定事件及 Host-only 访问，再通过公共 WASM contribution 执行已检查的消费方，连接 Rust Host。`remote-codec-oracle` 另外通过包导出导入这些值，将两种发布形式与固定版本的 codec 比较。全工作区安装、发布打包与完整 Web UI 等价性不属于这组聚焦证据。
...
- 导入 `@seekdeep-ai/seekdeep-goal/remote` 会加入严格的 `ctx.remote.goals.create(...)` 类型，并可通过 declaration 导航到 `remoteExportCreate`；不导入时不会出现该 namespace。
