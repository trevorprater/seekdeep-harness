...
- The directory is compartmented by `SessionId`, with per-key single-flight + an epoch guard (an old pull never overwrites newer state); `commands/change` soft-refreshes every touched key, `agent-preset/selected` soft-refreshes the recomposed session, and `connection/reset` drops and rewarms every key. Ready snapshots keep serving during a soft refresh, Enter strong-waits on the current key, and a failure keeps the draft with no downgrade. The source's `warm` hook prewarms each session at scope birth.
...
- The Rust implementation keeps directory, ranking, popup, and wire semantics in target-portable crates. The browser service, Cordis effect registrations, Slot assembly, React tree, and `AbortController` adapter compile to WebAssembly; popup business callbacks receive the exact JavaScript session projection captured at open. `seekdeep-commands` re-exports the descriptor from `seekdeep-commands-contract`, so browser builds do not link the native Agent/command runtime, and each optimized Client package exports only its own browser entrypoints.
...
- The popupSelect shell is shared by model selection and permission presets. Per-item Inbox operations, rich result cards, and roster configurability remain separate work.
