...
The central `seekdeep-invariants` service owns only configuration, registration uniqueness, child-fiber lifecycle, rollback, disposal, and package-attributed failure. It exposes no generic plugin-shape, service-shape, or startup-assertion helpers and imports no product package.
...
|---|---|
| `seekdeep-session` | Strict sequence growth, turn/step enclosure, and same-step tool call/result pairing. |
| `seekdeep-agent` | Non-repeating agent status and terminal disposal transitions. |
| `seekdeep-scope` | Scoped-event carrier presence and routed-subject consistency. |
| `seekdeep-agent-loop` | Explicitly marked, frozen loop request reconstruction from the session event log. |
| `seekdeep-llm` | Stream block grammar, delta type/index matching, single usage, closed blocks, and terminal finish. |
| `seekdeep-llm-retry` | Durable retry records identify the open turn's latest closed step, remain unique per step, increase monotonically, and stay within retry and non-negative timer bounds. |
| `seekdeep-tools` | Monotonic pre/execute/post stages and immutable final execution/result snapshots. |
| `seekdeep-system-prompt` | Authoritative assembly section, tool, and variable data constraints. |
| `seekdeep-compaction` | Compaction start/summary/end pairing, range endpoints, token counts, and successful-summary presence. |
| `seekdeep-hook-protocol` | Hook invocation/result correlation, dialect, identity, and duration constraints. |
| `seekdeep-sandbox-policy` | Durable `sandbox/mode` events use the closed sandbox-mode vocabulary. |
| `seekdeep-fs` | Filesystem decision/observation events carry usable target and version identities. |
| `seekdeep-goal` | Durable goal snapshots preserve source attribution, rendered content, revisions, lifecycle and timestamp relationships, and sequential admitted rounds. |
| `seekdeep-goal-round-driver` | Goal-sourced continuation messages match the prompt reconstructed from the preceding durable goal state. |
| `seekdeep-subagent` | Provider add/remove and child start/end events preserve identity and pairing. |
| `seekdeep-permission-presets` | Durable permission decisions name a preset in the active permission table. |
| `seekdeep-user-approval` | Approval asked/decided records pair by call and use valid outcomes and policies. |
| `seekdeep-workflow` | Workflow and child-agent start/end events preserve run metadata, identity, outcome, count, and error relations. |
| `seekdeep-jobs` | Current and terminal task snapshots preserve id/kind, owner, status, and timestamp relationships. |
| `seekdeep-tool-todo` | Durable whole-list snapshots use unique trimmed items and closed statuses. |
| `seekdeep-time-context` | Plugin-attributed clock readings agree with the session's open turn, next pre-step position, and elapsed baseline; rendered time parses and does not postdate its event. |
