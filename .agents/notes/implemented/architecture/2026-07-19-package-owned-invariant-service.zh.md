...
`@seekdeep-ai/seekdeep-invariants` 是与产品无关的 Cordis 服务插件，注册 `ctx.invariants`。它只负责配置、注册唯一性、子 fiber 生命周期和带包归属的失败；不导入 session、agent、scope 或 agent-loop 包，也不包含这些包的检查。
...
| 伴随入口 | 注册名 | 所属检查 |
|---|---|---|
| `@seekdeep-ai/seekdeep-session/invariant` | `@seekdeep-ai/seekdeep-session` | 会话序列、轮次/步骤包围关系和同一步骤的调用/结果轨迹 |
| `@seekdeep-ai/seekdeep-agent/invariant` | `@seekdeep-ai/seekdeep-agent` | agent 状态转换 |
| `@seekdeep-ai/seekdeep-scope/invariant` | `@seekdeep-ai/seekdeep-scope` | 作用域事件载体的存在性与主体一致性 |
| `@seekdeep-ai/seekdeep-agent-loop/invariant` | `@seekdeep-ai/seekdeep-agent-loop` | 模型请求重建 |
...
示例 agent 主干会挂载服务和四个有状态伴随子路径，并把 `enabled`、`package_allowlist` 与 `package_blocklist` 转发给服务。生成的 SDK Cordis 组合输出相同条目。子路径条目添加可安装的根 npm 包，而不会把子路径误当成包名。根据[交付配置决策](../simplification/2026-08-03-omit-invariants-from-shipped-config.md)，交付的 `seekdeep` TUI 与 Web 配置树会省略该服务及其伴随插件。
...
- **把所有检查保留在 `seekdeep-invariants`。** 不予采纳，因为注册表仍要导入所有被检查的产品领域，所有者变更仍需中央编辑，测试也继续远离被保护的约定。
