...
The two layers below made a pi-ai route [a declaration](2026-08-03-pi-ai-declared-provider-catalog.md) and gave the host a way to [interrogate a draft endpoint](2026-08-04-draft-provider-endpoint-interrogation.md). Neither reached a person who does not edit YAML: the Models page still offered one API-key field per provider and a fold with a base URL, so adding a gateway meant opening `$SEEKDEEP_HOME/settings.yaml` and knowing the profile shape, and correcting a stale context window meant the same. The capability existed and the surface did not expose it.
...
The Rust/WASM schema-form binding exposes the rehydrated node's structural children, metadata, and exact constant values, including null. Direct child access and path lookup share node identity; relation arrays and metadata retain stable object identity. Browser tests use the real rehydrator because a scripted protocol-choice helper cannot detect missing schema properties that disable provider creation in the assembled page.
...
The production `ui-settings-models` package is Rust/WASM. Target-portable Rust owns provider/settings/credential joins, latest-generation settlement, welcome acknowledgement, readiness, API-key judgment, capacity spelling, model validation, setup posture, provider identity, and minimal path operations. The browser binding renders through the page's injected React, `ui-primitives`, schema-form, and selector faces; Rust controllers own the observable snapshots and write ordering. The package's public Client face remains exactly `apply`, `inject`, and `refreshIfLoaded`.
...
The pinned source package's 219 tests drive the rendered page over a scripted wire face: adding, editing, and removing rows; a cleared optional field leaving the profile and a non-integer capacity never entering it; the interrogation carrying the edited endpoint, the unsaved key, and the profile's protocol; the picker's default selection, toggling, cancel, and adopt-keeps-tuned-rows; the empty, refused, and rejected-transport paths; the create writing one profile plus its credential; every gate on the create button; and the read-only posture. Native Rust tests pin the target-portable join, readiness, welcome, capacity, validation, path-operation, setup, identity, and route partitions. Live WASM tests drive compiled stores and components through registration, invalidation, modal lifecycle, buffer reindexing, discovery adoption, settings-before-credential retry, dormant-provider adoption, and credential-first deletion. The optimized classic-bundle smoke pins the exact module face and dependency isolation. `protocolChoices` remains covered against a schema that declares the union and one that does not. The stylesheet gate reads the package's own sources and fails any `<select>` that takes `.input` without `.selectInput`, because the OS arrow it would otherwise keep sits flush inside the 240px cap `select.input` imposes. The editor's own field inventory is asserted per route kind — a catalog route stops at the key and the endpoint, a declared one also carries the protocol — along with the protocol edit travelling as a single `api` path op, a rename travelling as a single `displayName` one, a cleared name unsetting rather than storing the empty string the adapter refuses, and a declared profile naming no protocol selecting nothing rather than the first choice. The assembled Web scenario reopens the declared route through the real wire, captures the card, and asserts the chosen protocol and the new name both reach `settings.yaml` and the row re-registers under the rename.
