...
单元测试覆盖固定估算、信封失效与锚点替换、回放边界、不可变快照、已路由压力、收敛、溢出 generation 证明与回滚。固定源 Loader 套件及其 Rust 镜像各通过 5 个依赖顺序激活与加载时拒绝案例，其中包括对陈旧 compaction 配置键给出精确具名诊断。固定源核心 compaction 套件通过 80 个案例；包含 60 项测试的 Rust 包通过分组的非法值与终止状态矩阵，以及真实 Session、token meter、AgentEvent、中间件、剪枝、事务、重试、取消和 dispose 路径，覆盖相同分区。
