...
`seekdeep-llm-pi-ai` 将已配置提供方的 `getModels(provider)` 返回的已安装条目映射为提供方无关的目录。其现有请求时目录查询仍是权威依据，未知模型仍以 `UNKNOWN_MODEL` 失败。`seekdeep-llm-deepseek` 接受包含展示条目的可选 `models` 配置，默认包含名为 `DeepSeek-V4-Flash` 的 `deepseek-v4-flash` 和名为 `DeepSeek-V4-Pro` 的 `deepseek-v4-pro`。显式列表会替换这些默认值，空列表则关闭发现。这些条目改善已知公开或私有模型的选择体验，而所有未列出的模型 ID 仍会原样透传。
...
每个由 ACP 创建的 agent 都会通过同一套 agent 作用域提示词／请求耦合安装这组固定部署目标，并在发布前把 Session cwd 安装为作用域提示词变量。编译后的 ACP 应用会在 app 插件加载期间准备 stdio bridge，但只在完整 Loader tree 稳定后开始接收 frame，因此快速客户端无法在依赖的提示词与 approval 插件激活前创建 Session。程序化 `apply_with_runtime()` 调用方仍保留立即启动约定，因为其组合已经显式完成。

### 提示词／请求一致性与持久化

`installModelSelection`（位于 `seekdeep-agent`）为前端拥有的选择安装 agent 作用域的 `system-prompt/assemble` 与 `agent/request` 监听器。提示词组装在每个步骤对所选组合做一次快照，在下游提示词监听器之后覆写组装出的 `provider` 与 `model` 变量；请求监听器在下游请求监听器之后应用同一快照。因此，发生在异步组装期间的选择会从下一个步骤生效，而不会让提示词文本与路由分裂。其他调用配置字段保持不变。
...
单元测试覆盖目录值副本与格式错误的元数据、pi-ai 和 DeepSeek 目录投影、提供方／模型请求路由，以及提示词变量对齐；监听器安装在 agent 作用域的上下文中，因此能够实现 agent 间隔离。ACP 传输测试独立验证固定提供方／模型的转发行为；编译应用快照会在完整 Loader 稳定后同时驱动协议握手与持久化的回放模型轮次。TUI 套件覆盖选择器交互与基于请求头的恢复。
