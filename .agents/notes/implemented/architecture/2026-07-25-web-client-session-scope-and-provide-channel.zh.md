...
运行时 `agents/scope.ts` 与 host `seekdeep-scope` 机制层一致（fiber + tag + filter 过滤；不 value-import：host 包携带 scoped-events 的 `Events` merge，进 client program 撞 Context merge）：
...
| React Context 层层传会话语境 | 插件在 host/client 两侧应是一个心智模型；scope 机制与 host seekdeep-scope 同构 |
