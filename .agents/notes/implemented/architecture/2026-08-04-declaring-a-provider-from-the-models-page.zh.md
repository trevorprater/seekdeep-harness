...
下面两层已经让 pi-ai 路由变成[一份声明](2026-08-03-pi-ai-declared-provider-catalog.md)，并给了 host [询问草稿端点](2026-08-04-draft-provider-endpoint-interrogation.md)的能力。但两者都没有抵达不编辑 YAML 的人：Models 页仍然只为每个提供方提供一个 API 密钥输入框和一个装着 API 地址的折叠区，因此接入一个网关意味着打开 `$SEEKDEEP_HOME/settings.yaml` 并知道 profile 的形状，更正一个陈旧的上下文窗口也是如此。能力已经存在，界面却没有暴露它。
...
Rust/WASM schema-form 绑定暴露重建后节点的结构子节点、元数据与精确常量值，包括 null。直接访问子节点与路径查找共享节点身份；关系数组和元数据保持稳定的对象身份。浏览器测试使用真实重建器，因为脚本化的协议选项辅助函数无法发现 schema 属性缺失导致组装后的页面禁用提供方创建的问题。
