...
The [Python runtime binding ABI](2026-09-04-rust-python-runtime-binding-abi.md) supplies Rust-backed carrier lookup, client classes, shared mutable observations, and target-specific native libraries. Its installed-wheel comparisons cover default SDK launch, custom text/code/workflow turns, minimal and advanced scenarios, and direct runtime checks on each advertised platform. These checks do not establish complete distribution parity: abstract service constructor compatibility and clean-checkout CI assembly remain separate gaps. The standalone smoke command and workflow invocation paths need their own native implementation; a differential runner using the external oracle does not supply that CI entry.
