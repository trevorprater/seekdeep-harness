...
`EffectHandle` changes its bookkeeping state synchronously and releases that lock before awaiting a disposer. An uncontended synchronous disposer therefore completes without an executor handoff, including inside a service-change callback after Tokio's cooperative budget is exhausted. Async disposers and concurrent callers still await the owning cleanup and receive its recorded outcome. Cordis tests pin immediate synchronous completion, joined failure replay, and reusable root generations.
