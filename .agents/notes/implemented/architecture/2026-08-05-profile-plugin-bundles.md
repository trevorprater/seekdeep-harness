...
The `seekdeep` launcher hardcoded its compositions: `base.cordis.yml` + `web.cordis.yml` shipped inside `apps/cli`, three bespoke entry modes (`--config`, `web`, `-p`) each with its own layer stack, and a single global personal overlay (`$SEEKDEEP_HOME/config.yaml`). There was no way to install an out-of-tree plugin (a TUI, a provider pack) into a shipped surface without editing the repository, and no place where a third-party package could contribute a default composition.

## Decision

Everything becomes a **profile**: a directory `$SEEKDEEP_HOME/profiles/<name>` with a `package.json` (pnpm-managed out-of-tree plugin `dependencies` plus the profile manifest `seekdeep.profile` with its ordered `bundles` layer list) and a user `cordis.patch.yml`. A **bundle** is an npm package declaring `"seekdeep": { "bundle": { "patch": "./cordis.patch.yml" } }`; the two manifest kinds live under distinct `seekdeep.profile` / `seekdeep.bundle` keys so a package.json states which role it plays. The tree composes over an empty root by applying each bundle's patch in `seekdeep.profile.bundles` order, then the user layer and `--patch` overlays — one `applyEntryPatches` call shared by boot and `--dump-config`. App invocation values later moved from launcher-derived patches to startup services in the [app-owned command-line decision](2026-08-06-app-owned-command-line.md).

The shipped bundles are `@seekdeep-ai/seekdeep-base` (shared core rows), `@seekdeep-ai/seekdeep-web-app` (browser Host rows and Web runtime glue), and `@seekdeep-ai/seekdeep-headless` (a direct one-shot runner over base, without web-app). Generic `seekdeep --profile <name>` hands its remaining arguments to that profile's command-line startup row: Web owns its flag family, while headless owns its task positional. Patch overlays use launcher-owned `--patch`. `seekdeep plugin --profile <name> <args...>` is a thin pnpm forwarder that initializes the profile and reconciles `seekdeep.profile.bundles` with installed bundle declarations; a package without a bundle declaration remains a plain dependency. [Headless as a direct core entry point](2026-08-09-headless-direct-core-entry-point.md) owns the headless composition contract.

Resolution is two-anchored by construction: `seekdeep.profile.bundles` names resolve from the seekdeep installation first, then the profile directory — so in-box bundles always come from the same installation as the running `seekdeep` and pnpm never manages them — while bare plugin names in patch rows resolve through the profile directory's Node parent-walk into the maintained flat fallback `$SEEKDEEP_HOME/profiles/node_modules` (one symlink per package the installation's app and bundles depend on, healed on every launch).

Two supporting refactors: the webserver's built-in static dist serving became the single-owner **fallback seat** (`registerFallback`/`applyIndexTaps`), with the SPA server extracted to `@seekdeep-ai/seekdeep-host-frontend-static` so the web bundle owns its dist as composition, not launcher code; and the personal-overlay machinery of the [seekdeep CLI personal-config decision](../feature/2026-07-20-seekdeep-cli-personal-config.md) (`loadPersonalPatches`, `$SEEKDEEP_HOME/config.yaml`) was retargeted to the per-profile and home-level `cordis.patch.yml` layers (`loadOptionalPatches`, `watchUserPatches` taking a filename), superseding that note's entry modes and file location while keeping its Harness-home root, patch semantics, and fail-loud parsing.

## Alternatives considered

- **Dependency-scan plus partial `patchOrder`** (the original sketch): scanning `dependencies` for bundles and ordering unlisted ones alphabetically has two sources of truth and an implicit tie-break; one explicit ordered `seekdeep.profile.bundles` list is smaller and fully deterministic. A raw `pnpm add` inside the profile installs a library without activating any patch — explicit, no spooky scan.
...
- **Transitive bundle auto-application**: only direct `seekdeep.profile.bundles` entries contribute layers; a meta-bundle wanting to re-export another bundle's patch must do so explicitly in its own patch file.
...
- Backends reject nothing old on disk (pre-release stance): `$SEEKDEEP_HOME/config.yaml` is simply no longer read.
