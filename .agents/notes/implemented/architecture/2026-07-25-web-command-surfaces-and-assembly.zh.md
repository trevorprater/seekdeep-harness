...
- 目录按 `SessionId` 分区，per-key single-flight + epoch guard（旧拉取永不覆盖新态）；`commands/change` 软刷新全部已触达 key，`agent-preset/selected` 软刷新被重组的会话，`connection/reset` 丢弃并重热全部 key。软刷新期间 ready 快照继续服务，Enter 必须等待当前 key 就绪，失败留草稿不降级。source 的 `warm` 钩子在各会话 scope 出生时预热。
...
- Rust 实现把目录、排序、popup 与 wire 语义保留在目标可移植 crate 中。浏览器 service、Cordis effect 注册、Slot 装配、React 树与 `AbortController` 适配器编译为 WebAssembly；popup 业务回调收到 open 时捕获的同一个 JavaScript 会话投影。`seekdeep-commands` 从 `seekdeep-commands-contract` 重新导出 descriptor，使浏览器构建不链接原生 Agent／命令运行时，并让每个优化后的 Client 包只导出自有浏览器入口。
...
- popupSelect 壳由模型选择与权限预设共用。逐项 Inbox 操作、富结果卡与 roster 可配置性仍是独立工作。
