...
持久化是一个具有抽象 Service Definition 的**能力 seam**（[能力 seam](2026-06-13-capability-seams.md)，`seekdeep-shell` 模板），而非循环或核心逻辑：

1. **接口**（`seekdeep-session-persistence`，`ctx.sessionPersistence`）：一个抽象的 `SessionPersistence` 服务，提供 `locate`/`create`/`append`/`prepare`/`load`/`inspect`/`readFrom`/`list`/`listSnapshots`。其持久化单元就是现有的 `SessionEvent`（`{ type, seq, time, data }`），原样复用，无转换类型。
2. **实现**（`seekdeep-session-persistence-jsonl`）：每个会话一个仅追加的逻辑 JSONL 日志：先是一行 `SessionHeader`，随后是无损表示连续 `SessionEvent` 流的存储记录。符合条件的 `assistant/chunk` 增量连续段默认使用打包行；[带校验和的 Zstandard 帧](2026-07-19-zstandard-jsonl-session-logs.md)是默认物理编码，也可通过配置使用原始行。
...
**在存储层无条件应用执行不变式。** 存储保留 schema、废弃格式、序号与已提交字节的检查；循环平衡审计归可选的核心不变式所有。源实现的模型默认值场景将独立的 `request/header` 持久化为已记录的选择事实。JSONL 与 SQLite 保留该事件，不为它虚构轮次。原始与压缩 JSONL 的追加、原始导出、检查及冷加载测试固定这一区别；应用核心不变式时，轮次外执行仍会被拒绝。
