...
Ordinary pipes do not cover one requirement. A persistent terminal needs PTY allocation, foreground-process-group inspection and signalling, and cleanup of the complete terminal session. Pretending those operations can be rebuilt in `seekdeep-terminal-bash` from an ordinary `spawn()` handle would either leak provider internals or weaken its lifecycle contract.
...
- `seekdeep-bash-local` continues to map Bash semantics onto ordinary `ctx.subprocess.spawn()`.
- `seekdeep-lsp-stdio` reads and contains source through `ctx.fs`, resolves and launches language servers through `ctx.subprocess`, and carries provider-owned file URIs through initialization and result rendering. One provider-lifetime signal aborts filesystem and protocol work during disposal, including workspace lookup before queue ownership; its JSON-RPC, pooling, synchronization, and normalization stay unchanged.
- `seekdeep-terminal-bash` maps persistent-shell semantics onto `ctx.subprocess.spawnTerminal()`. The local `node-pty` and process-inspection implementation moves into `seekdeep-subprocess-local`; another subprocess provider supplies the same primitive. `danger-full-access` needs no `ctx.sandbox`; a confined mode requires a same-world sandbox provider and fails before spawn when none is mounted. Prompt and silence evidence collected during asynchronous pre-write inspection is discarded when the provider write begins. Cancellation retains the send reservation while an in-flight write settles and then signals the foreground group, so late bytes or the signal cannot target a successor; an in-flight readiness poll cannot release that reservation, and a rejected write sends no signal. The absolute deadline remains armed throughout cancellation. A signal failure becomes terminal transport failure. Completion of a stale inspection resumes polling for the current send. Startup cancellation begins terminal rollback without waiting for a stalled readiness or signalling call. Close rejects new public signals and delegates provider-observable session quiescence to the handle's awaited termination operation.

## E2B POC boundary

The opt-in E2B realization has exactly three provider-specific packages under `packages/e2b/`: `seekdeep-e2b` creates one sandbox and deletes it on timeout or disposal, `seekdeep-fs-e2b` implements `ctx.fs`, and `seekdeep-subprocess-e2b` implements `ctx.subprocess` over E2B Commands, PTYs, and remote Linux process groups. The two adapters obtain the sole SDK handle from the owner and never create private sandboxes.

E2B owns the mutable filesystem, managed command and Bash processes, terminal allocation and terminal-session groups, language-server processes and source reads, and adapter-private files under `.seekdeep-e2b`. The host owns Cordis and plugin objects, the agent loop, agent/session/goal state, session logs and persistence, LLM calls, prompts and tools, authority, skills, subagent orchestration, PTY buffers and readiness, LSP protocol state, and E2B SDK/network buffers. The overlay neither uploads nor synchronizes the host workspace.

The Rust `seekdeep-e2b` owner starts one creation through an injected object-safe SDK factory, shares the resulting handle across callers, establishes and protects the remote runtime root before readiness, fences handle acquisition against disposal, and makes one rollback kill attempt after setup failure. Timeout deletion and an already-absent sandbox count as quiescence. Control shells receive process-monotonic isolated HOME values rather than ambient-random paths.

The Rust `seekdeep-fs-e2b` backend consumes one object-safe sandbox handle supplied by the E2B owner. Canonicalization, metadata, streams, staging, rename, and guarded-link publication stay remote; the host retains only decoded chunks, opaque versions, per-target mutation locks, and a process-monotonic staging/version source with no ambient randomness. Stream abandonment invokes the remote cancellation hook, and cancellation stops at the atomic rename or link commit boundary so a committed mutation remains success.

The Rust `seekdeep-subprocess-e2b` backend consumes the same handle through object-safe Commands and PTY interfaces. Ordinary commands publish a validated remote process-group id after asynchronous allocation, expose closable Tokio pipes that release SDK callback backpressure, keep `wait_for_exit()` provider failures observable, and make TERM-to-KILL cleanup retryable until a liveness probe proves quiescence. Terminal allocation owns private mode-`0600` state through its process-monotonic output boundary, then the published handle owns foreground inspection, signalling, in-flight operation settlement, and complete remote-session cleanup. Runtime disposal aborts and joins unpublished terminal setup before draining every retained command group and terminal session.

The adapters retain only substrate mechanics. Filesystem canonicalization crosses the SDK's decoded command transport as strict base64-encoded NUL framing; streamed reads leave byte ceilings with consumers. The Rust `seekdeep-subprocess-e2b` substrate uses strict ASCII/base64 environment and output frames where SDK callback chunking would otherwise lose bytes, retains byte-exact offset-addressed tails, advertises only complete in-cap remote spills, gives control shells isolated process-monotonic HOME values, and centralizes cancellable poll ticks plus tolerant group signalling. Process and terminal owners compose those mechanics into remote-group cleanup that proves quiescence before settlement.
...
**Move PTY readiness and session policy into the subprocess service.** Rejected because those are persistent-terminal consumer semantics, not OS process mechanics. A subprocess provider owns what only its substrate can do; `seekdeep-terminal-bash` owns what a Harness terminal means.
