...
The repository also used `SDK` in two meanings. The supported Python and TypeScript clients use the JSON-RPC SDK protocol. The project as a whole is SeekDeep Harness, not an SDK project. The removed SDK project toolchain made the broad meaning obsolete, but prose and names preserved parts of it.
...
`SDK` means the JSON-RPC-based client/server protocol used by the supported Python and TypeScript SDKs. The repository keeps `@seekdeep-ai/seekdeep-sdk-client`, `@seekdeep-ai/seekdeep-sdk-protocol`, and the wire identity `seekdeep-harness-sdk-runtime`; the JSON-RPC server belongs to the same family. SeekDeep Harness itself is not an SDK, and the removed project generator, launcher, helper, and launcher telemetry packages stay absent.
...
Do not remove an intentional vendor qualifier to avoid repetition. `seekdeep-subagent-seekdeep-sdk` names the SeekDeep Harness SDK provider and avoids confusion with another SDK. Its private class becomes `SdkSubagentProvider` because the class also needs to say what it provides.
...
|---|---|---|
| `@seekdeep-ai/seekdeep-jsonrpc` | `@seekdeep-ai/seekdeep-sdk-jsonrpc-server` | It is the server half of the SDK protocol. `jsonrpc` alone names an encoding; `sdk-jsonrpc-server` gives the family, mechanism, and role. |
| `HarnessSdkServer` | `HarnessSdkJsonRpcServer` | The class is one JSON-RPC server implementation, not every possible SDK server. |

Keep `@seekdeep-ai/seekdeep-sdk-client`, `@seekdeep-ai/seekdeep-sdk-protocol`, and `seekdeep-harness-sdk-runtime`. Exclude `@seekdeep-ai/create-sdk`, `@seekdeep-ai/seekdeep-scripts`, `@seekdeep-ai/seekdeep-helper`, and `@seekdeep-ai/seekdeep-telemetry`; the separate removal decision deletes them and their support graph.
...
| `@seekdeep-ai/seekdeep-bash`, `ctx.bash` | `@seekdeep-ai/seekdeep-shell`, `ctx.shell` | PowerShell already implements this seam. The capability is shell execution, not Bash. |
...
| `@seekdeep-ai/seekdeep-bash-env`, `ctx.bashEnv`, `BashEnvRegistry` | `@seekdeep-ai/seekdeep-shell-env`, `ctx.shellEnv`, `ShellEnvRegistry` | The environment registry is shared by Bash and PowerShell tools. |
...
| `@seekdeep-ai/seekdeep-pty`, `ctx.pty`, `PtyService` | `@seekdeep-ai/seekdeep-terminal`, `ctx.terminals`, `TerminalSessionService` | Callers manage multiple named terminal sessions. They do not allocate raw PTYs through this service. |
...
| `@seekdeep-ai/seekdeep-pty-local`, `LocalPtyBackend` | `@seekdeep-ai/seekdeep-terminal-bash`, `BashTerminalBackend` | The provider depends on Bash prompt and shell behavior. `local` hides the actual dialect. |
| `@seekdeep-ai/seekdeep-tool-pty` | `@seekdeep-ai/seekdeep-tool-terminal` | The model-facing tools are already `terminal_*`; the package should use the same product noun. |
...
| Former | Current | Reason |
|---|---|---|
| `@seekdeep-ai/seekdeep-lsp-local` | `@seekdeep-ai/seekdeep-lsp-stdio` | The provider speaks LSP over stdio through replaceable filesystem and subprocess services. It is not necessarily local. |
...
| `@seekdeep-ai/seekdeep-tasks`, `ctx.tasks`, `TaskService` | `@seekdeep-ai/seekdeep-jobs`, `ctx.jobs`, `JobRegistry` | The service registers, owns, observes, waits for, and cancels multiple background jobs. It is a registry, not a general task service. |
...
| `@seekdeep-ai/seekdeep-tasks-local`, `LocalTaskService` | `@seekdeep-ai/seekdeep-jobs-local`, `LocalJobRegistry` | This is the process-local provider of the job registry. Here `local` is meaningful because the jobs and callbacks live in one process. |
| `@seekdeep-ai/seekdeep-tool-tasks` | `@seekdeep-ai/seekdeep-tool-jobs` | The consumer controls the job registry and should use the same domain noun. |
...
| `@seekdeep-ai/seekdeep-client-ui-task`, `client/ui-task/` | `@seekdeep-ai/seekdeep-client-ui-jobs`, `client/ui-jobs/` | The client package presents the background-job collection. It is not one user task. |
...
| Former | Current | Reason |
|---|---|---|
| `@seekdeep-ai/seekdeep-client-ui-slash`, `ui-slash/` | `@seekdeep-ai/seekdeep-client-ui-input-trigger`, `ui-input-trigger/` | The client handles `/`, `@`, keyboard arbitration, candidate menus, and programmatic launch. It is not only slash commands. |
...
| `@seekdeep-ai/seekdeep-agent-tool-mode`, plugin `tool-mode` | `@seekdeep-ai/seekdeep-agent-tool-presentation`, plugin `tool-presentation` | The plugin changes how tools are presented to the model. It does not change execution behavior. Keep local `Config.mode` and `ToolPresentationMode`. |
...
| `@seekdeep-ai/seekdeep-permission`, `ctx.permission`, `PermissionService` | `@seekdeep-ai/seekdeep-permission-presets`, `ctx.permissionPresets`, `PermissionPresetService` | The service selects and persists presets. Sandbox and approval services enforce the result. |
| `@seekdeep-ai/seekdeep-client-ui-permission` | `@seekdeep-ai/seekdeep-client-ui-permission-presets` | The UI edits and selects permission presets. |
...
| `@seekdeep-ai/seekdeep-user-interaction`, `user-interaction/` | `@seekdeep-ai/seekdeep-user-questions`, `user-questions/` | The seam supports question batches and answers only. Approval, commands, and directory picking are separate interaction seams. |
| `ctx.userInteraction`, `UserInteractionService`, `UserInteractionProvider`, `UserInteractionError` | `ctx.userQuestions`, `UserQuestionService`, `UserQuestionProvider`, `UserQuestionError` | These names state the one supported interaction form. Keep `AskUserQuestion*`, the `ask_user_question` tool, and `@seekdeep-ai/seekdeep-tool-ask-user`. |
...
| Former | Current | Reason |
|---|---|---|
| `packages/typert/type-meta/`, `@seekdeep-ai/seekdeep-type-meta` | `typert/protocol/`, `@seekdeep-ai/seekdeep-typert-protocol` | The package owns the Typert Remote protocol, decorators, bindings, codecs, lookups, and context contracts. It is not generic type metadata. |
...
Keep `@seekdeep-ai/seekdeep-tools` and `ctx.tools`. Keep `@seekdeep-ai/seekdeep-api-gateway`, its `gateway/` folder, `ctx.typertGateway`, and `TypertGatewayService`; that service is a real API gateway. Its internal `TypeRT*` identifiers still follow the `Typert*` spelling rule.

### Workspace instructions, telemetry, identity, and launch environment

| Former | Current | Reason |
|---|---|---|
| Host `ctx.workspace` | Host `ctx.workspaceRegistry` | `WorkspaceRegistry` owns multiple workspaces, but Client `ctx.workspaces` already has an incompatible type. Both declarations merge into the same Cordis `Context` interface at compile time even though their runtime contexts are separate. The role suffix states the host service and avoids that collision. Keep `@seekdeep-ai/seekdeep-workspace`, `WorkspaceRegistry`, `Workspace`, and `workspace.*` wire names. |
| `@seekdeep-ai/seekdeep-workspace-context`, `context/workspace-context/` | `@seekdeep-ai/seekdeep-agent-instructions`, `context/agent-instructions/` | The package loads hierarchical `AGENTS.md` and `CLAUDE.md` files for the agent. It is not general workspace context. |
...
| `TelemetryOtel`, `TelemetryMode`, plugin `telemetry-otel` | `OpenTelemetrySessionBackend`, `SessionTelemetryMode`, plugin `session-telemetry-otel` | The provider name states both the OpenTelemetry mechanism and session scope. Keep the package names `seekdeep-session-telemetry` and `seekdeep-session-telemetry-otel`. |
...
| `session/user-id/`, `@seekdeep-ai/seekdeep-user-id` | `identity/anonymous-user-id/`, `@seekdeep-ai/seekdeep-anonymous-user-id` | The value is a random correlation id shared by telemetry, feedback, and DeepSeek requests. It is neither a Session concern nor an authenticated user identity. |
...
| `util/environment/`, `@seekdeep-ai/seekdeep-environment` | `util/launch-environment/`, `@seekdeep-ai/seekdeep-launch-environment` | The package captures one immutable layered snapshot at launch. It is not a general environment API. |
| Public `Environment*`, `createEnvironmentSnapshot`, `environmentOf`, `SEEKDEEP_ENVIRONMENT_KEY` | `LaunchEnvironment*`, `createLaunchEnvironmentSnapshot`, `launchEnvironmentOf`, `SEEKDEEP_LAUNCH_ENVIRONMENT_KEY` | The names state the snapshot's lifetime and purpose. |
...
| Former | Current | Reason |
|---|---|---|
| `@seekdeep-ai/seekdeep-tool-schedule`, `schedule/tool-schedule/`, plugin `tool-schedule` | `@seekdeep-ai/seekdeep-schedule`, `schedule/schedule/`, plugin `schedule` | The package owns the durable Schedule domain, persistence barriers, management tools, timers, follow-ups, and runtime lifecycle. `tool-` describes only one part. |
...
| `WorkflowService`, `ctx.workflows` | `WorkflowEngine`, `ctx.workflowEngine` | One engine parses and executes workflow programs. The plural key wrongly suggests a registry. Keep `@seekdeep-ai/seekdeep-workflow` and workflow events and tools. |
| `@seekdeep-ai/seekdeep-workflow-workerthread`, `WorkerWorkflowEngine` | `@seekdeep-ai/seekdeep-workflow-worker-thread`, `WorkerThreadWorkflowEngine` | `worker thread` is the precise Node mechanism and the repository spelling uses the full words. |
| `@seekdeep-ai/seekdeep-goal-session`, `goal/goal-session/` | `@seekdeep-ai/seekdeep-goal-round-driver`, `goal/goal-round-driver/` | The plugin drives same-session Goal Rounds. It neither stores goals nor defines sessions. Keep `GoalService`, goal source, events, and contracts. |
| `packages/compact/` | `packages/compaction/` | The group is a noun-domain family. `compact` remains the user command verb. |
| `@seekdeep-ai/seekdeep-compact`, `ctx.compact`, `CompactService` | `@seekdeep-ai/seekdeep-compaction`, `ctx.compaction`, `CompactionEngine` | The object runs the compaction algorithm and lifecycle. It is an engine, not a generic service. |
...
| `@seekdeep-ai/seekdeep-compact-basic`, `BasicCompactService`, public `BasicCompact*` | `@seekdeep-ai/seekdeep-compaction-basic`, `BasicCompactionEngine`, corresponding `BasicCompaction*` | `basic` is plain but honest. `compaction-llm` adds no information because LLM use is already part of the current implementation family. |
| `@seekdeep-ai/seekdeep-compact-tool-result-prune`, `ToolResultPruneService`, `ctx.toolResultPrune` | `@seekdeep-ai/seekdeep-compaction-tool-result-pruner`, `ToolResultPruner`, `ctx.toolResultPruner` | The plugin is an actor that prunes tool results. The noun `pruner` names that role. |
...
| `@seekdeep-ai/seekdeep-settings-local`, `SettingsLocal` | `@seekdeep-ai/seekdeep-settings-file`, `FileSettingsProvider` | The implementation is file-backed through the filesystem seam. `file` states the mechanism; `local` does not. |
...
| Former | Current | Reason |
|---|---|---|
| `HttpServerService`, `ctx.httpServer` | `WebServer`, `ctx.webServer` | The server owns HTTP routes and WebSocket upgrade routes. `Web` leaves room for both; `Http` is too narrow here. Keep `packages/host/webserver`, `@seekdeep-ai/seekdeep-host-webserver`, `WebRoute`, and `WebUpgradeRoute`. |
...
| `@seekdeep-ai/seekdeep-session-title-first-message-llm`, cadence `first-message` | `@seekdeep-ai/seekdeep-session-title-first-prompt-llm`, cadence `first-prompt` | The trigger is the first user prompt, not any message in the session log. |
| `@seekdeep-ai/seekdeep-session-title-all-messages-llm`, cadence `all-user-messages` | `@seekdeep-ai/seekdeep-session-title-all-prompts-llm`, cadence `all-prompts` | The backend refreshes from user prompts. `all messages` wrongly includes assistant and tool events. |
| `@seekdeep-ai/seekdeep-code-runtime-worker`, `WorkerCodeRuntime` | `@seekdeep-ai/seekdeep-code-runtime-worker-thread`, `WorkerThreadCodeRuntime` | The implementation uses a Node worker thread. `worker` alone is too broad. |
...
| Former | Current | Reason |
|---|---|---|
| `@seekdeep-ai/seekdeep-fs-policy` | `@seekdeep-ai/seekdeep-fs-observation-policy` | The package defines which filesystem observations authorize later effects. It is not the complete filesystem or sandbox policy. |
...
| `@seekdeep-ai/seekdeep-skill-local`, `LocalSkillProvider`, provider id `local` | `@seekdeep-ai/seekdeep-skill-filesystem`, `FileSystemSkillProvider`, provider id `filesystem` | The provider discovers skill files through `ctx.fs`, which can be local or remote. The mechanism is filesystem access, not locality. |
...
| `@seekdeep-ai/seekdeep-subagent-spawn`, `SpawnProvider` | `@seekdeep-ai/seekdeep-subagent-spawn-in-process`, `SpawnInProcessProvider` | This provider starts a child agent in the current process. The configured provider id remains `spawn`. |
| `@seekdeep-ai/seekdeep-subagent-fork`, `ForkProvider` | `@seekdeep-ai/seekdeep-subagent-fork-in-process`, `ForkInProcessProvider` | This provider forks an agent in the current process. The configured provider id remains `fork`. |
| `@seekdeep-ai/seekdeep-subagent-inprocess`, `subagent-inprocess/` | `@seekdeep-ai/seekdeep-subagent-in-process-driver`, `subagent-in-process-driver/` | The package contains common in-process driving logic, not a third provider. |
| Private `SdkProvider` in `seekdeep-subagent-seekdeep-sdk` | `SdkSubagentProvider` | The repeated package qualifier is intentional, and the class must say that it provides subagents through the SDK. |
...
| `@seekdeep-ai/seekdeep-web-fetch-local`, `LocalFetchProvider`, `LocalFetchLimits`, provider id `local-http` | `@seekdeep-ai/seekdeep-web-fetch-http`, `HttpFetchProvider`, `HttpFetchLimits`, provider id `http` | This provider performs direct HTTP fetches. `local` says where code happens to run, not which mechanism it provides. |

Keep `@seekdeep-ai/seekdeep-subagent-seekdeep-sdk`, its provider id `seekdeep-sdk`, external ACP, Codex, and Claude Code provider families, the subagent tool package names, the main filesystem package and backends, filesystem tools and events, and the skill badge and tool packages.

### Hooks, guards, plan mode, extensions, and diagnostics

| Former | Current | Reason |
|---|---|---|
| `@seekdeep-ai/seekdeep-hooks-claude`, `ClaudeHookConfig`, `parseClaudeConfig`, dialect `claude` | `@seekdeep-ai/seekdeep-hooks-claude-code`, `ClaudeCodeHookConfig`, `parseClaudeCodeConfig`, dialect `claude-code` | The hook bridge targets Claude Code, not every Anthropic or Claude product. |
| `@seekdeep-ai/seekdeep-repeat-tool-guard`, plugin/source `repeat-tool-guard` | `@seekdeep-ai/seekdeep-repeat-tool-reminder`, plugin/source `repeat-tool-reminder` | The plugin adds a model reminder. It does not block or enforce a guard decision. |
| `@seekdeep-ai/seekdeep-timeout-policy` | `@seekdeep-ai/seekdeep-tool-call-timeout-policy` | The full `tool-call` qualifier names what the policy limits without calling the plugin a model-facing tool. Keep its `guard/timeout-policy/` directory and plugin id `timeout-policy`; the `packages/*/tool-*` catalog convention still applies only to packages that register tools. |
...
| `InvariantService` | `InvariantRegistry` | The object owns registered invariant checks. Keep `@seekdeep-ai/seekdeep-invariants` and `ctx.invariants`. |
...
| Former | Current | Reason |
|---|---|---|
| `util/paths/`, `@seekdeep-ai/seekdeep-paths` | `util/home-paths/`, `@seekdeep-ai/seekdeep-home-paths` | The helpers resolve paths under the Harness home. They are not a general path library. Keep the individual function names when they already state the returned path. |
| `util/retention/`, `@seekdeep-ai/seekdeep-retention` | `util/output-retention/`, `@seekdeep-ai/seekdeep-output-retention` | The policy retains command and tool output. It is not a general data-retention framework. |
| `E2BSandboxService` | `E2BRuntime` | The class creates, reuses, and disposes the E2B execution environment used by filesystem and subprocess adapters. It is broader than one sandbox handle and narrower than a generic owner. Keep `@seekdeep-ai/seekdeep-e2b`, `ctx.e2b`, and the `e2b/` group. |
| `@seekdeep-ai/seekdeep-frontend-static` | `@seekdeep-ai/seekdeep-host-frontend-static` | The package is the Host plugin that serves the frontend assets. The prefix distinguishes it from frontend application code. |
...
| `@seekdeep-ai/seekdeep-jsonrpc-demo` | `@seekdeep-ai/seekdeep-sdk-jsonrpc-demo` | The example demonstrates the runtime SDK over JSON-RPC. It belongs to the one SDK meaning. |
| `@seekdeep-ai/seekdeep-frontend` | `@seekdeep-ai/seekdeep-web-frontend` | The application is the web frontend. Keep its physical `apps/web/` folder. |

Keep atomic-write, brand, native-command, timeout utility, directory-picker, `seekdeep-base`, `seekdeep-web-app`, app boot, CLI names, and the `headless` package, bundle, and example identity. `headless` is the intended product essence and may later support more than one-shot execution.
...
| `@seekdeep-ai/seekdeep-client-ui-model` | `@seekdeep-ai/seekdeep-client-ui-model-selection` | The package controls the model selection for a session. The singular `model` name is too broad. |
...
| `@seekdeep-ai/seekdeep-client-ui-models` | `@seekdeep-ai/seekdeep-client-ui-settings-models` | This package owns the Models settings panel. Keep `ModelsSettingsStore`; it holds one settings view model with data operations and subscriptions and is a real store. |
| `@seekdeep-ai/seekdeep-client-ui-plugin-config`, `client/ui-plugin-config/` | `@seekdeep-ai/seekdeep-client-ui-settings-plugins`, `client/ui-settings-plugins/` | This package owns the Plugins settings section, not a general plugin-configuration system. The target joins the `ui-settings-*` family and uses the section's plural product name. |
...
| `@seekdeep-ai/seekdeep-client-ui-plugins`, `client/ui-plugins/`, Loader id `ui-plugins`, `client-ui-plugins-invariant` | `@seekdeep-ai/seekdeep-client-ui-settings-plugin-inventory`, `client/ui-settings-plugin-inventory/`, Loader id `ui-settings-plugin-inventory`, `client-ui-settings-plugin-inventory-invariant` | This later package owns the read-only Plugin Inventory tab in the Plugins settings section. `ui-plugins` is too broad and does not distinguish the inventory from editable plugin settings. |
...
| `@seekdeep-ai/seekdeep-client-ui-feedback`, `client/ui-feedback/`, Loader id `ui-feedback`, `client-ui-feedback-invariant` | `@seekdeep-ai/seekdeep-client-ui-message-feedback`, `client/ui-message-feedback/`, Loader id `ui-message-feedback`, `client-ui-message-feedback-invariant` | This package presents ratings and notes for assistant messages through the `messageFeedback` Remote. The old name also appears to cover command feedback and any later feedback UI. It does not. |
...
| `@seekdeep-ai/seekdeep-client-ui-question` | `@seekdeep-ai/seekdeep-client-ui-user-questions` | The UI presents the user-question seam, not an arbitrary question domain. |
| `@seekdeep-ai/seekdeep-client-ui-command`, `ui-command/` | `@seekdeep-ai/seekdeep-client-ui-commands`, `ui-commands/` | The package presents and runs a collection of commands. |
| `@seekdeep-ai/seekdeep-client-ui-directory-picker`, `client/ui-directory-picker/`, Loader id `ui-directory-picker`, `client-ui-directory-picker-invariant` | `@seekdeep-ai/seekdeep-client-ui-directory-picker-browse`, `client/ui-directory-picker-browse/`, Loader id `ui-directory-picker-browse`, `client-ui-directory-picker-browse-invariant` | The Client packages now contain separate `browse` and `native` directory-picker presentations. The unqualified package is the browse implementation, not their shared definition. The target matches the Host backend family and changes no boundary. |
...
- Keep `@seekdeep-ai/seekdeep-api-gateway`, `ctx.typertGateway`, and `TypertGatewayService`.
- Keep session projection names. A projection is not only a reducer function.
- Keep `@seekdeep-ai/seekdeep-session-stats`, `sessionStats`, and `SessionStatsProjection`. They accurately name whole-session statistics and the maintained read model that carries them.
...
- Keep `seekdeep-subagent-seekdeep-sdk` and provider id `seekdeep-sdk`; the repeated qualifier prevents ambiguity.
...
- Keep `@seekdeep-ai/seekdeep-client-ui-directory-picker-native`; its suffix names the native-chooser presentation beside the renamed `-browse` variant. Keep `SURFACE_PACKAGES`; within the directory-picker auto selector it is the package map for the Client presentation half, contrasted with `BACKEND_PACKAGES`.
- Keep `@seekdeep-ai/seekdeep-host-plugin-inventory`, `ctx.pluginInventory`, the `pluginInventory/list` Remote, and the `PluginInventory*` payload types. They accurately name the Host-owned read-only inventory; only the adapter class and the overly broad Client presentation names change.
...
**Rename `seekdeep-compact-basic` to `seekdeep-compaction-llm`.** Rejected. `LLM` adds no distinction in the current backend family. `basic` is less ambitious and does not claim an algorithm that does not exist.
...
- The paired package-creation guide contains the role-word contract, `packages/AGENTS.md` links to it, the terminology table records the chosen words and `Typert` spelling, and root project prose calls the product SeekDeep Harness rather than SeekDeep Harness SDK.
