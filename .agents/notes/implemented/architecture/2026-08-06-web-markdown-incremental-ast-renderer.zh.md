...
- **语法**（[markdown.rs](../../../../crates/client-ui-primitives/src/markdown.rs)，以源码 [`parse.ts`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/client/ui-primitives/src/markdown/parse.ts) 为 oracle）：编译后的 Rust `parse_gfm`（流式臂与 `extractMarkdownPlainText`）和 `parse_gfm_with_math`（定稿臂）保留源码语法、兼容定界符、CJK attention 例外与位置模型，因此各处块边界完全一致。
- **增量解析**（[markdown.rs](../../../../crates/client-ui-primitives/src/markdown.rs)，对应源码 [`incremental.ts`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/client/ui-primitives/src/markdown/incremental.ts)）：CommonMark 块解析按行推进，追加文本只会重塑解析前沿。Rust `IncrementalMarkdownParser` 保留末尾两个块不稳定（最后一块是前沿；倒数第二块是安全裕量），冻结其前的所有块，只从最后一个冻结块的 `position.end.offset` 起重新解析源尾部——用的是解析器自己的偏移量，没有任何自制源扫描。每个源区间在整个流中解析 O(1) 次而非每 chunk 一次；单个巨型块（未闭合围栏）退化为旧的全量重解析成本，不会更差。非追加输入在递增的 generation 下重置状态。
- **渲染**（[browser_markdown.rs](../../../../crates/client-ui-primitives/src/browser_markdown.rs)、[browser_markdown_render.rs](../../../../crates/client-ui-primitives/src/browser_markdown_render.rs)，以源码 [`render.tsx`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/client/ui-primitives/src/markdown/render.tsx) 与 [`katex.tsx`](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/client/ui-primitives/src/markdown/katex.tsx) 为 oracle）：一个穷尽式 Rust mdast 节点映射逐字节复刻被替换管线的 DOM——表格对齐渲染为 `text-align` 样式、紧凑列表段落解包、任务列表类名与复选框空格、脚注区（其页内锚点本就被协议白名单降为纯文本）、字面 raw HTML、会与字面 HTML 文本相邻显形的分隔换行，以及 rehype-katex 的三臂容错链；KaTeX HTML 经浏览器自带的 `DOMParser` 映射为 React（无包裹元素，首／末子元素的 margin 规则仍能作用于 `.katex-display`；React 18 会把 `.katex-mathml` 子树放进 HTML 命名空间，与被替换管线完全一致——既有限制，不在本对等性约定范围内，对承担视觉渲染的 `.katex-html` 臂不可见）。ESM 适配器只提供外部 `renderToString` 与 URI 归一化调用。冻结块缓存其 React 元素并保持源偏移 key，跨过冻结边界时走 reconcile 而非重挂载；`MarkdownText` 已 memo 化。

DOM 由 `tests/fixtures/markdown-dom` 钉死：fixture 录制自替换前的 react-markdown 实现；编译后的渲染器在只归一化 CSS Module hash 与 server／client style 序列化后复现全部 46 个 settled／streaming 用例。fixture 差异即用户可见的 markdown 样式变更，必须按此评审，绝不能为重构而重录。挂载后的 release-WASM 检查保留等价性性质——在每个追加前缀处（含逐个 UTF-16 code unit 的分块），常驻组件的 DOM 都等于全新挂载——外加冻结边界 DOM 节点同一性、label 身份重置、分歧文本重置及 streaming→settled KaTeX 行为。
