...
- Persistence stays a plugin concern; the in-memory store ships in seekdeep-session.
...
- Derivation cost grows with log length — compaction (seekdeep-compaction) is the intended mitigation, not log mutation.
