...
> 范围：`seekdeep-settings-file` 的写路径数据完整性（操作链、读-改-写、跨进程写锁、diff 形态的 YAML 编辑）与 `seekdeep-settings` 的观察者生命周期（watch 的 dispose（资源释放）、异步监听器收容、JSON 形态写入边界）。本 note 推翻了[用户设置 seam note](2026-07-28-user-settings-seam.md)所记录的一项延后决定：跨进程锁文件现已交付。
...
Rust 写入器在渲染新文档或已修改的叶子节点前，将 JSON token 转为原生 YAML 值。直接通过 Serde 序列化任意精度的 `serde_json::Number` 会暴露一个私有 map，而不是数值标量。因此，测试在创建与修改两条路径上检查原始 YAML，并用 YAML 值读取器验证整数、小数及嵌套数值；仅经 `serde_json::Value` 往返会解码该私有 map，从而掩盖不兼容的文件格式。
...
[用户设置 seam note](2026-07-28-user-settings-seam.md)里「延后锁文件」那条替代方案已被本 note 取代。同类缺陷曾存在于 `seekdeep-credentials-local`（两条链共用一个 `.env`、按缓存整文件写回、持久化之后才发事件）与 `llm/adapters-updated` 扇出；[credential-boundaries note](2026-07-30-credential-boundaries-and-atomic-registration.md) 在那里套用了本模板。
