...
普通管道无法满足其中一项要求。持久终端需要分配 PTY、检查前台进程组并发送信号，以及清理完整的终端会话。如果假设可以在 `seekdeep-terminal-bash` 中基于普通 `spawn()` 句柄重建这些操作，最终不是泄漏提供方内部细节，就是削弱其生命周期约定。
...
- `seekdeep-bash-local` 继续把 Bash 语义映射到普通的 `ctx.subprocess.spawn()`。
- `seekdeep-lsp-stdio` 通过 `ctx.fs` 读取源文件并验证包含关系，通过 `ctx.subprocess` 解析和启动语言服务器，并让由提供方负责的文件 URI 贯穿初始化与结果渲染。一个提供方生命周期信号会在资源释放期间中止文件系统与协议操作，包括取得队列所有权之前的工作区查找；其 JSON-RPC、池化、同步和规范化保持不变。
- `seekdeep-terminal-bash` 把持久 shell 语义映射到 `ctx.subprocess.spawnTerminal()`。本地 `node-pty` 与进程检查实现移入 `seekdeep-subprocess-local`；其他进程管理提供方则提供相同原语。`danger-full-access` 不需要 `ctx.sandbox`；受限模式要求同一执行世界中存在沙箱提供方，未挂载时会在 spawn 前失败。提供方开始写入时，系统会丢弃异步写入前检查期间收集的提示符与静默证据。取消会在在途写入结算期间保留发送预留，随后向前台进程组发送信号，因此延迟字节和该信号都无法落到后续发送；在途就绪检查无法释放该预留，写入被拒绝时也不会发送信号。绝对截止时间会在整个取消期间保持启用。信号发送失败会成为终结性传输失败。陈旧检查完成后，会针对当前发送恢复轮询。启动取消会立即开始终端回滚，而不等待停滞的就绪检查或信号发送调用。关闭操作会拒绝新的公开信号，并把提供方可观察会话成员的完全停稳委托给句柄上须等待的终止操作。

## E2B POC 边界

可选启用的 E2B 实现在 `packages/e2b/` 下恰好只有三个提供方专用包：`seekdeep-e2b` 创建一个沙箱，并在超时或资源释放时将其删除；`seekdeep-fs-e2b` 实现 `ctx.fs`；`seekdeep-subprocess-e2b` 基于 E2B Commands、PTY 和远程 Linux 进程组实现 `ctx.subprocess`。两个适配器都从所有者取得唯一的 SDK 句柄，绝不创建私有沙箱。

E2B 负责可变文件系统、受管命令与 Bash 进程、终端分配与终端会话组、语言服务器进程与源文件读取，以及 `.seekdeep-e2b` 下的适配器私有文件。宿主负责 Cordis 与插件对象、agent loop（智能体循环）、agent 状态、会话状态与目标状态、会话日志与持久化、LLM（大语言模型）调用、提示词与工具、权限、skill（技能）、subagent 编排、PTY 缓冲区与就绪状态、LSP 协议状态，以及 E2B SDK／网络缓冲区。该叠加层既不上传，也不同步宿主工作区。

Rust `seekdeep-e2b` 所有者通过注入的对象安全 SDK factory 启动一次创建，在调用方之间共享所得句柄，在就绪前建立并保护远端运行时根目录，阻止资源释放期间的新句柄获取，并在设置失败后尝试一次回滚 kill。超时删除和沙箱已不存在都视为完全停稳。控制 shell 使用进程内单调编号的隔离 HOME，而不是依赖环境随机路径。

Rust `seekdeep-fs-e2b` 后端消费由 E2B 所有者提供的唯一对象安全沙箱句柄。规范化、元数据、stream、暂存、rename 和带防护链接发布都留在远端；宿主只保留已解码分片、不透明版本、逐目标变更锁，以及不使用环境随机性的进程内单调暂存／版本来源。消费者放弃 stream 时会调用远端取消钩子，取消则止于原子 rename 或 link 提交边界，因此已提交的变更仍报告成功。

Rust `seekdeep-subprocess-e2b` 后端通过对象安全的 Commands 与 PTY 接口消费同一句柄。普通命令在异步分配后发布经过验证的远端进程组 id，公开可关闭且能释放 SDK 回调背压的 Tokio pipe，让 `wait_for_exit()` 的提供方失败保持可观察，并使 TERM→KILL 清理可重试，直到存活探针证明完全停稳。终端分配通过进程内单调编号的输出边界拥有私有 mode-`0600` 状态，随后由已发布句柄负责前台检查、信号发送、在途操作结算和完整远端会话清理。运行时 dispose 会先中止并等待尚未发布的终端设置，再排空每个保留的命令进程组与终端会话。

适配器只保留执行基底机制。文件系统规范化以严格的 base64 加 NUL 分帧穿过 SDK 已解码的命令传输；流式读取把字节上限留给消费方执行。Rust `seekdeep-subprocess-e2b` 执行基底在 SDK 回调分片可能丢失字节之处使用严格的 ASCII/base64 环境与输出帧，保留字节精确且按偏移读取的尾部，只发布完整且未超上限的远端 spill 文件，为控制 shell 提供进程内单调编号的隔离 HOME，并集中实现可取消的轮询 tick 与容忍已消失目标的进程组信号发送。进程与终端所有者把这些机制组合为远端进程组清理，并在结算前证明完全停稳。
...
**把 PTY 就绪判断与会话策略移入进程管理服务。** 不予采纳，因为这些属于持久终端消费方的语义，而非 OS 进程机制。进程管理提供方负责只有其执行基底才能完成的操作；`seekdeep-terminal-bash` 负责 Harness 终端的语义。
