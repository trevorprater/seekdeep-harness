...
Rust `seekdeep-subprocess-e2b` 后端通过对象安全的 Commands 与 PTY 接口消费同一句柄。普通命令在异步分配后发布经过验证的远端进程组 id，让 `wait_for_exit()` 的提供方失败保持可观察，并使 TERM→KILL 清理可重试，直到存活探针证明完全停稳。终端分配通过进程内单调编号的输出边界拥有私有 mode-`0600` 状态，随后由已发布句柄负责前台检查、信号发送、在途操作结算和完整远端会话清理。运行时 dispose 会先中止并等待尚未发布的终端设置，再排空每个保留的命令进程组与终端会话。
