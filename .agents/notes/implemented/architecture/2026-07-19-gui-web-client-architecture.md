...
The Rust `seekdeep-client-runtime` object layer takes scheduling policy through injected microtask and animation-frame adapters. Its Notifier keeps freshness separate from pending delivery and rebuilds before notifying; its partial Assistant accumulator replaces only the changed `Rc` block; its Tool-call tree structurally shares unchanged recursive projections and consumes malformed cycle or over-depth edges without dropping surrounding Session data, with the source's 256-level ceiling. Its ProjectionValueStore applies the same strictly-higher-sequence rule to live frames and baselines, clears only rows not newer than an omitting baseline, truncates post-generation phantom rows, and preserves per-key face, whole-value, and aggregate-snapshot identity through a WASM facade; key and coarse subscriptions publish once per injected microtask.
...
- **Session** (session.ts): lazily built, resident — once created it keeps eating frames in the background, so switching away and back renders instantly. Operations: `prompt`/`cancel` (RPC passthrough; failures land in the snapshot's `promptError`), `open` (pull the tail history page, idempotent), `loadOlder` (upward paging, reentry-guarded), `resync` (reconnect = clear the window and rerun open). Subscription: `subscribe`/`getSnapshot` (always the cached reference) — `implements ObservableSnapshot<ConversationSnapshot>`, with `useSelector = bindSnapshotSelector(this)` attached at construction, so a Session is directly a uSES source. Frame dispatch is one switch: `session/event` frames dedup by seq (the only dedup key), buffer while open is in flight, otherwise append + incremental projection; open/stitch merges the live buffer by seq and backfills once if `subscribed.lastSeq` outruns the window tail.
  The Rust `ClientSession` core takes transport, time-zone resolution, notification scheduling, and detached-task ownership through injected seams. Calling `open()` and `prompt()` performs their source-observable loading and blank→engaging prefixes synchronously; shared open work is identity-guarded, and every history await rechecks the exact resync generation. Paging keeps one contiguous window, live gaps coalesce behind one tail repair, pending waits re-mint across resync, queue baselines clear only at `session/subscribed`, and Host-computed projections retain the same higher-seq-wins store across instantiation and reload.
  Its WASM facade adapts the generated API client and Remote namespaces without changing the object-layer protocol: concurrent opens share one JavaScript Promise, `getSnapshot()` caches the exact top-level object, unchanged Chat/pending/queue/projection readers retain their JavaScript identities, generated RPC results remain plain objects, attachment bytes remain `Uint8Array`, and pending `respond()` restores the private `rpcId` before crossing the carrier.
...
- **SessionManager** (manager.ts): instance cluster + frame entry + the session list. sessionId-bearing frames go only to existing instances (a mux broadcast must not instantiate every session); approval/question `requested` frames are the exception — they never land in history, so they buffer in `pendingBuffers` and replay on instantiation.
  The Rust `SessionManager` core keeps Session instances, projection stores, pending status, queue/request buffers, completion reminders, and job mirrors on independent lifetime axes. A list pull is single-flight and replays every in-flight mutation over its baseline without reordering established IDs; projection and job frames land before instantiation; `session/subscribed` truncates phantom projection rows and clears omitted-generation mirrors; answerable requests compact by stable key and die at generation loss; only direct user messages advance activity; and running→idle arms a reminder only while the Session is unwatched.
  Direct-child catalogs are an independent retained read model: healthy rows mint a durable parent/child transport address, ordinary selection cannot erase it, activity and expandability frames overlay older in-flight responses, activation removal returns durable children to inactive instead of deleting their lineage, and parent removal immediately invalidates every addressed child's writable availability even when an older catalog response is still in flight. Membership frames use one injected 50 ms debounce per selected or open parent; if it fires during a pull, it marks that response stale and schedules exactly one trailing refresh after settlement, while closing the catalog cancels the timer.
  The manager's WASM facade caches one JavaScript wrapper per Rust Session and one list object per Rust snapshot, routes raw mux/Host envelopes into the same core, preserves generated RPC result objects, and keeps search request-local by passing the caller's exact `AbortSignal` directly to `api.sessions.search` without adding query state to the list observable.
- **Workspace / WorkspaceManager / WorkspaceRuntime** (`workspaces/`): the Rust entity preserves local-intent identity through one shared, retryable Host materialization; a newer adopted view defeats a late create result. The manager single-flights baselines, replays concurrent changed/remove/order frames, permanently tombstones removed random-UUID identities for the connection, rejects older unary snapshots, and arbitrates optimistic reorder echoes by exact request and Host-frame generations. The service joins Workspace and Session baselines, derives recent-Workspace selection without changing Host order, reuses only visible accounted blank Sessions, coalesces creation per Workspace, shields archive frames from stale baselines, and clears an archived current Session. Its WASM facade preserves Workspace-row and returned-action identity, one shared refresh Promise and one Promise per in-flight Workspace creation while immediate reuse returns independent Promises, structured create/browse errors, the caller's exact directory `AbortSignal`, and raw Host-frame routing through the same Rust core.
...
- **SessionProvideChannel** (provide.ts): owns the static hook/prop roster, definite per-Session bundle materialization, and the atomic current-Session observable. The runtime-owned `session` hook is always first. A provider must return every declared member and no undeclared member, and names stay unique across the roster; any live-bundle failure rolls registration back before it can poison later materializations. Selection and roster changes publish through the same identity-deduped source, with subscriber failures contained so one render boundary cannot starve later consumers.
