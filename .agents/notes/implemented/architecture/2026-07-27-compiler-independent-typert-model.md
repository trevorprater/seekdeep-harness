...
The [browser reflection implementation](../../../../crates/client-foundation-wasm/src/wasm_typert.rs) preserves live schema/model identities, atomic package and local-invocation registration, ordered queries, endpoint history, and calling-fiber disposal. Its [live WASM differential](../../../../crates/client-foundation-wasm/tests/wasm_foundation_parity.rs) compares these operations with the pinned service using real Zod and Cordis. Native registry tests do not establish browser parity: complete lookup providers, Host/Client Context providers, Remote subscriptions, and package-export assembly remain separate verification obligations.
