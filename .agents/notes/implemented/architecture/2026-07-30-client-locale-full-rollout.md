...
**The Rust port keeps one locale authority across native Host and browser WASM faces.** `seekdeep-client-locale` owns the target-portable dictionary registry, browser-provisional/Host-explicit preference arbitration, immutable revision snapshots, stable per-namespace translators, duplicate-seat and stale-disposer protection, subscriber isolation, and the Language-row revision mirror. Native-only Cordis/Settings dependencies register the optional Host schema without entering `wasm32`; the Client module factory injects the shell's existing React, ui-primitives, and Client Runtime modules into the WASM face. That face provides the locale Service, installs the standard Slot locale seat, uses Client Runtime's existing Store engine, declaration-injects the Language row, and returns ordinary React element trees; it does not introduce a second renderer, Store implementation, or handwritten JavaScript behavior layer.

**Zero-cordis atoms (ui-primitives) take copy as props**: `copyLabel`/`copiedLabel` on `HoverCard`, `labels` on `TerminalBlock`/`JsonTree`, `copyLabel`/`copiedLabel` on `CodeBlock`, `codeLabels` on `MarkdownText`, `truncatedLabel` on `JsonBlock`, `label` on `ConnectionBanner`, `closeLabel` on `Modal` — defaults are the previous hardcoded strings, so a consumer passing nothing renders byte-identical output. The compiled `JsonBlock` keeps that formatter reference in its source-compatible memo key and applies the 20,000-character cap in JavaScript UTF-16 code units before calling it. Localized plugins pass dictionary-driven labels from their own `t` seat; call sites passing object props memoize them on the `t` identity (`MarkdownText` caches its component table on the `codeLabels` identity).
