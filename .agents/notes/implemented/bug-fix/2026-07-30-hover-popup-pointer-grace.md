...
The pinned source `hover-card.client.spec.tsx` suite covers all 24 component cases. `crates/client-ui-primitives/tests/wasm_hover_card_parity.rs` drives the compiled component through live WASM and pins dwell/grace boundaries, portal placement and reflow, selection-safe copy activation, feedback races, keyboard input, disabled teardown, and timer/listener cleanup; `wasm_clipboard_parity.rs` independently pins the shared host boundary. Assembled-browser hit testing remains part of the pending `apps/web` workspace-management row because a hook harness cannot prove pointer transit across real portal geometry.
