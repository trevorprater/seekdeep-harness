...
The Rust implementation keeps the same compensating transaction in `seekdeep-loader`, `seekdeep-cordis`, and `seekdeep-app-boot`. The live `loader` service inventories configured entries and serializes create, update, move, remove, and file-generation reconciliation. It resolves every enabled replacement before changing live entries, retains unaffected fibers, restores earlier updates and additions after a later failure, and publishes settlement only after compensation finishes. Raw `!!js` nodes stay in entry configuration while a Rust-owned evaluator resolves them after declared services activate; service replacement repeats evaluation against that dependency generation. File-backed JavaScript plugins run in dedicated Rust-owned interpreter workers whose module realm survives configuration restarts and whose effect disposers join fiber teardown. Host HMR records ESM and CJS dependency files, prepares every affected worker while the active generation serves, replaces entries in Loader order, restores prior workers after an apply failure, contains old-generation disposer failures at the source-visible HMR boundary, and returns a full-restart decision for launcher dependencies. Exact-path watchers contain failed refreshes, keep the last-good generation, and drain admitted work before disposal completes.

Expression evaluation preserves JavaScript `undefined` until config interpolation decides its position. An object property whose expression returns `undefined` is omitted so the consumer's schema default applies; explicit `null` remains a value, while top-level and array positions use JSON `null` because those positions cannot be omitted. The raw expression node remains unchanged for write-back and later generations.
...
Rust tests in `crates/loader/tests/runtime_parity.rs` and `crates/app-boot/tests/reload_parity.rs` pin raw-expression preservation, injection-ready evaluation, named and local isolation, exact-generation settlement, live Loader operations, import preflight, fiber retention, compensation, group disablement, and failed-move restoration. `crates/hmr/tests/host_hmr_parity.rs` pins recursive watching, config precedence, full-restart requests, and joined disposal; Loader runtime tests pin ESM/CJS invalidation, shared-dependency ordering, import/apply rollback, and contained disposer failures. `crates/app-boot/tests/watch_parity.rs` pins exact-path identity, serialization, failure containment, and disposal drainage.
