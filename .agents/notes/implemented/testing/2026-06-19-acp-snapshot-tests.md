...
`AcpSnapshotSuite` owns the remaining suite boundary without importing a test framework into runtime code. Definition validates unique scenario names, exactly one header pin per class, and independent prompt and schema sidecar ownership. Replay scenarios are polled concurrently and all started scenarios settle before failures are combined; record and refresh remain serial because they mutate fixtures in registration order. The runner compares normalized stdout and primary-first Session logs, reconstructs every live header from its class pin and sidecars, rejects `UNKNOWN_TOOL`, writes record and refresh outputs, prunes only valid stale child fixtures, and then applies the complete orphan, inventory, canonical-sidecar, unique-content, child-pin, and fixed-point fixture guards. Refresh retains unambiguous message identities, correlated normalized strings, event times, packed member timing, spill paths, and cwd aliases while fresh semantic payloads remain authoritative; ambiguous layouts or mappings keep fresh values. Twenty-three Rust tests cover the pure algorithms plus real replay, record, and refresh subprocess suites, matching the 107 passing and one intentionally skipped source suite cases. The crate root re-exports all four support layers, so no `acp-snapshot` source or test surface remains pending.
