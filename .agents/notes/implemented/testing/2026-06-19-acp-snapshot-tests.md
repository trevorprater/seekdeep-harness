...
In the Rust port, `seekdeep-acp-snapshot` owns this pure normalization boundary: ACP id sequencing and stdout purity, cwd aliases and separator modes, spill locators, event-read volatility, Session and packed-row timing, fixture cwd tokenization, and composable request-header scrubbing. The 44 source normalizer cases have matching Rust tests.

The crate also owns the native launcher and scenario-harness boundaries. The launcher selects compiled source or publish-shaped artifacts through `seekdeep-loader-smoke`, applies isolated SeekDeep and agent homes after caller environment, tees the exact stdout bytes into the Rust ACP client, captures stderr, provides reverse-order future-update waiters and asynchronous permission policy, and closes through stdin EOF or an explicit signal before joining the process, inherited stdio, parser, and incoming permission callbacks. Rust reports spawn failures synchronously from `launch_acp_test_agent` rather than exposing Node's asynchronous `spawned` promise; the failure is therefore resolved earlier without weakening teardown.

The scenario owner interprets every closed `input.json` operation, arms update waiters before prompt dispatch, seeds and prepares an isolated workspace, forwards the replay or record environment, waits on durable parent and child boundaries, consumes permission choices by kind, harvests logs primary-first, attaches stderr, and attempts every owned cleanup. The data-driven Rust fake mirrors every `behavior.json` field. It issues distinct deterministic UUID-shaped Session identities instead of using ambient randomness; identity volatility is normalized at the same boundary as the source fake's random UUID. Rust represents combined scenario and cleanup failures as one `anyhow` diagnostic rather than JavaScript's `AggregateError`, while retaining every underlying diagnostic. Eighteen real-process scenario tests, ten launcher tests, and one launcher timing-seam test cover these owners; the shared ACP bridge retains its 24 tests.

`AcpSnapshotSuite` owns the remaining suite boundary without importing a test framework into runtime code. Definition validates unique scenario names, exactly one header pin per class, and independent prompt and schema sidecar ownership. The compiled `seekdeep-acp-demo` owner registers exactly 78 scenario directories and accepts a bounded `SEEKDEEP_SNAPSHOT_SCENARIOS` filter. Replay polls at most `min(5, available parallelism)` scenarios concurrently unless `SEEKDEEP_SNAPSHOT_MAX_CONCURRENCY` supplies another positive bound; record and refresh remain serial because they mutate fixtures in registration order. Every started scenario settles before failures are combined. The runner compares normalized stdout and primary-first Session logs, reconstructs every live header from its class pin and sidecars, rejects `UNKNOWN_TOOL`, writes record and refresh outputs, prunes only valid stale child fixtures, and then applies the complete orphan, inventory, canonical-sidecar, unique-content, child-pin, and fixed-point fixture guards. Refresh retains unambiguous message identities, correlated normalized strings, event times, packed member timing, spill paths, and cwd aliases while fresh semantic payloads remain authoritative; ambiguous layouts or mappings keep fresh values. Twenty-three Rust tests cover the pure algorithms plus real replay, record, and refresh subprocess suites, and the example owner additionally requires all three packed chunk-row kinds and exact decoded equality with the independent source recording. The complete repository corpus passes both refresh and replay. Two separate compiled goal scenarios retain their source-specific timestamp normalization and semantic fold checks: one exhausts and pauses a two-round goal after cancellation, while the other completes during an automatic round, injects exactly one wrap-up context, and delivers the closing message in that turn. Those scenarios also pin the goal tools' snake_case model arguments independently from their camelCase durable output. The crate root re-exports all four support layers, so no `acp-snapshot` source or test surface remains pending.

Target-only accommodations remain explicit. The job-admission scenario uses the target-native `jobs` config instead of the source fixture's stale `tasks` key, so the configured limit rejects the second job before it starts. SeekDeep's longer product and temp-path names change only byte-capped spill previews, omitted-byte counts, and the retained PTY suffix. Native Rust process launch reports the operating system's `ENOENT` wording and may publish an immediately failed background job before the current step closes, instead of Node's later spawn event. The LSP scenario runs a compiled Rust stdio fixture from `PATH` and materializes `subject.ts` only inside its owned temporary workspace, preserving the source wire and location behavior without storing Node or TypeScript implementation files. PTY guidance uses an internal half-step order to preserve the source prompt order under serialized Cordis activation.

Credential-independent compiled-binary tests complement replay with world effects. A local DeepSeek-compatible SSE server scripts the same model tool turns while the real ACP subprocess, Loader, bash provider, sandbox, approval service, and hook bridge remain in the path. The tests prove an ordinary write lands on disk, a Claude pre-tool hook prevents that write, an allow-once escalation writes only after the initial read-only denial, and a reject-once escalation leaves the file absent. Every case also checks that stdout stays JSON-RPC and ACP publishes only committed assistant chunks.

The source PowerShell Loader test remains conditional on a local `pwsh` executable. The Rust port separates that proof into Loader activation/disposal, schema and prompt assertions, model-facing foreground/background job behavior, and native executor process/output/cleanup suites. This preserves the driver's complete contract on hosts without PowerShell while native resolution tests retain Windows candidate ordering and command encoding.
...
`@seekdeep-ai/seekdeep-llm-replay` is a support package rather than example-local glue. It replaces the real adapter by short-circuiting `llm/stream` with streams reconstructed from JSONL, and its package placement keeps the replay logic under normal coverage gates.
