...
场景 owner 会解释每一项封闭的 `input.json` 操作，在发送 prompt 前注册 update waiter，创建并准备隔离的 workspace，转发 replay 或 record 环境，等待 parent 和 child 的持久边界，按 kind 消耗 permission 选择，以 primary 优先顺序采集日志，附加 stderr，并尝试每一项归属自己的清理。数据驱动 Rust fake 与 `behavior.json` 的每个字段一致。它不使用环境随机源，而是发放互不相同、确定性且形状为 UUID 的 Session identity；其 identity 易变值仍在与 source fake 随机 UUID 相同的边界归一化。Rust 用一条 `anyhow` 诊断表示场景与清理的组合失败，而不是 JavaScript 的 `AggregateError`，但会保留每条底层诊断。18 项真实进程场景测试、10 项 launcher 测试和 1 项 launcher 计时 seam 测试覆盖这些 owner；共享 ACP bridge 则保留其 24 项测试。

`AcpSnapshotSuite` 在不向 runtime code 引入测试框架的情况下持有剩余的套件边界。定义阶段会验证场景名称唯一、每个 header class 恰有一个 pin，并分别解析 prompt 与 schema sidecar 的所有权。已编译的 `seekdeep-acp-demo` owner 会注册恰好 78 个场景目录，并接受有界的 `SEEKDEEP_SNAPSHOT_SCENARIOS` 过滤器。Replay 默认至多并发轮询 `min(5, 可用并行度)` 个场景，除非 `SEEKDEEP_SNAPSHOT_MAX_CONCURRENCY` 提供另一个正整数上限；record 与 refresh 因为按注册顺序修改 fixture，仍保持串行。所有已启动场景都会在组合报告失败前完成。Runner 会比较规范化后的 stdout 与 primary 优先 Session 日志，根据 class pin 和 sidecar 重建每个 live header，拒绝 `UNKNOWN_TOOL`，写入 record 与 refresh 输出，只清理名称有效的陈旧 child fixture，随后执行完整的 orphan、inventory、规范 sidecar、内容唯一性、child pin 与 fixed-point fixture guard。Refresh 会保留无歧义的 message identity、相互关联且归一化等价的字符串、event time、打包成员计时、spill path 与 cwd alias，同时由 fresh 语义 payload 保持权威；有歧义的布局或映射会保留 fresh 值。23 项 Rust 测试覆盖纯算法以及真实 replay、record 和 refresh 子进程套件；示例 owner 还要求同时存在三种打包 chunk 行，并要求解码结果与独立 source 录制精确相等。完整仓库语料同时通过 refresh 与 replay。另有两个已编译的 goal 场景保留各自 source 专属的时间戳归一化与语义 fold 检查：一个场景在取消后耗尽并暂停两轮 goal，另一个场景在自动 round 中完成 goal、恰好注入一次 wrap-up context，并在同一轮中交付 closing message。这些场景还会独立固定 goal tool 的 snake_case 模型参数与 camelCase 持久输出之间的边界。Crate root 重新导出四层支撑能力，因此 `acp-snapshot` 不再有 pending 的 source 或 test surface。
...
`@seekdeep-ai/seekdeep-llm-replay` 是一个支撑包，而非示例本地的胶水代码。它通过用从 JSONL 重建的流短路 `llm/stream` 来替换真实适配器，其包级放置使回放逻辑处于正常覆盖率门禁之下。
