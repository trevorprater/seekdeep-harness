...
`AcpSnapshotSuite` 在不向 runtime code 引入测试框架的情况下持有剩余的套件边界。定义阶段会验证场景名称唯一、每个 header class 恰有一个 pin，并分别解析 prompt 与 schema sidecar 的所有权。Replay 场景并发轮询，且所有已启动场景都会在组合报告失败前完成；record 与 refresh 因为按注册顺序修改 fixture，仍保持串行。Runner 会比较规范化后的 stdout 与 primary 优先 Session 日志，根据 class pin 和 sidecar 重建每个 live header，拒绝 `UNKNOWN_TOOL`，写入 record 与 refresh 输出，只清理名称有效的陈旧 child fixture，随后执行完整的 orphan、inventory、规范 sidecar、内容唯一性、child pin 与 fixed-point fixture guard。Refresh 会保留无歧义的 message identity、相互关联且归一化等价的字符串、event time、打包成员计时、spill path 与 cwd alias，同时由 fresh 语义 payload 保持权威；有歧义的布局或映射会保留 fresh 值。23 项 Rust 测试覆盖纯算法以及真实 replay、record 和 refresh 子进程套件，对应 source 套件 107 项通过与 1 项有意跳过的 case。Crate root 重新导出四层支撑能力，因此 `acp-snapshot` 不再有 pending 的 source 或 test surface。
