...
Rust 提供方的查询与 schema 层把同一组自有子句编译为参数化的 `rusqlite` 谓词，在准备语句前强制执行可移植绑定上限与 FTS5 外层谓词上限，规范化游标请求身份，并按 Unicode 码点限制 snippet。其 schema 所有者以仅所有者可访问的权限创建缺失路径，在修改 journal mode 前拒绝外来或未知数据库，只在版本不匹配时重置已识别派生表，并重新创建持久化 FTS5 与连接本地 TEMP 覆盖层。
