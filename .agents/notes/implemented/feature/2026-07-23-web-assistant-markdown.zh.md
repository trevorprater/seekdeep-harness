...
消费桥接编译在 [`browser_assistant.rs`](../../../../crates/client-ui-conversation/src/browser_assistant.rs) 中。Rust 拥有可见 shell 门控、text／reasoning／image／unknown 派发、连续图片分组与首 index key、tool head 抑制、中断文案、code label 身份、默认图片 loader 失败、未知 JSON footer 格式化，以及 `AssistantNodeView` 的 closing-turn／file-mention ownership gate。它组合 shell 已预置的 ui-primitives 与 ui-attachment module，不链接重复 WASM runtime。更广的 `ChatView`、节点注册与会话组装仍是独立表面。

编译为 Rust/WASM 的 `MarkdownText` 位于 [`browser_markdown.rs`](../../../../crates/client-ui-primitives/src/browser_markdown.rs)，经 [`markdown.rs`](../../../../crates/client-ui-primitives/src/markdown.rs) 中共享的 Rust 语法与增量前沿解析，再由 [`browser_markdown_render.rs`](../../../../crates/client-ui-primitives/src/browser_markdown_render.rs) 把 mdast 映射为 React。生成的 ESM 模块只提供 KaTeX `renderToString` 与 `normalizeUri` 能力；语法选择、URL／图片策略、节点映射、KaTeX 容错链与 DOM 投影、冻结元素生命周期和定稿切换均归 Rust 所有（[增量 AST 渲染器 Note](../architecture/2026-08-06-web-markdown-incremental-ast-renderer.md) 拥有该机制及其 DOM 一致性约定）。它覆盖 CommonMark 块，以及 GFM 表格、任务列表、删除线与自动链接，且不解析原始 HTML。解析器允许至少两个星号组成的连续序列在 Unicode 标点后闭合，前提是其后紧邻 CJK 文本。这一例外涵盖流式输出期间与完成后无空格 CJK 文本中以标点结尾的粗体；单星号强调、紧邻非 CJK 文本的情况、已转义源文本、代码与数学公式仍沿用上游解析行为。围栏代码经共享且已编译的 `CodeBlock` 路由；该组件用客户端的 Shiki 单例（`--shiki-*` token）高亮已注册语法，否则回退为纯等宽文本。轮次流式输出期间，围栏停留在纯文本分支，以免每收到一个分片就对增长中的围栏重新分词。
