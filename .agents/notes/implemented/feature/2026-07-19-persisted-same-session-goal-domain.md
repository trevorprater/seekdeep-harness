...
- **Add goal state or a generic loop abstraction to `seekdeep-agent-loop`** — rejected because state and continuation policy can compose through existing plugins, `Agent` verbs, and events without privileging the shipped loop implementation.
