...
Matcher semantics, exit-code handling, and merge precedence live in `seekdeep-hook-protocol`; each bridge only parses config, builds dialect payloads, and maps outcomes. The pinned Codex source suite passes 59 cases. Rust verification adds 78 shared protocol/invariant tests and ten bridge/parser groups covering all five event registrations, real ToolRuntime decisions, prompt/post-tool context folding, Stop steering, exact payload/workspace/no-newline behavior, durable hook pairs, tolerant config failure, HMR disposal, and detached SessionStart abort/drain. Native plugins bypass the external hook wire after parsing and return typed decisions directly.
