# Agent Note: seekdeep-hooks-claude-code + seekdeep-hooks-codex — the Claude Code / Codex hook bridges
...
Two independent compatibility plugins own the dialects. The Rust production bridge is a native Cordis plugin (`NAME`/`INJECT`/`Config`/`apply`, no default-export ambiguity) injecting only the provider-neutral `shell` capability; the `packages/hooks/` compatibility entries project the same Loader contract (see [postmortem 0001](../../../../docs/postmortem/0001-acp-default-export-drops-inject.md)):

- **`seekdeep-hooks-claude-code`** — the CC dialect. Its Rust bridge owns seven of Claude Code's current hook points: `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `Stop`, `SubagentStart`, and `SubagentStop`. It builds CC-shaped per-event stdin payloads (a base of `session_id`/`transcript_path`/`cwd`/`hook_event_name` plus per-event fields), exports `CLAUDE_PROJECT_DIR`, applies `${CLAUDE_PLUGIN_ROOT}`/`${CLAUDE_PROJECT_DIR}` substitution, retains a local child through its paired subagent end, and uses the literal-or-regex matcher mode. `transcript_path` is the persistence locator result or `''`; stdin carries a **trailing newline**.
- **`seekdeep-hooks-codex`** — five of Codex's current hook points: `PreToolUse`, `PostToolUse`, `SessionStart`, `UserPromptSubmit`, and `Stop`. Its Rust bridge uses an always-regex matcher, Codex-shaped snake_case payloads with `turn_id`/`model`/`permission_mode` extras written WITHOUT a trailing newline, no Codex plugin-env injection or config-time placeholder substitution, and no pre-tool approval or rewrite path. `transcript_path` is the same locator result or `null`; tool payloads carry the real `tool_name` in the reduced `tool_input: { command }` shape. Timeout seconds and default milliseconds remain source-number values through the shared shell request boundary, including fractional values.
...
Matcher semantics, exit-code handling, and merge precedence live in `seekdeep-hook-protocol`; each bridge only parses config, builds dialect payloads, and maps outcomes. The pinned Claude Code and Codex source suites pass 122 cases. Rust verification adds 78 shared protocol/invariant tests and 21 bridge/parser groups across both dialects, covering all event registrations, real ToolRuntime decisions, prompt/post-tool context folding, Stop steering, exact payload/workspace/newline behavior, Claude substitutions/environment/ask/subagent retention, durable hook pairs, tolerant config failure, HMR disposal, and detached lifecycle abort/drain. Native plugins bypass the external hook wire after parsing and return typed decisions directly.
