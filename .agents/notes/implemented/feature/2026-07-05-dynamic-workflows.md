...
### The engine (seekdeep-workflow-worker-thread): one Rust worker thread per run

**Trust premise**: workflow scripts have the same trust as the model's bash access. The Rust-owned Boa evaluator exposes only the documented workflow globals; it exposes no Node `process`, `require`, ambient environment, or credential API. The engine contains buggy scripts and guarantees settled results, JSON-safe values, and cancellation quiescence, but a native thread is not a memory or hard-termination security boundary.

**Why Boa on a dedicated native thread**: production execution stays compiled Rust while preserving the model-authored JavaScript body. Each run owns one unpooled thread, a loop-budgeted Boa context, a root-promise-aware async job executor, and a typed `ChildPort` that bridges `agent()` to host-side child loops. Synchronous guest work cannot block the host; nested `parallel()`/`pipeline()` jobs wake the executor without polling; cancellation rejects queued work and drives published children to disposal.

The host validates metadata and parses the body before publication. It owns pending starts, idempotently disposed child records, one cancellation signal, first-terminal-result precedence, exact start/end pairing, and `disposeGraceMs`-bounded caller settlement. A late provider result is refused and disposed; worker death synthesizes missing cancelled ends before `workflow/end`. The serializable closed message enums remain the protocol definition for a future killable transport, while the compiled in-process path uses typed handles so unknown call ids cannot enter. The [agent-scope runtime-design Agent Note](../architecture/2026-07-12-agent-scope-runtime-design.md#workflow-children-are-pending-starts-or-published-records) owns the child-race algorithms.

Rust cannot forcibly stop an arbitrary native thread. Every supported guest wait and host child boundary is cancellation-driven, so ordinary cancel and disposal reach thread exit; the host grace still releases the caller if an evaluator or injected provider violates that contract. A helper-process or epoch-interruptible runtime is required before the Rust port can claim the source's hard termination of a non-cooperative worker.
...
Worker-side logic runs through typed in-process child ports, and host tests independently drive pending starts, late readiness, slow or failed disposal, worker death, grace settlement, external signals, HMR unload, and lifecycle pairing. Unit tests cover script helpers, unforgeable fatal errors, nullable failures, JSON boundaries, caps, cancellation, child ownership, and structured output through real loops. The compiled-default smoke runs keylessly; built-installed and live-provider execution remain separate parity-manifest boundaries.
...
- **Killable isolation beyond native threads**: a helper-process or epoch-interruptible engine behind the same seam supplies hard termination, memory limits, and stronger sandboxing. The Rust port requires that boundary for non-cooperative worker parity; ordinary supported execution does not depend on it.
...
- **Run Boa on the host thread**: mechanically simplest, but `start()` would let synchronous guest work block the caller and would entangle the guest scheduler with host progress. One native worker thread preserves the JavaScript API while isolating scheduling; the separate killable boundary remains independently replaceable behind `WorkflowEngine`.
...
Fan-out plans live in rerunnable scripts, and `outputSchema` provides authoritative structured child results. Each run pays native-thread and typed child-port costs, while host startup stays non-blocking, cancellation reaches every supported wait, and Rust types plus explicit materialization enforce the value boundary. Native threads are not a hard security boundary. Invalid options fail rather than degrading to Claude Code's `null`; consumers retain control through the run handle while observers receive snapshots only. Top-level Web users also receive a durable, replayable workflow record without widening the execution seam or coupling the original tool card to workflow-specific UI.
