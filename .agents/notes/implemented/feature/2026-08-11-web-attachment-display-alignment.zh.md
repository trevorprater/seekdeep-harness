...
这些 UI 还全部住在 `seekdeep-client-ui-conversation` 里——附件栏内联在 700 行的 `InputBar` 中，历史图片和灯箱分散在 `chat/` 与 `skeleton/`——没有其他界面可复用的接缝，纯 props 的纪律也无从约束。

## 决定

附件展示落位到新的零 cordis 原子组件包 `@seekdeep-ai/seekdeep-client-ui-attachment`（`packages/client/ui-attachment`），模式照 `seekdeep-client-ui-primitives`：`AttachmentRail`（64px、16px 圆角缩略图，单击 `onOpen`，卡片内部的删除按钮悬停或聚焦显示、`pointer: coarse` 下常显，隐藏滚动条配两端圆形箭头并依滚动几何重算，纵向滚轮转横向平移且单次钳制 60px，新增条目滚到栏尾），`MessageImage`/`ImageGallery`（单击预览），以及 `ImageLightbox`。文案经 label props 传入；`ui-conversation` 通过 `src/client/image-labels.ts` 桥接 `conversation` 词典，并保留状态机接线（草稿 id、预览状态、接收回调）。跨包 import 之所以是被允许的路径，正因为它是原子组件库而非 client 插件：插件之间仍禁止互相 import 组件，且附件栏是输入框自有的渲染，不是插槽。

Rust port 在 `seekdeep-client-ui-attachment` 中编译完整组件集合：[`browser_message.rs`](../../../../crates/client-ui-attachment/src/browser_message.rs) 拥有 fit 几何、异步加载／重试／liveness 状态、gallery variant 与 lightbox 组合；[`browser_lightbox.rs`](../../../../crates/client-ui-attachment/src/browser_lightbox.rs) 拥有 body portal、焦点捕获／恢复、Escape、遮罩与关闭行为；[`browser_rail.rs`](../../../../crates/client-ui-attachment/src/browser_rail.rs) 拥有边缘推导、ResizeObserver、非 passive 滚轮归一化、翻页与增长揭示；[`browser_overlay.rs`](../../../../crates/client-ui-attachment/src/browser_overlay.rs) 拥有 body portal、文案门控及两份机械提取的源码 illustration。组件直接读取纯 JavaScript attachment props，不导入由 Cordis 支撑的原生 attachment service。
...
输入框与历史图片界面的交互模型现已与 DeepSeek Chat 一致，label props 接缝让原子组件在任何语言环境下渲染而无需触达 locale。代价是一个真实的包边界：`ui-attachment` 背上标准脚手架（invariant 伴生、双语 README、tsconfig face、逐文件 100% 覆盖率），且每个未来消费者都要自行解析条目文案而非继承。Rust package builder 产出与源码一致的五名称 root index、invariant、原始 WASM binding 与六文件 oracle declaration tree；npm 发布覆盖全部 15 个预期条目，wrapper 消费 shell 既有 ui-primitives module，不链接重复 runtime。固定源码测试与 live WASM 测试锁定 fit 极值、Promise 拒绝／重试、晚到 settlement 抑制、焦点恢复、连续 gallery variant、rail 几何／滚轮／resize／增长行为、overlay 文案门控及两份源码提取 illustration。错误横幅变为短时——用户移开视线四秒就会错过消息，这正是 DeepSeek Chat 自己做的取舍。非图片附件仍不支持；附件栏的卡片模型已就绪，但输入框的接收仍只认图片（记录于包 README 的限制一节）。
