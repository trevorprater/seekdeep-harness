...
`seekdeep-tool-session-query` 是 `ctx.sessionQuery` 的 Rust 生产消费方；`@seekdeep-ai/seekdeep-tool-session-query` 兼容包投影同一模型约定。它注册五个职责单一的只读工具：`session_search`、`session_event_search`、`session_trace`、`session_event_trace` 和 `session_event_read`。该 crate 依赖接口而非 SQLite 实现，负责模型参数校验与易读文本渲染，并贡献一个精简的提示词段，说明历史搜索以及从搜索转向追踪／读取的工作流。

该 crate 入口仅作为配置、提示词注册、原生 Cordis 生命周期与工具注册的公开组合根。内部模块沿执行边界划分：`input.rs` 负责模型 schema、精确 ISO 时间戳规范化与过滤条件构造；`service_boundary.rs` 包含提供方调用与面向模型的安全错误转换；`workspace_access.rs` 负责调用者身份、工作区授权、标题访问与迭代式谱系投影；`operations.rs` 编排五个服务工作流；`presentation.rs` 渲染工具结果与调用卡片。这样可让策略留在其所属层，同时不改变模型约定。
...
每个受信任的 `ctx.sessionQuery` 调用都会经过同一个模型边界净化器。它首先检查执行信号，保留调用者取消。对于其他失败，Rust 边界会把可检查的完整 `anyhow` cause 链写入内部 Cordis 日志，穷尽映射封闭的 `SessionQueryErrorCode` 词汇为固定的模型安全消息，只保留固定授权拒绝，并把所有其他失败收敛为 `SESSION_QUERY_TOOL_FAILED`。逐标题失败也会先经过同一个净化器，再转为不可用标记。工具自身的输入校验错误会在工具运行时边界转换为具名 `SessionQueryError`，授权错误则因在受信任服务调用之外创建而保持精确。
...
固定的源包通过 102 个工具与 SQLite 集成用例。Rust 分组测试固定参数校验、跨正数／零点／纪元前瞬间的相邻浮点时间戳转换、精确工作区授权、父级预授权、变更观测拒绝、服务错误码穷尽脱敏、身份缺失、隐藏边界裁剪、当前步骤排除、内部提供方翻页与重复游标拒绝、搜索独占与精确读取并行分类、数量上限、逐标题隔离、10,000 层迭代式谱系、搜索／追踪／读取渲染、原生注册与配置默认值。真实集成会驱动完整的 JSONL 持久化 → SQLite FTS → 会话查询工具运行时路径，覆盖实时与持久化会话、小数时间边界和未删节精确事件读取。发布配置与 TUI/Web 组合测试证明面向模型的消费方不存在于 TUI、Web 与无头界面，而组装后的 ACP 请求头快照证明自动化界面默认不包含它。
