...
`@seekdeep-ai/seekdeep-client-ui-primitives` exports `MarkdownText` as the untrusted assistant-text renderer, and `ui-conversation` selects it only for assistant `text` blocks. Finalized history, the streaming tail, and interrupted partials already share `AssistantMarkdown`, so they receive the same renderer without changing events or snapshots. User and steering messages keep the compiled Rust/WASM `MessageText` in [`browser_markdown_atoms.rs`](../../../../crates/client-ui-primitives/src/browser_markdown_atoms.rs) and remain literal.

The consumer bridge is compiled in [`browser_assistant.rs`](../../../../crates/client-ui-conversation/src/browser_assistant.rs). Rust owns visible-shell gating, text/reasoning/image/unknown dispatch, consecutive-image grouping and first-index keys, tool-head suppression, interruption copy, code-label identity, default image-loader failure, unknown JSON footer formatting, and `AssistantNodeView`'s closing-turn/file-mention ownership gate. It composes the shell's already-seeded ui-primitives and ui-attachment modules rather than linking duplicate WASM runtimes. Broader `ChatView`, node registration, and conversation assembly remain independent surfaces.

The compiled Rust/WASM `MarkdownText` in [`browser_markdown.rs`](../../../../crates/client-ui-primitives/src/browser_markdown.rs) parses through the shared Rust grammar and incremental frontier in [`markdown.rs`](../../../../crates/client-ui-primitives/src/markdown.rs), then maps mdast to React in [`browser_markdown_render.rs`](../../../../crates/client-ui-primitives/src/browser_markdown_render.rs). The generated ESM module supplies only KaTeX `renderToString` and `normalizeUri` capabilities; Rust owns the grammar choice, URL/image policy, node mapping, KaTeX error chain and DOM projection, frozen-element lifecycle, and finalization swap (the [incremental AST renderer note](../architecture/2026-08-06-web-markdown-incremental-ast-renderer.md) owns that mechanism and its DOM-parity contract). It covers CommonMark blocks plus GFM tables, task lists, strikethrough, and autolinks without raw-HTML parsing. The parser lets runs of at least two asterisks close after Unicode punctuation when followed immediately by CJK text. This exception covers punctuation-terminated strong emphasis in whitespace-free CJK prose during streaming and after settlement; single-asterisk emphasis, non-CJK adjacency, escaped source, code, and math retain upstream parsing. Fenced code routes through the shared compiled `CodeBlock`, which highlights registered grammars with the client's Shiki singleton (`--shiki-*` tokens) and falls back to plain monospace otherwise. While a turn streams, fences stay on the plain arm so growing fences are not retokenized every chunk.
...
The dependencies are explicit in `ui-primitives`; because that pure library is seeded by the Web shell, the compiled parser and highlighter are part of the initial browser bundle. The Rust package builder projects the pinned KaTeX stylesheet, all 60 relative font assets, and its MIT license into the generated library, and Rust installs one deduplicated stylesheet link before rendering math. The same build emits the exact 102-name root index (72 names derived directly from the Rust catalog: 70 icons plus two brand glyphs), the invariant and internal compatibility entries, and the oracle's 43-file declaration tree with only required package-identity substitutions; the publish allowlist includes every WASM, backend, type, CSS, font, and license artifact.
