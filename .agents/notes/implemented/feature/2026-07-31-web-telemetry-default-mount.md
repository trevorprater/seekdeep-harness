# Agent Note: Default session-telemetry mount (OTel reporting) in the seekdeep web composition
...
| CI isolation | top-level `env: SEEKDEEP_TELEMETRY_DISABLED: '1'` in GitHub workflows | Defense in depth keeps test sessions local even when a job explicitly selects an uploading mode |
...
- A developer running `seekdeep web` without telemetry configuration makes no telemetry network request. An internal deployment sets `SEEKDEEP_TELEMETRY_MODE` and may point `SEEKDEEP_TELEMETRY_OTLP_URL` at another collector.
