...
Rust 监督器使用单一顺序异步所有者来表达同一套代协议，而不是回调图和 Promise 队列。每个全新的 `McpClient` 代都必须完成连接与完整发现后才发布；只有当前代能够接收工具列表变更工作，所有再同步天然由所有者循环串行化。一套注入的单调时钟与 sleeper 在一致性测试中驱动退避、稳定窗口重置和关闭屏障。原生计时适配器被限制在边界内，因此核心重试策略不会由环境 executor 顺序或墙上时钟决定。

关闭信号意味着 stdio 子进程已经退出，而不只是 stdout 已结束。失败代必须在循环休眠或构造替代代之前完成有界关闭；超时会停止本次故障恢复，以防子进程重叠。取消可中断连接、发现、通知再同步和退避，然后等待当前代关闭与工具 effect 释放。真实进程套件验证了回复后子进程死亡、有界重新拉起、精确工具代替换、恢复后调用，以及退避期间立即卸载；确定性假代验证了尝试上限、稳定重置、崩溃循环耗尽、过时通知惰性、禁用恢复、同步失败保留，以及进行中同步期间 dispose。
