...
[`base.cordis.yml`](../../../../packages/bundle/base/cordis.patch.yml) owns one sandbox and permission stack for every shipped TUI, Web, and browser-backed headless session: `seekdeep-sandbox-local`, `seekdeep-sandbox-policy`, `seekdeep-bash-sandbox`, `seekdeep-fs-sandbox`, `seekdeep-user-approval`, and `seekdeep-permission-presets`. The composition fallback is the `workspace-write` preset, which bundles `workspace-write` file effects with the `ask` approval policy. `SEEKDEEP_PERMISSION_MODE` remains an explicit process override; a stored `permission.defaultPreset` remains the user preference for later sessions and outranks the fallback through the Settings seam.
