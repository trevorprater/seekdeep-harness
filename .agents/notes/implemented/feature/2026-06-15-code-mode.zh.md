...
原生 Rust 后端按提交顺序在调用方的 Tokio 运行时上调用绑定，并轮询至其首次挂起。挂起的调用可以重叠。worker 完成或取消不会销毁绑定所创建的宿主任务；Code Mode 工具桥仍负责取消并排空自己的子调用。只有 JavaScript 求值属于每次程序运行独有的 worker 运行时。纯程序不需要调用方运行时，可调用的绑定则需要。
...
通用 Node 模块导入，以及 Node type-strip 的精确位置和诊断，仍是原生实现的兼容性缺口；堆回归测试不验证这些行为。在这些行为得到覆盖前，manifest（元数据清单）仍将公开运行时条目标为 pending。
