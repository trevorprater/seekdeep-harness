...
1. **Code Mode 是 `ToolRuntime`（`seekdeep-tools`）的一等呈现模式**，通过经校验的 `mode` 配置选择：`'native'`（默认，贡献可见能力 schema）、`'code'`（注册表仅贡献其保留的 `run_code` 传输通道加一份生成的 SDK `.d.ts` 到系统提示词中）或 `'both'`（原生 schema 加传输通道 + SDK）。注册表在源头构建其规范贡献；协作式提示词组装的结果仍具权威性，记录在日志中的请求头精确反映该返回的呈现。
2. **代码执行是一个能力 seam**——`packages/code-runtime/` 包含 Service Definition 包 `@seekdeep-ai/seekdeep-code-runtime`，拥有 `ctx.codeRuntime`（[能力 seam](../architecture/2026-06-13-capability-seams.md)；消费方 = `seekdeep-tools`，core 消费 seam 的先例见 `agent-loop` → `seekdeep-llm`）。运行时对工具一无所知：它接收一段程序和命名的异步绑定，执行程序，报告 `{ value, logs, error? }`。语言和基底是后端属性，因此未来的 Python 或容器后端只是另一个 Service Provider 包，而非重新设计。
3. **实现为 `@seekdeep-ai/seekdeep-code-runtime-worker-thread`**：每次运行 spawn 一个全新的 worker 线程，对模型的 TypeScript 进行 type-strip 后执行，绑定经过桥接，环境为空，堆/输出/时间上限可配置，并支持硬终止。固定版本的源实现使用 Node worker 线程；下文说明原生 Rust 实现。其信任姿态在设计上等同于 bash——无需 unsafe-acknowledgement flag——因为 harness 已经交付了 `seekdeep-bash-local`，后者以严格*更高*的环境权限执行模型编写的任意 shell 命令。
...
`packages/code-runtime/code-runtime/`——`@seekdeep-ai/seekdeep-code-runtime`，仅依赖 `cordis`。一个抽象的 `CodeRuntime extends Service`（`super(ctx, 'codeRuntime')`）加上词汇：
...
固定版本源实现中的 `@seekdeep-ai/seekdeep-code-runtime-worker-thread` 是 `packages/code-runtime/` 组的第二个包，它定义了每次 `run()` 的以下约定：
...
原生 Rust 后端按提交顺序在调用方的 Tokio 运行时上调用绑定，并轮询至其首次挂起。挂起的调用可以重叠。worker 完成或取消不会销毁绑定所创建的宿主任务；Code Mode 工具桥仍负责取消并排空自己的子调用。只有 JavaScript 求值属于每次程序运行独有的 worker 运行时。纯程序不需要调用方运行时，带有可调用绑定的可执行程序则需要。

原生类型剥离在宿主绑定分发或 worker 创建之前运行。它使用编译后的 Rust SWC 31.0.0，即 [Amaro 1.1.2 源构建](https://github.com/nodejs/amaro/blob/v1.1.2/deps/swc/Cargo.lock)中的擦除版本，并采用 SWC common 的 Serde 兼容性补丁。保留的 token、注释、换行符以及 UTF-8/UTF-16 偏移保持不变；首条发出的诊断成为程序的异常消息。V8 的导入钩子通过每次运行独有的模块命名空间解析消息端口提供方，接受计算得到的模块标识符及普通导入空白，而不改写程序字符串。

原生求值器为每次运行创建一个 V8 isolate，并将 `maxOldGenerationSizeMb` 应用于其老生代上限。接近堆上限时，回调以 `worker-exit` 终止求值，仅提供有限的应急余量，让终止过程完成展开而不杀死宿主。一个会被等待退出的监控线程独立计量求值器的活跃时间，不计入等待绑定的时间；计算预算、挂钟上限到期或取消时，它会中断同步 JavaScript 和微任务。终止值通过所有权转移传递，而非递归克隆，因此深层嵌套的完成值仍遵守迭代式 JSON 约定。这一原生 Code Mode 选型不会替换 Cordis/Host 求值器，也不决定其重载策略。
...
- **Rust headless 集成：** 七个确定性的真实 worker 案例固定大型中间值、`ToolCallError` 字段隔离、后台 job id 与取消所有权、前台取消、精确的 Cordis 版本化 DTO、仅通过 `run_code` 执行并写入拼接输出的 Bash 程序，以及在下一次模型请求前交付的嵌套文件系统指令。两个真实提供方源场景仍受密钥门控；对应的确定性 Rust 镜像无条件运行。
- **快照：** `code-mode-turn`、`both-mode-turn` 和 `code-mode-workspace-context` fixture（测试前置数据）固定 SDK 文本、请求头工具列表、分发事件、延后上下文和结果卡片。

原生宿主回归测试要求绑定任务在 worker 退出后继续存活、初始调用保持有序，并让已接受的调用在 worker 取消后完成。公开运行时的堆上限回归测试在 16 MiB 和 512 MiB 下运行同一个有限的保留数组程序：较小上限产生与源实现兼容的 `worker-exit` 诊断，并完整保留此前日志；较大上限返回精确值；后续运行则证明宿主仍然存活。深层伪造完成值会检验终止过程中的所有权转移。

实时 Node 差分测试逐项比较 63 个程序的精确擦除结果和首条诊断，覆盖环境声明、被拒绝的运行时语法、Unicode、换行符、注释以及格式错误的输入。没有宿主运行时的回归测试固定了语法失败发生在绑定分发及 worker 创建之前这一约定。导入回归测试固定命名空间身份、源码文本保留，以及源实现对已缓存 JavaScript 类型和失败的处理行为。

通用 Node 模块 API，包括其余 `worker_threads` 导出，仍是原生实现的兼容性缺口。类型剥离和消息端口导入不能证明这些 API；manifest（元数据清单）仍将公开运行时条目标为 pending。
...
**在程序 worker 上执行原生绑定 future。** 否决：worker 终止会取消会话写入器及其他生命周期属于应用的宿主服务。宿主分发保留这些所有者，同时不把模型代码的执行移入宿主。

**使用 Boa 计数器或进程 RSS 近似老生代上限。** 否决：Boa 的公开运行时限制不提供堆分配上限，而整个进程的内存包含宿主绑定，以及源实现的老生代上限不计入的分配。V8 提供所需的原生 isolate 上限；它增加了原生库及平台构建依赖，但不会使 worker 成为安全边界。

**移除类型后重新生成 JavaScript。** 否决：代码生成会改变注释、位置和接受的语法；额外的语义分析可能拒绝源实现会完成类型剥离并留给 worker 编译的代码。源实现的仅剥离算法和首条诊断保留了这一职责划分。
...
**注册表 scope 增长。** `seekdeep-tools` 吸收了代码生成、一个工具、一个桥和一个事件。包内模块把这些职责分开（`ts-types.ts`、`code-mode.ts` 与 `schema.ts`、`json-schema.ts`、`presentation.ts` 并列），所有 code-runtime 专用实现都由 `ctx.codeRuntime` 提供。
