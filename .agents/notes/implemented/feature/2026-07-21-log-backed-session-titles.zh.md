...
只有人类来源的 `user/message` 事件中的文本块才符合条件。空提示词、仅含控制内容的提示词和非文本提示词会等待下一条合格消息。对于在线 agent，`agent/request` 屏障会在合格的用户消息和运行时上下文消息之后、`request/header` 之前生成首个回退标题；没有在线 agent 的会话仍使用脱离提示词路径的回退任务。共享的会话本地异步门会防止并发提示词路径与 refresh 路径追加重复的回退事件。回退规范化会消除空白和控制序列差异，应用已配置的单词数和 UTF-8 字节限制且不拆分代码点，并记录第一条消息的 seq。

仅当主循环存在已记录在日志中的当前提供方/模型路由时，自动提供方工作才会启动。`request/header` 新追加到日志时，会直接启动待执行工作；如果请求头没有变化，则由循环构建并带有标记的 `llm/stream` 请求会先与折叠所得的路由匹配，再启动该工作。主模型流只会等待脱离响应路径的标题任务被首次轮询；这保证可审计的 `session/title-llm-request` 先于主模型输出，而不会把标题完成时间放入响应关键路径。随后，生成工作独立于 agent 响应运行；完成结果会追加一个独立事件，而不改变轮次状态。显式调用 `refresh(session, signal?)` 会生成尚缺的回退标题并等待已注册的提供方；没有提供方时则返回回退标题。调用方取消不会回滚已接受的回退事件，`refresh()` 会在返回成功前重新检查信号。并发刷新会在提供方工作之前预留会话本地修订号，因此较新的调用会在任何调用有机会造成提供方完成顺序倒置之前中止并取代较早的调用。自动工作与并发刷新共用会话本地回退门，因此首个回退只会创建一个标题事件。异步压缩（compaction）期间接受的标题仍是纯日志事件，因此压缩器在摘要完成后执行的表层节点检查不会因该标题而失败；并发的表层变更仍会使替换失效。
