...
`cargo xtask web-assembled --export` exercises the real Header against the Rust Host after searching, opening, and reloading persisted source history. It observes the HEAD preflight, saves the native browser download from the expected GET URL, compares its JSONL bytes with the independently decoded durable artifact, and opens and dismisses the success Modal. The original recording must remain an unchanged prefix; normal Session setup may append events. Chromium can omit browser-owned downloads from request events, so the download URL and saved archive are checked through its Download API. This keyless fixture does not establish a live-model interaction or the slash-command path.
