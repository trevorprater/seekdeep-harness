...
The existing `/permission` command and `permissions` projection remain the current-session path. `/permission` routes the approval half of a switch through `ApprovalService.set_policy`, so the durable knob change and its model-facing policy-change injection remain one live-agent operation; the service's programmatic `set` path writes the same durable facts without requiring a live agent. The browser plugin contributes the Permission row to `settings.general.item`, reads the dynamic enum from the redacted Settings descriptor, and writes only `defaultPreset` through a revision-checked `settings.mutate`. The row injects its observable through the slot `hooks` compartment instead of binding a renderer-specific hook, and the Permission service sweeps already-live sessions when it mounts so HMR cannot leave an unpinned session. The ownerless General-settings package contributes no placeholder rows.
...
The Rust permission-preset suites drive the same 31 cases as the pinned source: table order and tie-breaking, write-through/no-op/repair behavior, composition failures, dynamic settings validation, fresh/seeded/partial/late session pinning, invariant replay and pre-commit rejection, projection change notifications and HMR removal, and `/permission` success, status, rejection, command lifecycle, and approval injection. The source suites and Rust suites run independently so a shared implementation cannot satisfy both sides of the comparison.
