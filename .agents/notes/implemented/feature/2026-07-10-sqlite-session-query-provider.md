...
The Rust implementation in `seekdeep-session-query` represents range bounds as finite source numbers, preserving negative and fractional values for created-at, sequence, and time comparisons. `seekdeep-session-query-sqlite` provides the native Cordis plugin and the complete serialized engine: it compiles owned clauses into parameterized `rusqlite` predicates, enforces the portable binding and FTS5 outer-predicate ceilings before statement preparation, canonicalizes cursor request identity, and bounds snippets by Unicode code point. Its schema owner creates missing paths and sidecars with owner-only modes, preserves existing modes, refuses foreign or unknown databases before journal-mode mutation, resets only recognized derived tables on version mismatch, and recreates persistent FTS5 plus connection-local TEMP overlays.
...
The shared extractor includes visible message text, nested tool-call/result content, tool names and arguments, blocked-prompt reasons, todo status/content, and error or terminal status detail. Assistant reasoning blocks, structural boundaries, stream chunks, request headers, successful completion markers, and unknown declaration-merged event/content variants produce no document. Surface classification reuses `foldSurface()` so search agrees with model-history derivation.
...
Unit coverage pins extraction, finite fractional and pre-epoch ranges, both search scopes, all default surfaces, metadata-before-ranking, snippets, literal escaping, deterministic ties, complete pagination, scoped cursor invalidation, dynamic persistence mount/unmount, hostile observation churn, restart reconciliation, live shadow/reveal/reopen, schema safety, rollback retry, and queued/in-flight source-wait cancellation. The source Loader oracle and the native Rust Cordis plugin test both combine the query package with a real SQLite persistence backend.
