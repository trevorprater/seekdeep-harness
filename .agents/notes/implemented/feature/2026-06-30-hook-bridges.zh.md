# Agent Note: seekdeep-hooks-claude-code + seekdeep-hooks-codex —— Claude Code / Codex 钩子桥接插件
...
- **`seekdeep-hooks-claude-code`**——CC 方言。其 Rust 桥接拥有 Claude Code 当前钩子点中的七个：`SessionStart`、`UserPromptSubmit`、`PreToolUse`、`PostToolUse`、`Stop`、`SubagentStart` 和 `SubagentStop`。它构建 CC 形态的逐事件 stdin payload（基础字段 `session_id`/`transcript_path`/`cwd`/`hook_event_name` 加每事件字段），导出 `CLAUDE_PROJECT_DIR`，应用 `${CLAUDE_PLUGIN_ROOT}`/`${CLAUDE_PROJECT_DIR}` 替换，在配对 subagent end 前保留本地子 Agent，并使用字面量或正则的匹配模式。`transcript_path` 是持久化定位器结果或 `''`；stdin 带有**尾部换行**。
...
CC 桥接的 `ask` 结果是一条真正的权限路径，而非终态桥接决策：`seekdeep-tools` 通过可选的[审批 seam](2026-07-06-approval-seam.md) 来解析它。ACP（Agent Client Protocol）自动化客户端可以应答所属会话的一次性机器策略请求，`allowed-once` 后继续执行；如果没有 ApprovalService 或应答器，调用以 `deny` 安全关闭。
...
匹配语义、退出码处理和合并优先级位于 `seekdeep-hook-protocol`；每个桥接只负责解析配置、构建方言 payload 和映射结果。固定的 Claude Code 与 Codex 源套件共通过 122 个用例。Rust 验证另含 78 个共享协议／invariant 测试与两种方言共 21 个桥接／解析器分组，覆盖全部事件注册、真实 ToolRuntime 决策、提示词与工具后上下文折叠、Stop steering、精确 payload／工作区／换行行为、Claude 替换／环境／ask／subagent 保留、持久 hook 对、容错配置失败、HMR 释放，以及 detached 生命周期的取消／排空。原生插件在解析后绕过外部 hook wire，直接返回类型化决策。
