...
The harness spoke one shell dialect on every platform: `bash`. Windows hosts could run it only through WSL or Git-Bash shims, and the shipped `seekdeep-bash-local` executor is POSIX-only (`bash` hardcoded, process-group semantics POSIX). The Windows roadmap — defaulting hosts to `pwsh`, later pwsh TUI/GUI rendering — had no execution foundation: there was no PowerShell implementation of the bash executor seam and no model-facing tool that taught the PowerShell dialect. The bash tool is also larger than a Windows-first profile strictly needs — the persistent-PTY twin in particular is bash-shaped surface the `pwsh` tool still does not carry. The original minimal profile also left out background jobs and sandbox escalation: background arrived with the [parity decision](2026-08-02-pwsh-tool-bash-parity.md), and the sandbox surface (denial rendering plus `sandbox_permissions` escalation) arrived with the [Windows ACL sandbox decision](2026-08-08-windows-acl-restricted-token-sandbox.md) — the minimal tool was sized for the danger-full-access Windows posture, and that premise ended when the sandbox PR re-enabled confinement and approval on Windows.

## Decision

The public plugin identities remain as two packages under `packages/shell/`, backed by Rust workspace crates:

- **`@seekdeep-ai/seekdeep-pwsh-local`** — a local implementation of the `ctx.shell` executor seam over `ctx.subprocess`, mirroring `seekdeep-bash-local` call-for-call: `resolve()` defaults and caps from config, `run()` fuses the config-clamped timeout with the caller's signal through one deadline, `start()` returns a consuming background handle whose processes belong to the subprocess service. The command string rides as ONE argv element to `pwsh -NoLogo -NoProfile -NonInteractive -Command`, so PowerShell parses it and no shell-quoting layer exists. Executable resolution (`resolvePwshPath`) is a pure function of `(configured, env, platform)`: explicit config first, then Windows probes PowerShell 7's install, PATH entries (quotes stripped), and Windows PowerShell 5.1, else a bare `pwsh` via PATH. Production code lives in [`crates/pwsh-local`](../../../../crates/pwsh-local/src/lib.rs).
- **`@seekdeep-ai/seekdeep-tool-pwsh`** — the model-facing tool over `ctx.shell`, PowerShell-dialect by contract, mirroring `seekdeep-tool-bash` call-for-call: foreground and `run_in_background` execution through the generic job runtime, managed `SEEKDEEP_*` environment through the shared [`seekdeep-shell-env`](../feature/2026-08-02-pwsh-tool-bash-parity.md) registry, the bash marker/truncation rendering story (a clean exit produces no marker), and — since the Windows ACL sandbox decision — the sandbox denial rendering and `sandbox_permissions` escalation surface, plus the Windows-specific ConstrainedLanguage and named-pipe contracts in the tool description. The parity decision supersedes this note's minimal-profile tool description. Production code lives in [`crates/tool-pwsh`](../../../../crates/tool-pwsh/src/lib.rs).

The focused TypeScript compatibility suites run real `pwsh` cases when the executable is available and self-skip those cases when it is absent. Rust process and lifecycle suites use a deterministic argv shim on POSIX so timeout, cancellation, background ownership, spill, and teardown remain exercised without PowerShell; the Windows CI lane owns native build and static checks.
