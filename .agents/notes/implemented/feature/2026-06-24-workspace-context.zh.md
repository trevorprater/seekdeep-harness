...
该实现在 `packages/context/agent-instructions` 中，包名为 `@seekdeep-ai/seekdeep-agent-instructions`。它是请求上下文扩展，不是核心服务或文件系统后端。共享 demo 主干与 Host Runtime 根据显式的 `{ maxBytes } | false` 部署选择挂载它；`seekdeep web` 启用 65,536 字节预算，Host Runtime 的 headless 消费方则禁用它。该插件使用 `agent/pre-step`、不可变的 `tools/result` 结果、`session/event` 边界和可选的 `ctx.fs` 功能。
...
用户全局文件固定为 `$SEEKDEEP_HOME/AGENTS.md`，不受任一候选列表影响，也没有本地覆盖层。`$SEEKDEEP_HOME` 默认为 `~/.seekdeep`，与 `~/.codex` 或 `~/.claude` 在 harness 层的 home 角色一致，而不会引入插件专用 home。波浪号展开与默认值位于 `seekdeep-home-paths` 中，以便未来的 harness 功能共享同一约定。
...
基线是一条 user 角色的 `<system-reminder>`，包含 `Instructions from: <path>` 章节，以及明确的权威性与优先级说明。这种熟悉的模型可见框架避免引入 harness 专用的 XML 词汇。项目路径相对于根目录；使用默认 home 时，用户全局路径为 `~/.seekdeep/AGENTS.md`，使用已配置 home 时则为 `$SEEKDEEP_HOME/AGENTS.md`。最终渲染边界会在完成字节核算前，转义指令内容或模型可见的路径、scope 与预算元数据中出现的字面量 `</system-reminder>`。包 README 负责规定当前准确的[提示词形态](../../../../packages/context/agent-instructions/README.md#prompt-shape)。
...
真实模型源 e2e 定义了 3 个受密钥门控的案例。确定性的 Rust 镜像通过生产持久 driver、system prompt、tool runtime、本地文件系统、真实 read 工具、Session store 与指令运行时，无条件运行全部 3 个案例。它证明基线可见、真实读取之后发现后代指令，以及把已变化基线追加为替换转换而不重写冻结前缀。嵌套案例还固定：当下游 pre-step 不提供已领取消息时，插入位置必须是索引 0，与 JavaScript `findLastIndex()` 返回 `-1` 的语义一致。

组装恢复镜像会为已完成轮次的历史提供种子，并验证两种首次请求转换。兼容标识只保留一条基线，并把离线 `AGENTS.md` 编辑追加为 `replace`；不兼容的候选优先级标识则追加一条完整的取代基线，其中 `AGENTS.md` 排在 `CLAUDE.md` 之前。两种情况下，当前上下文都会在恢复后的响应之前到达模型。

Rust 单元层用 30 项分组测试覆盖配置、发现与优先级、本地覆盖、符号链接、内容去重、UTF-8 字节预算、微小预算回退、仅提供方可见的文件、有界流、取消、不可用探测、会话隔离、插件 dispose、动态 set／replace／remove 对账、回放抑制、compaction 后重新启用，以及版本更新保留。微小预算覆盖可防止零候选二分搜索陷入循环；提供方测试则要求一次元数据探测失败只影响该候选，绝不能隐藏可用的 sibling。
