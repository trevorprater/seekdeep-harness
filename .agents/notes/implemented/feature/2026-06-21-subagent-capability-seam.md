...
> The full seam is shipped: the `seekdeep-subagent` interface and `seekdeep-tool-subagent` consumer; the two in-process backends (`seekdeep-subagent-spawn-in-process`, `seekdeep-subagent-fork-in-process`); the nested-agent snapshot infrastructure ([per-session snapshot replay](../testing/2026-06-22-subagent-snapshot-replay.md)); and the out-of-process ACP, Codex, and Claude Code backends ([ACP Agent Note](2026-06-22-acp-subagent-backend.md), [product-provider Agent Note](2026-08-04-claude-code-and-codex-subagent-backends.md)).

## Problem

The harness has a long-deferred seam for **subagents** — an agent delegating work to another agent. The intent was sketched in the `Agent`/`AgentLoop` interfaces ([packages/core/agent/src/types.ts](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/core/agent/src/types.ts), [packages/core/agent-loop/src/index.ts](https://github.com/fugue-labs/deepseek-harness/blob/37200a934324dd7167ec8a8d3ac1fd01e2239909/packages/core/agent-loop/src/index.ts)): a creation option referencing a parent agent (fork = seed the child session with the parent's event log; spawn = fresh session), with the child returned as an `Agent` handle so steering and event subscription work uniformly.
...
| Package | Role |
|---|---|
| `@seekdeep-ai/seekdeep-subagent` | interface: `SubagentRuntime` (`ctx.subagents`), `SubagentProvider`, `SubagentRun`, the request/result/capability vocabulary, the `subagent/*` events |
| `@seekdeep-ai/seekdeep-subagent-spawn-in-process` | implementation: a fresh in-process child via `ctx.agents.create` |
| `@seekdeep-ai/seekdeep-subagent-fork-in-process` | implementation: an in-process child seeded with a snapshot of the parent's log |
| `@seekdeep-ai/seekdeep-subagent-acp` | implementation: an ACP client driving a configured child process |
| `@seekdeep-ai/seekdeep-subagent-codex` | implementation: a one-shot official Codex app-server process |
| `@seekdeep-ai/seekdeep-subagent-claude-code` | implementation: a one-shot official Claude Code process through the Agent SDK |
| `@seekdeep-ai/seekdeep-tool-subagent` | consumer: the model-facing `subagent` tool over `ctx.subagents` |

### The primitive: async `start → SubagentRun`

A provider exposes `start(request) → Promise<SubagentRun>`. Fulfillment publishes a child and transfers its run handle to the caller. Work that fails before publication rejects `start()`, while prompt, turn, cancellation, and infrastructure outcomes after publication settle through `run.result` without hiding the child id. The Rust `SubagentRun::result()` returns `anyhow::Result<SubagentResult>`, preserving an infrastructure rejection independently from `dispose()` failure; lifecycle telemetry maps a rejected result channel to `error` without losing the disposal diagnostic at the collecting boundary. One signal covers cancellation before and after publication; `dispose()` cancels remaining work and awaits quiescence. A rejected start cleans unpublished resources and emits no lifecycle event, while a post-publication result failure closes the published lifecycle pair. Provider-added listeners run at the registration commit boundary: a synchronous veto rolls the provider entry back, while provider removal and run lifecycle observations contain listener failures. `start` is transport-neutral; `spawn` names only the fresh in-process backend.
...
`seekdeep-tool-subagent` passes its execution signal to `start()`, awaits the child result, and disposes the run before reporting. Non-completed outcomes become error results rather than successful partial output, and independent result and disposal rejections retain both diagnostics.

### Provider selection is config, not model-facing

`seekdeep-tool-subagent` binds to exactly one provider name (`Config.provider`); the model sees only `{ description, prompt }`. To expose more than one transport, load the tool plugin more than once, each bound to a different provider and a distinct `toolName` (the tool registry rejects a duplicate name). The *service* holds the multi-provider registry; the *tool* picks one — the schema carries no provider/type parameter.
