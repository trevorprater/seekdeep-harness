...
- **`@seekdeep-ai/seekdeep-session-telemetry-otel`** —— 参考后端：原生 Rust OpenTelemetry 日志流水线（`SdkLoggerProvider` → 异步批处理器 → OTLP/HTTP JSON 导出器）。对象安全的 `OtelLogPipelineFactory` 会收到完整的 `exporter`/`processor` 值并默认使用原生 SDK 绑定，而确定性测试和平台集成只可替换该上报边界。`DISABLED` 是默认值，且不构造任何传输；[反馈门控遥测决策](2026-08-05-feedback-gated-session-telemetry.md)定义了需显式启用的 `FULL` 与 `FEEDBACK_ONLY` 投递模式，这两种模式要求 `exporter.url`，且不移动脱敏或后端边界。[无缓冲反馈回放](../simplification/2026-08-06-buffer-free-feedback-telemetry.md)避免在内存中创建会话前缀的第二份副本。
...
**把 seam 的轮次边界 `flush()` 提示转发给 OTel 提供方。** 该方案被否决，因为它会在原生异步批处理器之外创建第二个执行 flush 的组件，使普通轮次边界与最终排空争用。可由部署方调节的 `scheduledDelayMillis` 负责导出节奏，`shutdown()` 负责完整排空。仅当某个部署提出 `scheduledDelayMillis` 无法满足的轮次边界延迟要求时才恢复此功能，并且应通过留存的处理器实现，而非在提供方范围执行 flush。
