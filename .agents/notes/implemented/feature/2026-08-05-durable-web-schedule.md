...
The [`examples/web-schedule`](../../../../examples/web-schedule/README.md) overlay explicitly loads `@seekdeep-ai/seekdeep-time-context` and `@seekdeep-ai/seekdeep-schedule`; the default Web tree remains unchanged. Schedule observes only root Agents published after the plugin loads and installs its three tools plus one disposable owner in that Agent scope. Cold history reads, already-published roots, child Agents, and other hosts do not activate it.
...
Rust production lives in [`crates/schedule`](../../../../crates/schedule/src/runtime.rs). Its timer decisions take wall time through an injected `ScheduleClock`, and reminder identity/framing crosses an injected `ScheduleMessageFactory`, so rollback, forward-jump, invalid-clock, and construction failures are deterministic in tests. Each future root receives one plugin-owned child fiber containing its tools and runtime; descendants receive a plugin-owned restriction that masks inherited Schedule tools. Agent disposal or Schedule HMR therefore removes the exact registrations without leaving a descendant mask or timer generation behind.
