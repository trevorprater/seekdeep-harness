...
The workflow package exposes browser-safe run and observation vocabulary through `@seekdeep-ai/seekdeep-workflow/types`; live `Agent` requests and control handles remain Host-only. `@seekdeep-ai/seekdeep-tool-workflow/types` owns the four Session events. Client code imports only these type faces, so the Host and Client TypeScript programs share the durable contract without merging Host Cordis context.
