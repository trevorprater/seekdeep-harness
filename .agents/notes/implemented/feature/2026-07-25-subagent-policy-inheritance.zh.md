...
- 无密钥源快照与确定性的 Rust 组装子 agent 回归测试都只把父级设为 `read-only`，而部署默认值保持为 `workspace-write`。若移除捕获，子 agent 的委派事件、模型可见策略上下文与被拒的磁盘写入会一起失败；Rust 敏感性检查随后会在较宽的默认策略下实际写入文件。
- 每次委派最多增加两条仅日志事件。两个策略服务的可选 peer 类型由 `seekdeep-subagent` 拥有——其共享辅助函数持有 `ctx.get` 消费；未组合任一服务的组合保持原有行为。进程外子 agent 仍采用自身的部署策略，正在运行的子 agent 不跟随父级后续切换。
