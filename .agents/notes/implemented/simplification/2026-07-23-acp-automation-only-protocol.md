...
Keyless Rust tests drive the bridge through real `AgentLoop` instances and a duplex JSON-RPC connection. They pin negotiation, baseline prompt validation and resource-link rendering, committed output, max-token prompt semantics, independent prompt slots, cancellation, approval ownership, multi-session isolation, and whole-connection teardown. The pinned source's 107 unit cases remain the differential oracle; the broader retry, pre-step, transport-failure, and aggregate-disposal test rows remain pending until every branch has equivalent Rust evidence.
