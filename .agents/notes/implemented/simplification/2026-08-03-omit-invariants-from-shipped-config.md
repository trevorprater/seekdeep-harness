# Agent Note: Omit runtime invariants from shipped seekdeep config
...
The built CLI config-dump test checks both shipped surfaces and rejects either the service entry or any `@seekdeep-ai/seekdeep-*/invariant` entry.
...
- Ordinary `seekdeep` TUI and Web runs install no invariant listeners or trace state and cannot fail through `InvariantError`.
