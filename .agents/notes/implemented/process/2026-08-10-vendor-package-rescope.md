...
The nine packages under `vendor/` kept their upstream npm names (`cordis`, `cosmokit`, `schemastery`, `@cordisjs/plugin-*`). That premise does not survive publication: every harness package declares `cordis` as a peer dependency, so a consumer installing `@seekdeep-ai/seekdeep-*` must resolve it from the registry, which means publishing the harness publishes this framework layer too. Publishing it under the upstream names squats them on the registry, and where that registry proxies npmjs, the same-name entries shadow the real upstream packages and install the wrong framework into unrelated projects.
...
|---|---|---|
| `cordis/` | `@seekdeep-ai/cordis` | `cordis` |
| `cosmokit/` | `@seekdeep-ai/cosmokit` | `cosmokit` |
| `schemastery/` | `@seekdeep-ai/schemastery` | `schemastery` |
| `loader/` | `@seekdeep-ai/cordis-plugin-loader` | `@cordisjs/plugin-loader` |
| `include/` | `@seekdeep-ai/cordis-plugin-include` | `@cordisjs/plugin-include` |
| `group/` | `@seekdeep-ai/cordis-plugin-group` | `@cordisjs/plugin-group` |
| `timer/` | `@seekdeep-ai/cordis-plugin-timer` | `@cordisjs/plugin-timer` |
| `hmr/` | `@seekdeep-ai/cordis-plugin-hmr` | `@cordisjs/plugin-hmr` |
| `logger-console/` | `@seekdeep-ai/cordis-plugin-logger-console` | `@cordisjs/plugin-logger-console` |

The rewrite touches only **delimited, complete package-name tokens**: quoted or backticked specifiers (optionally with a `/subpath`), `package.json` names and dependency keys, `cordis.yml` `name:` values, and `tsconfig.base.json` `paths` keys. Identically spelled strings that are not package names therefore stayed as they were: the `cordis.yml` config-file family, the Loader's literal `cordis:` builtin prefix (`cordis:include`, `cordis:group` — see `vendor/loader/src/config/tree.ts`), kind strings like `cordis-config-entry`, `@seekdeep-ai/seekdeep-tool-cordis`, Schemastery's upstream `Symbol.for('schemastery')` and `vendor:` metadata field, the `packages/<group>/` directory names in `GROUP_ORDER` (`scripts/gen-module-graph.ts`, `scripts/gen-doc-graphs.ts`), and the upstream install instructions in `vendor/*/README.md`.

Two classes are invisible to a token rule and are checked at their named sites. First, property access — `manifest.peerDependencies?.cordis` — where TypeScript cannot catch a stale `Record<string, string>` key. Second, constants that carry the name as data: the vendored set in `check-workspace-constraints.ts`, the Rust [Cordis config verifier](../../../../crates/repository-tools/src/cordis_config_verifier.rs) group/include names, the `declare module` target strings in `cordis-walk.ts` and typert's `analyzer.ts`, and `alwaysBundle` in `app-boot/tsdown.config.ts`.
...
- No upstream name remains in the publication set. `publish-npm-baseline.ts` now requires every published package to be `@seekdeep-ai/*` with no vendored exemption, so regressing the rename fails before packing.
...
- `pnpm-workspace.yaml` drops the `cordis` and `@cordisjs/plugin-loader` `minimumReleaseAgeExclude` entries, which can no longer be fetched from a registry, and `knip.json` drops the `@cordisjs/.+` ignore pattern that `@seekdeep-ai/.+` already covers.
...
**Keep the upstream names and exclude `vendor/` from publication.** Rejected because every harness package declares `cordis` as a peer dependency, so an installed `@seekdeep-ai/seekdeep-*` would have no resolvable framework.
