...
- **Constraints are package-manager-independent.** `pnpm run constraints` invokes the [Rust workspace checker](../../../../crates/repository-tools/src/workspace_constraints.rs). It checks package hierarchy, release-family versions, publication metadata and payloads, matching Cordis peer/dev dependencies, compatibility exports, `workspace:` dependency ranges, and Host/Client project-reference isolation. The [differential fixtures](../../../../crates/repository-tools/tests/workspace_constraints_parity.rs) compare complete ordered diagnostics with the pinned source command; the checker does not silently omit missing discovery roots or rewrite manifests.
