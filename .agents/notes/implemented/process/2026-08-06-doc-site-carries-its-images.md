...
`docs_source_files()` reports the placed images alongside the Markdown, so the dev server's watcher re-projects when a screenshot is replaced instead of serving the previous copy until something touches the page.

`place_image` is optional because `rewrite_markdown` is also called directly by its spec, where no generated tree exists. Without it the GitHub-raw fallback points at the public source home, which keeps that seam honest for a consumer that only rewrites text.
...
Images in published documentation work regardless of who is reading or whether the repository is public, and the site build has no runtime dependency on GitHub for them. The generated tree carries each page's referenced images; the model-provider guide supplies two screenshots for each locale.
