...
Pre-commit 不运行类型分析、测试、快照、文档检查、构建、`hygiene` 或门禁调度器。Pre-push 只增加仓库类型检查所需的 Host 约定构建。可选运行的 `check:all` 包脚本独立于这些钩子，从 Rust [门禁调度器](../../../../crates/repository-tools/src/run_gates.rs)中选择 `check-all` 清单；它是贡献者命令，而非对 agent 的指令。
