...
The [Rust release tool](../../../../crates/python-release/src/main.rs) owns version resolution, isolated package staging, exact runtime dependency pins, legal-file inclusion, native build-hook policy, and wheel verification. Workflow version steps consume its `version --github-output` command. A generated Hatch binding forwards build-hook inputs and results to the compiled Rust tool; it carries no duplicate platform or payload policy. Distribution filenames use `seekdeep_harness_sdk` and `seekdeep_harness_runtime_bin`, while import namespaces remain `deepseek_harness` and `deepseek_harness_runtime`.

The builder rejects packages without their generated Python binding entry point. [Fixture-wheel tests](../../../../crates/python-release/tests/release_parity.rs) exercise real Hatch builds and native payload checks; [workflow tests](../../../../crates/python-release/tests/workflow_contract.rs) preserve publication authorization and ordering. These checks establish the packaging mechanism, not completion of the Python API bindings, native executable builder, or complete installed-SDK smoke path, which remain separate porting gaps.
